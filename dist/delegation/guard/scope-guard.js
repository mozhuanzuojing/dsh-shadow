...
export const actionWithinScope = (allowedScope, action) => allowedScope.some((s) => action.includes(s) || s.includes(action));
export const assertScopeWithin = (allowedScope, action) => ({
...
export const permissionNotOwnership = (p) => !OWNERSHIP.test([p?.permission, p?.scope, (p?.constraint || []).join(",")].filter(Boolean).join(" "));
export const assertPermissionNotOwnership = (p) => ({
