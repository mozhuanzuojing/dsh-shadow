 ...
 */
export const renderIndexBudgeted = (idx, maxChars) => {
    ...
    if (partial.length)
        note.push(`> 部分返回的段：${partial.map((d) => `「${d.title}」(前 ${d.kept} 行 / 共 ${d.total} 行)`).join(" · ")}`);
    if (dropped.length)
        note.push(`> 未返回的段：${dropped.map((d) => `「${d.title}」(${d.lines} 行)`).join(" · ")}（主题索引/意识轨迹是**派生视图**：按主题穿透比整篇读回更省）`);
