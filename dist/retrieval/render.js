...
import { lossLine, neverWorseChars } from "./loss.js";
/** 索引路径的默认恢复句柄（`_index.md` 自身）。传 `null` = 显式声明没有源路径。 */
export const INDEX_HANDLE = { file: ".shadow/_index.md" };
/**
...
const shortTitle = (t) => (t.length > SHORT_TITLE_CHARS ? `${t.slice(0, SHORT_TITLE_CHARS)}…（段名共 ${t.length} 字）` : t);
 ...
 */
export const splitIndexSections = (text) => {
 ...
 */
export const renderIndexBudgeted = (idx, maxChars, source = INDEX_HANDLE) => {
    ...
    for (const s of sections) {
        ...
        if (remaining <= 0) {
            dropped.push({ title: s.title, lines: s.body.split("\n").length });
            continue;
        }
        ...
        if (take.length) {
            kept.push(take.join("\n"));
            used += size;
            partial.push({ title: s.title, kept: take.length, total: bodyLines.length });
        }
        else
            dropped.push({ title: s.title, lines: bodyLines.length });
    }
    ...
    if (loss)
        note.push(loss);
    if (partial.length)
        note.push(`> 部分返回的段：${partial.map((d) => `「${shortTitle(d.title)}」(前 ${d.kept} 行 / 共 ${d.total} 行)`).join(" · ")}`);
    if (dropped.length)
        note.push(`> 未返回的段：${dropped.map((d) => `「${shortTitle(d.title)}」(${d.lines} 行)`).join(" · ")}（主题索引/意识轨迹是**派生视图**：按主题穿透比整篇读回更省）`);
