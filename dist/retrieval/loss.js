...
//   （SQLite + gzip）+ 内容哈希去重 + 老化；而**本仓的权威源就是工作区里的文件**（`.shadow/<日期>/<入口>.md`、
//   `.shadow/_index.md`）⇒ **恢复句柄 = 源文件路径 + 定位，零新增存储**（不需要哈希主键、去重与老化语义
...
export const handleText = (h) => h && String(h.file || "").trim() ? `\`${h.file}${h.locator ? `#${h.locator}` : ""}\`` : null;
/** 形态词（披露用；`none` 不进这个表 —— 无损失就**不披露**）。 */
export const LOSS_WORD = {
 ...
 */
export const neverWorseChars = (filtered, raw) => filtered.length > raw.length ? raw : filtered;
 ...
 */
export const lossLine = (o) => {
    const word = LOSS_WORD[o.loss];
    if (!word)
        return null; // `loss: "none"`（或未知取值）⇒ 没有损失 ⇒ 不披露，避免噪声
 ...
 */
export const excerptWorthwhile = (text, min = EXCERPT_WORTHWHILE_CHARS) => String(text || "").trim().length > min;
/**
 * 召回路径的**分层省略披露**（一次，O(1) 输出；逐条句柄最多列 3 个 + 规则）。
 * 返回 `""` = 本次没有「本是够长却被省略」的条目 ⇒ 不添一句话（零额外文字）。
 */
export const tierLossNote = (o) => {
