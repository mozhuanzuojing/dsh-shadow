...
export const readIdentity = async (fs, ws, fallbackId) => {
...
export const renderIdentity = (i) => {
    const lines = ["[Identity]"];
    lines.push(`id ${i.id}${i.name ? ` · ${i.name}` : ""}${i.role ? ` · ${i.role}` : ""}`);
    if (i.values?.length)
        lines.push(`价值观 ${i.values.join("、")}`);
    if (i.principles.length)
        lines.push(`原则 ${i.principles.join("、")}`);
    if (i.antiPatterns?.length)
        lines.push(`反模式 ${i.antiPatterns.join("、")}`);
    if (i.decisionStyle?.length)
        lines.push(`决策风格 ${i.decisionStyle.join("、")}`);
    if (i.boundaries?.length)
        lines.push(`边界 ${i.boundaries.join("、")}`);
    if (i.observerLens)
