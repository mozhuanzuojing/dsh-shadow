...
export const projectContext = async (fs, ws, memories, task, soul, verifyEvidence, lens, identity, intent) => {
    ...
    const experiences = [];
    const exclReason = {};
    const unreadable = []; // 读不出的记忆（**既不进 relevant 也不进 excluded** —— 必须单列）
    ...
    for (const mm of memories) {
        ...
        if (!text) {
            unreadable.push(mm.rel.split("/").pop());
            continue;
        }
...
export const renderProjection = (p, task, project, ctx) => {
    ...
    if (p.unreadable?.length)
        lines.push(`unreadable: ${p.unreadable.slice(0, 6).join("、")}（读不出 ≠ 不相关：它们**没有**参与任何桶的判定）`);
    if (p.capDropped)
        lines.push(`> ⚠ 另有 ${p.capDropped} 条具体路径**未核验**（单条上限）：「不确定」只覆盖已核验的那些`);
    if (p.visible?.length)
        lines.push(`visible: ${p.visible.slice(0, 4).join("、")}`);
    if (p.distortion?.reason)
        lines.push(`distortion: ${p.distortion.reason}${p.distortion.byIntent ? ` · ${p.distortion.byIntent}` : ""}`);
    if (p.unc.length)
