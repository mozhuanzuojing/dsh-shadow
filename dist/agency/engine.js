...
export const buildAgencyEvent = async (fs, ws, args) => {
    ...
    if (!hasNoAutonomousTransition(result))
        return { ok: false, reason: "执行结果禁自主转换（Bounded→Autonomous 须外部权威+显式协议变更）" };
    if (/autonomous|自主|更多自由|expanded scope|self purpose|control the world|拥有世界|控制世界|become self|become autonomous|autonomous agency|自主转换/i.test(result))
        return { ok: false, reason: "执行结果禁 Autonomy/所有权（Agency 有行动能力 ≠ 有自我目的）" };
    if (!isNotAgencyExpansion(result))
        return { ok: false, reason: "执行结果禁扩权（Success→Observation/Validation，非 Success→Authority；禁 agencyLevel++/allowedActions.add）" };
    if (!hasNoOwnership(result))
        return { ok: false, reason: "执行结果禁所有权声称（permission to modify ≠ ownership of）" };
    if (!hasNoIdentityClaim(result))
        return { ok: false, reason: "执行结果禁身份声称（successful action ≠ 『我是更好规划者』；identity 只来自 Reflection→Candidate→Evaluator）" };
    if (!isNotInternalReason(result))
        return { ok: false, reason: "执行结果禁内部理由（系统只答『因为外部目标X/授权Y/约束Z』，不能答『因为我认为应该这样』）" };
