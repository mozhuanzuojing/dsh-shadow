 ...
 */
export const distributionMatchesCandidates = (d) => {
    if (d?.reportedDistribution === null)
        return true; // 显式「本引擎不产出分布」⇒ 本条不适用
    if (!d?.reportedDistribution)
        return false; // undefined = 没填，不是「没有分布」
    const keys = Object.keys(d.reportedDistribution);
    const unique = new Set(keys);
    if (unique.size !== keys.length)
        return false; // 同一个键出现两次（理论上不可达，仍显式挡）
    if (keys.length !== d.candidates.length)
        return false;
    const set = new Set(d.candidates);
    return keys.every((k) => set.has(k));
};
/** 判据 ⑤：分布里每个值都是**有限数**且 ∈ [0,1]。 */
export const distributionInRange = (d) => {
    if (d?.reportedDistribution === null)
        return true; // 同上：不适用
    if (!d?.reportedDistribution)
        return false;
    return Object.values(d.reportedDistribution).every((v) => typeof v === "number" && Number.isFinite(v) && v >= 0 && v <= 1);
};
/** 判据 ⑥：分布求和 ≈ 1（容差见 `DISTRIBUTION_SUM_TOLERANCE`）。 */
export const distributionSumsToOne = (d) => {
    if (d?.reportedDistribution === null)
        return true; // 同上：不适用
    if (!d?.reportedDistribution)
        return false;
...
export const declarationViolations = (d) => {
    ...
    if (!distributionInRange(d))
        out.push("reportedDistribution 里有非有限数或超出 [0,1] 的值");
    if (!distributionSumsToOne(d))
        out.push(`reportedDistribution 求和偏离 1 超过容差 ${DISTRIBUTION_SUM_TOLERANCE}`);
    return out;
};
/** 判据合取。 */
