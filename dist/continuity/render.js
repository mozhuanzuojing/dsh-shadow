export const renderObserverContext = (data) => {
    const lines = ["[Observer Context]（global observer 层：谁保持连续）"];
    const b = data.boundary;
    if (b)
        lines.push(`boundary: planningNoObjective ${b.planningCannotCreateObjective} · recallNoKnowledge ${b.recallCannotCreateKnowledge} · adaptNoAuthority ${b.adaptationCannotIncreaseAuthority} · delegNoExpand ${b.delegationCannotExpandAuthority} · agencyNoPurpose ${b.agencyCannotCreatePurpose}`);
    else
        lines.push("boundary: (无)");
...
export const renderWorkspaceContext = (rows) => {
    const lines = ["[Workspace Context]（project world 层：这个世界是什么）"];
    if (rows.length)
        for (const r of rows)
            lines.push(`${r.kind} · ${r.content} · ${r.createdAt}`);
    else
        lines.push("(无 workspace 记录)");
    lines.push("（只读当前 workspace 的 .dsh-shadow；不提供 observer 边界——Constraint ⊃ Context，非 Memory Union）");
    return lines.join("\n");
};
export const renderContinuityIndex = (ri) => {
