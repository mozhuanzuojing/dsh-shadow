...
export async function runReadShadow(deps, args, exec) {
    ...
    if (String(args?.mode) === "episode" || String(args?.mode) === "decision") {
        ...
        if (forgetQ.enabled === true)
            memories = memories.filter((mm) => !isForgettable(mm.rel, metaQ, forgetQ));
        const parsed = [];
    ...
    if (String(args?.mode) === "temporal") {
        ...
        if (args?.perceptionOnly || args?.identityContext) {
            ...
            if (!node)
                return scrubFinal(RECALL_PREFIX + "（无 Temporal 节点）" + flushWarn);
    ...
    if (String(args?.mode) === "offline") {
        const sw = buildSleepWindow({ observerId: agent?.id || "unknown", from: String(args?.from || ""), to: String(args?.to || today()), trigger: args?.trigger || "scheduled" });
    ...
    if (String(args?.mode) === "simulate") {
        ...
        if (!a.ok)
            return scrubFinal(RECALL_PREFIX + "[Simulation Rejected] " + a.reason + "（Assumption ≠ Fact：须 'Assume X'，禁 'X will cause'）" + flushWarn);
        const basedOn = args?.basedOn || [String(args?.subject || "")].filter(Boolean);
        ...
        if (!b.ok)
            return scrubFinal(RECALL_PREFIX + `[Simulation Rejected] ${b.reason}` + flushWarn);
        if (!outcomeHasLineage(outcome))
            return scrubFinal(RECALL_PREFIX + "[Simulation Rejected] 无 derivedFrom（lineage 不完整）" + flushWarn);
    ...
    if (String(args?.mode) === "candidate") {
        const conds = args?.assumptions || [String(args?.condition || "Assume change")].filter(Boolean);
        const candidate = { id: `ac-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, basedOnSimulation: args?.basedOnSimulation || [], assumedConditions: conds, proposedChange: String(args?.proposedChange || ""), uncertainty: Number(args?.uncertainty) || 0.5 };
        const g = assertCandidateClean(candidate);
    ...
    if (String(args?.mode) === "execute") {
        ...
        const g = assertExecutionEvent(exec);
        if (!g.ok)
            return scrubFinal(RECALL_PREFIX + "[Action Rejected] " + g.reason + flushWarn);
        if (!exec.candidateId)
            return scrubFinal(RECALL_PREFIX + "[Action Rejected] 无 candidateId（需先 mode:candidate + 批准，SimulationOutcome 不直接执行 Action）" + flushWarn);
    ...
    if (String(args?.mode) === "agency-context") {
        ...
        if (!c.ok || !c.ctx)
            return scrubFinal(RECALL_PREFIX + "[AgencyContext Rejected] " + c.reason + flushWarn);
    ...
    if (String(args?.mode) === "agency-select") {
        ...
        if (!s.ok || !s.sel)
            return scrubFinal(RECALL_PREFIX + "[AgencySelection Rejected] " + (s.reject || "") + flushWarn);
    ...
    if (String(args?.mode) === "horizon-context") {
        ...
        if (!c.ok || !c.ctx)
            return scrubFinal(RECALL_PREFIX + "[Interaction Rejected] " + c.reason + flushWarn);
    ...
    if (String(args?.mode) === "horizon-summary") {
        ...
        if (!s.ok || !s.summary)
            return scrubFinal(RECALL_PREFIX + "[HistorySummary Rejected] " + s.reason + flushWarn);
    ...
    if (String(args?.mode) === "horizon-event") {
        ...
        if (!e.ok || !e.event)
            return scrubFinal(RECALL_PREFIX + "[ContinuityEvent Rejected] " + e.reason + flushWarn);
    ...
    if (String(args?.mode) === "horizon-link") {
        ...
        if (!l.ok || !l.link)
            return scrubFinal(RECALL_PREFIX + "[InteractionLink Rejected] " + l.reason + flushWarn);
    ...
    if (forgetCfgQ.enabled === true && Array.isArray(memories)) {
        memories = memories.filter((mm) => !isForgettable(mm.rel, meta, forgetCfgQ));
    }
