...
export function retiredApiMessage(args) {
...
export async function runReadShadow(deps, args, exec) {
    ...
    if (retired)
        return scrubFinal(RECALL_PREFIX + retired + flushWarn);
    ...
    const viaRead = await dispatchReadQuery(deps, args, exec, {
        ...
        clearDirty: deps.derivedIndexClearDirty ? (rels) => deps.derivedIndexClearDirty(ws, rels) : undefined,
    });
    ...
    if (debugMode)
        diag.push(renderAuditStreamDiag(await readAuditStream(fs, ws)));
    ...
    if (args?.verifyEvidence) {
        ...
        for (const text of texts.slice(0, 3)) {
            ...
            for (const p of evidencePathsOf(text).filter(isPathLike).filter(isConcreteLocator).slice(0, 6)) {
                ...
                if (r.status === "unavailable" && !unavailableRef)
                    unavailableRef = { provider: r.source, reason: r.provenance?.reason };
