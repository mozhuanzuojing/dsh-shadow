...
export async function runReadShadow(deps, args, exec) {
    ...
    if (viaObserverKernel !== undefined)
        return viaObserverKernel;
    const viaValidation = await runValidation(deps, args, { fs, ws, flushWarn });
    if (viaValidation !== undefined)
    ...
    if (viaContVerify !== undefined)
        return viaContVerify;
    ...
    if (Array.isArray(memories)) {
        memories = memories.filter((mm) => !isForgettable(mm.rel, meta, forgetCfgQ) && !isCompacted(meta, mm.rel));
    }
