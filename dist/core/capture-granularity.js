// dsh-shadow —— core/capture-granularity.ts：**记录粒度判据**（v1.19.0 / `adr/0097`）。
//
// ## 问题（实测，见 `adr/0097` §1）
//
...
// ## 为什么不需要新增字段就能复核（门用它）
//
// `core/memory.ts:60` 的 `> 证据链：来源(动作·用户·agent·决策)` 已经把「这批里有什么」编码进去了 ——
 ...
 */
export const isAuditBatch = (arr) => (arr || []).every((e) => String(e?.kind || "") === ACTION_KIND);
/** 配置约定的落点是不是审计流（`adr/0097` D5）：**默认是**；显式 `"memory"` 才回到旧行为。 */
export const echoToAudit = (cfg) => cfg?.capture?.echo !== ECHO_MEMORY;
/**
 ...
 */
export const auditStreamRel = (date) => `${SHADOW_ROOT}/audit/${date}.jsonl`;
/**
 * 事件 → 可落盘的正文行。
 * **记忆正文与审计流共用这一份归一化**（同 `sanitizeText`、同 `isUnsafe` 判据）——
 * 否则两条路径会各自演化出不同的「什么算一行安全正文」。
 */
export const bodyLinesOf = (traces, entry) => (traces || []).map((t) => `- [${t.at}] [${t.comp || entry}] ${sanitizeText(t.text)}`).filter((l) => !isUnsafe(l));
/** 审计流的一行（对象形态）：字段 = 原本会进记忆正文的那些 ⇒ **一字不丢**，只是不再各占一个 inode/样板头。 */
const auditRecordOf = (t, ctx) => ({
    at: t.at,
    kind: t.kind,
    comp: t.comp,
