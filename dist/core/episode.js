...
export const deriveDecisions = (parsed, opts = {}) => {
    ...
    for (const p of parsed) {
        ...
        for (const d of p.decisions) {
            const at = `${p.date} ${fmt(p.time)}`;
            if (needle && !`${d} ${p.entry}`.toLowerCase().includes(needle))
                continue;
...
export const renderDecisions = (dl) => {
    if (!dl.count)
        return "（无决策血缘：当前记忆树未采集到决策 —— 决策来自 goal/changed 与用户拍板）";
    const parts = [`# Decision Lineage · 共 ${dl.count} 条决策`, ""];
    const entries = Object.keys(dl.byEntry).sort();
    for (const entry of entries) {
        parts.push(`## ${entry}`);
        for (const d of dl.byEntry[entry])
            parts.push(`- ${d.at.slice(5)} ${D(d.text)}  ·  ${d.rel.split("/").slice(-2).join("/")}`);
        parts.push("");
    }
