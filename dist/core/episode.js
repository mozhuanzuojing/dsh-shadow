 ...
 */
export const isSessionMetadataAtom = (p) => p.entry === "shadow" && p.userMessages.length > 0 && !p.materials.length && !p.decisions.length;
...
export const isMetadataMemoryText = (text) => {
    const t = String(text || "");
...
export const parseMemory = (text, rel, name) => {
    ...
    const addDecision = (stmt, source, reason) => {
        const s = scrubUnsafe(String(stmt || "")).trim();
        if (!s)
            return;
        const st = s.slice(0, 80);
        if (!decisions.includes(st))
            decisions.push(st);
        if (!decisionEvents.some((e) => e.statement === st))
            decisionEvents.push({ statement: st, source, reason: scrubUnsafe(String(reason || "")).trim().slice(0, 120) });
    };
...
export const deriveEpisodes = (parsed, opts = {}) => {
    ...
    const append = (e, p) => {
        ...
        if (p.entry && !e.entries.includes(p.entry))
            e.entries.push(p.entry);
        if (!e.objective && p.goal && p.goal !== e.title) {
            e.objective = p.goal;
            e.title = p.goal;
        }
        for (const d of p.decisions)
            if (!e.decisions.some((x) => x.text === d))
                e.decisions.push({ text: d, rel: p.rel, at });
        for (const a of p.actions)
            if (!e.actions.some((x) => x.text === a))
                e.actions.push({ text: a, rel: p.rel, at });
...
export const deriveDecisions = (parsed, opts = {}) => {
    const byEntry = {};
    let count = 0;
    ...
    for (const p of parsed) {
        ...
        for (const ev of events) {
            ...
            const at = `${p.date} ${fmt(p.time)}`;
            if (needle && !`${text} ${p.entry}`.toLowerCase().includes(needle))
                continue;
    ...
    for (const k of Object.keys(byEntry))
        byEntry[k].sort((a, b) => a.at.localeCompare(b.at));
    return { byEntry, count };
};
// ── 渲染 ──
const D = (text) => (String(text || "").slice(0, 60));
/** _index.md 里的「任务回溯（Episodes）」段（保守：不引号包裹入口名，避免与主题索引撞名）。 */
export const episodesIndexText = (eps, limit = 8) => {
    if (!eps.length)
        return "## 任务回溯（Episodes）\n\n（暂无连续任务片段，当前仅原子记忆）\n";
...
export const renderEpisodes = (eps, topic) => {
    ...
    if (!filtered.length)
        return needle ? `（无匹配 episode：${topic}）` : "（暂无 Episode 派生结果）";
    const parts = filtered.map((e) => {
        ...
        if (e.objective && e.objective !== e.title)
            seg.push(`- 目标：${D(e.objective)}`);
        if (e.entries.length)
            seg.push(`- 涉及入口：${e.entries.slice(0, 8).join("、")}`);
        if (e.materials.length)
            seg.push(`- 背景/材料：${e.materials.slice(0, 8).join("、")}`);
        seg.push(`- ${e.memoryCount} 条记忆 · ${e.decisions.length} 决策 · ${e.actions.length} 动作`);
        if (e.decisions.length) {
            seg.push(`- 决策链：`);
            for (const d of e.decisions.slice(0, 12))
                seg.push(`    · ${d.at.slice(5)} ${D(d.text)}`);
        }
        if (e.actions.length) {
            seg.push(`- 动作摘要：`);
            for (const a of e.actions.slice(0, 10))
                seg.push(`    · ${a.at.slice(5)} ${D(a.text)}`);
        }
...
export const renderDecisions = (dl) => {
    if (!dl.count)
        return "（无决策血缘：当前记忆树未采集到决策 —— 决策来自 goal/changed、用户拍板、assistant 明确决策；Confirmation 不算决策，Reason 不补写）";
    const parts = [
