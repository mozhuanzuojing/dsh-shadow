...
export const deriveL1 = (faces) => {
    ...
    for (const f of sorted)
        lines.push(`- [${f.time || "------"}] ${f.entry || "(无入口)"} · ${f.name}`);
...
const cap = (s, max) => (s.length <= max ? s : s.slice(0, Math.max(0, max - 1)).trimEnd() + "…");
/**
 * 渲染 sidecar 全文。
 *
 ...
 */
export const renderSidecar = (date, l1, coverage) => {
...
export const parseSidecar = (text) => {
    ...
    if (l0 === undefined || l1 === undefined || cov === undefined)
        return undefined;
    const num = (k) => {
    ...
    if (covered === undefined || pending === undefined)
        return undefined;
 ...
 */
export const sidecarDrift = (faces, sidecarText) => {
    const parsed = parseSidecar(sidecarText);
    if (!parsed)
        return ["sidecar 无法解析（缺 L0 / L1 / 覆盖率 段）"];
    const errs = [];
    // ① L1 必须**逐字节**等于「由当前源重新派生」的结果 —— 这是 ②（层间不漂移）的可执行形式。
    const expectL1 = deriveL1(faces);
    if (parsed.l1 !== expectL1)
        errs.push("L1 与当前源重新派生的结果不一致（sidecar 落后于源头）");
    // ② L0 必须等于「由 sidecar 自己的 L1 抽取」的结果 —— 若不等，说明 L0 被单独改过（层间已漂移）。
    const expectL0 = deriveL0(parsed.l1);
    if (parsed.l0 !== expectL0)
        errs.push("L0 与 L1 的抽取结果不一致（L0 被单独改过 ⇒ 层间漂移）");
    ...
    if (parsed.coverage.pending !== 0)
        errs.push(`覆盖率自报 pending=${parsed.coverage.pending}（应为 0）`);
    return errs;
};
/** sidecar 的仓库相对路径（`_` 前缀 ⇒ 不进 `listMemories`）。 */
export const sidecarRel = (date) => `${SHADOW_ROOT}/${date}/${SIDECAR_NAME}`;
