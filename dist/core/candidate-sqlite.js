...
const okSet = (sources, atoms) => ({ provider: "sqlite", state: "ok", sources, atoms });
const failSet = (state, reason) => ({ provider: "sqlite", state, unavailable: true, reason, sources: [], atoms: [] });
 ...
 */
const coarseOf = (entries) => {
    const parts = [];
    for (const e of entries || []) {
        if (!isSourceDirEntry(e))
            continue; // 与投影指纹同一判据（日期目录 + resources）
        const v = e.version;
...
const coarseMap = (s) => {
    ...
    for (const seg of String(s || "").split("|")) {
        const i = seg.indexOf(":");
        if (i > 0)
            m.set(seg.slice(0, i), seg.slice(i + 1));
    }
    return m;
};
/** 路径**必须**由 `resolve` 得到、由 `processPath` 产出（纪律②；不做字符串拼接）。 */
const processPathOf = async (fs, ws, name) => {
    const target = await fs.resolve(`${ws}/${SHADOW_ROOT}/${name}`, { cwd: ws });
    const p = fs.processPath(target);
    if (typeof p !== "string" || !p)
        throw new Error("processPath 返回空路径");
    return p;
};
/** 打开连接：每次调用 open→用→close（**不做模块级连接缓存** —— 那会跨实例泄漏）。 */
const openDb = (DatabaseSync, path) => {
    const db = new DatabaseSync(path);
    try {
        db.exec("PRAGMA busy_timeout = 3000");
    }
    catch { /* 老版本不支持该 PRAGMA 不算失败 */ }
    return db;
};
const closeQuietly = (db) => { try {
    db?.close?.();
}
catch { /* 关闭失败不改变结论 */ } };
const prepared = (db) => ({
    insSource: db.prepare("INSERT OR REPLACE INTO source(rel,date,name,size,version,shadow_root) VALUES(?,?,?,?,?,?)"),
    insAtom: db.prepare(`INSERT OR REPLACE INTO atom(rel,date,time,entry,project,agent,goal,kind,decisions_json,decision_events_json,
...
const writeMeta = (st, key, value) => { st.metaSet.run(key, value); };
/** 记忆文件判据与 `listMemories` **同判据**（`.md`、非 `_index.md`、非 `_` 前缀）—— 判据收一处：
 *  唯一实现在 `persistence/files.ts` 的 `isMemoryFileName`（两处各写一遍就会漂移，见它的注释）。 */
const enumDateDir = async (fs, ws, date) => {
    const files = (await fs.listDir(await fs.resolve(`${ws}/${SHADOW_ROOT}/${date}`, { cwd: ws }))) || [];
    const out = [];
    for (const f of files) {
        const n = String(f?.name || "");
        if (!isMemoryFileName(n))
            continue;
        out.push({ rel: `${SHADOW_ROOT}/${date}/${n}`, name: n, size: Number.isFinite(Number(f.size)) ? Number(f.size) : -1, version: String(f.version ?? "") });
    }
    return out;
};
const insParsed = (st, p, date, size, version) => {
    st.insSource.run(p.rel, date, String(p.rel).split("/").pop() || "", size, version, SHADOW_ROOT);
    st.insAtom.run(p.rel, p.date, p.time, p.entry, p.project, p.agent, p.goal, p.kind, JSON.stringify(p.decisions ?? []), JSON.stringify(p.decisionEvents ?? []), JSON.stringify(p.userMessages ?? []), JSON.stringify(p.materials ?? []), JSON.stringify(p.actions ?? []), JSON.stringify(p.thinkLines ?? []), JSON.stringify(p.lineage ?? {}), p.body ?? "", p.lineage?.createdAt ?? "");
};
/** 把坏索引**挪走**（而不是留在原地）⇒ 下一次调用走整体重建。挪不动就删；都失败则如实返回 false。 */
const markCorrupt = async (fs, ws, indexHost) => {
    try {
...
const readSchema = (db) => {
    try {
        ...
        if (missing.length)
            return { bad: true, reason: `表缺失（${missing.join(",")}）` };
...
const querySet = (db, keep) => {
    ...
    for (const row of st.allAtoms.all()) {
        if (!kept.has(String(row.rel)))
            continue;
        try {
            atoms.push(atomFromRow(row));
        }
        catch (e) {
            const err = new Error(`索引行反序列化失败（${row.rel}）：${msgOf(e)}`);
            err.__corrupt = true;
            throw err;
        }
    }
 ...
 */
const syncDirs = async (db, fs, ws, dirs, forceRels, full, removedDirs = []) => {
    const st = prepared(db);
    db.exec("BEGIN");
    try {
        if (full) {
            ...
            for (const r of st.allDates.all()) {
                ...
                if (!dirs.includes(d)) {
                    st.delByDateSource.run(d);
                    st.delByDateAtom.run(d);
                }
            }
        }
        ...
        for (const dir of dirs) {
            ...
            for (const f of files) {
                ...
                if (!text) {
                    st.delRel.run(f.rel);
                    st.delSourceRel.run(f.rel);
                    continue;
                }
                try {
                    insParsed(st, parseMemory(text, f.rel, f.name), dir, f.size, f.version);
                }
                catch {
                    st.delRel.run(f.rel);
                    st.delSourceRel.run(f.rel);
            ...
            for (const rel of prevByRel.keys())
                if (!byRel.has(rel)) {
                    st.delRel.run(rel);
                    st.delSourceRel.run(rel);
                }
            // soundness 网：目录实数必须与索引行数一致，否则整体重建
            const cnt = st.countByDate.get(dir);
            if (Number(cnt?.n ?? -1) !== files.length) {
                db.exec("ROLLBACK");
                return true;
            }
        }
        ...
        for (const rel of forceRels) {
            ...
            try {
                insParsed(st, parseMemory(text, rel, String(rel).split("/").pop() || ""), dirOfRel(rel), -1, "");
            }
            catch {
                st.delRel.run(rel);
                st.delSourceRel.run(rel);
 ...
 */
export const createSqliteCandidateProvider = (opts) => {
    const load = opts?.load ?? (async () => await import(SQLITE_SPEC).catch(() => undefined));
    return {
        id: "sqlite",
        async provide(fs, ws, cfg, keep, callOpts) {
            ...
            let indexHost;
            try {
                indexHost = await processPathOf(fs, ws, INDEX_FILE);
            }
            catch (e) {
                return failSet("unavailable", `拿不到宿主绝对路径：${msgOf(e)}`);
            }
            if (!isAbsolute(indexHost))
                return failSet("unavailable", `拿不到宿主绝对路径：processPath 返回的不是绝对路径（${indexHost}）`);
            // ── 守卫② 只读会话（D13）：不写、直接 unavailable；**不删任何东西** ──
            if (!writable)
                return failSet("unavailable", "会话策略只读（read-only）⇒ 派生索引不落盘");
            // ── 模块（D14）──
            let mod;
            try {
                mod = await load();
            ...
            const dirty = [...new Set((callOpts?.dirtyRels ? [...callOpts.dirtyRels] : []).map(String))];
            // ── 门① 先取目录版本（纪律③）──
            let rootEntries;
            try {
            ...
            const rebuildAndQuery = async () => {
                let tmpHost;
                try {
                    tmpHost = await processPathOf(fs, ws, `${INDEX_FILE}.tmp-${process.pid}-${Math.random().toString(36).slice(2, 8)}`);
                }
                catch (e) {
                    return failSet("unavailable", `拿不到宿主绝对路径（tmp）：${msgOf(e)}`);
                }
                let wdb;
                try {
                    if (existsSync(tmpHost))
                        unlinkSync(tmpHost); // 上次崩在中途留下的残件
                    ...
                    for (const dir of dateDirs) {
                        for (const f of await enumDateDir(fs, ws, dir)) {
                            ...
                            let p;
                            try {
                                p = parseMemory(text, f.rel, f.name);
                            }
                            catch {
                                continue;
                            } // 单条解析失败跳过（与读侧同）
                            ...
                            if (n % REBUILD_YIELD_EVERY === 0)
                                await new Promise((r) => setImmediate(r));
                        }
                    }
                    writeMeta(st, "schema_version", INDEX_SCHEMA_VERSION);
                    if (coarseNow !== undefined)
                        writeMeta(st, "coarse_signal", coarseNow);
            ...
            let db;
            if (!existsSync(indexHost))
                return await rebuildAndQuery(); // 「没有索引」= 重建的唯一时机之一
            try {
            ...
            try {
                ...
                if (needFull) {
                    ...
                    if (changed.length || removed.length)
                        mismatch = await syncDirs(db, fs, ws, changed, dirty, false, removed);
                    ...
                    if (!mismatch && dirty.length) {
                        ...
                        if (recheck.length)
                            mismatch = await syncDirs(db, fs, ws, recheck, dirty, false);
                    }
                }
                if (mismatch) {
                    closeQuietly(db);
                    return await rebuildAndQuery();
                }
                // **把先取到的那个版本落库**（纪律③：存下的版本必须不新于内容）
                if (coarseNow !== undefined)
                    writeMeta(stx, "coarse_signal", coarseNow);
