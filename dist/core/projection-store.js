            ...
            try {
                ...
                for (const line of String(txt).split("\n")) {
                    ...
                    try {
                        ...
                        return null; /* 单行坏 → 触发 rebuild（判据见上） */
                    }
                }
                return nodes;
            }
...
export const getProjectionStore = (fs, ws) => createJsonlProjectionStore(fs, ws);
/**
 ...
 */
export const invalidateProjection = async (fs, ws) => {
    if (!fs || !ws)
        return;
    try {
        await getProjectionStore(fs, ws).invalidate();
    }
    catch { /* best-effort：缓存不是真相，重建即可 */ }
};
 ...
 */
export const shadowSourcesFingerprint = async (fs, ws) => {
    if (!fs || !ws)
        return undefined;
    try {
...
const writeFingerprint = async (fs, ws, fp) => {
    try {
        const t = await fs.resolve(`${ws}/${fingerprintRel()}`, { cwd: ws });
        await fs.writeText(t, fp);
    }
    catch { /* best-effort */ }
};
const readFingerprint = async (fs, ws) => {
    try {
        const t = await fs.resolve(`${ws}/${fingerprintRel()}`, { cwd: ws });
        const txt = await fs.readText(t);
