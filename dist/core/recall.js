...
export const renderRecoveryFor = (query, t, refs) => {
    ...
    if (t.objective)
        seg.push(`- **目标**：${D(t.objective, 60)}`);
    if (t.trigger)
        seg.push(`- **触发**：${D(t.trigger, 60)}`);
    seg.push("");
    if (t.decisions.length) {
        seg.push("## 关键决定");
        for (const d of t.decisions.slice(0, 10))
            seg.push(`- ${D(d.text, 44)}〔${d.source || "?"}〕${d.reason ? ` 因：${D(d.reason, 40)}` : " 因：未明确"}`);
        seg.push("");
    }
    ...
    if (t.constraints.length) {
        seg.push("## 当前注意 / 约束");
        for (const c of t.constraints.slice(0, 6))
            seg.push(`- ${D(c, 60)}`);
        seg.push("");
    }
...
export const renderRecovery = (query, tasks, refs) => renderRecoveryFor(query, bestTask(tasks, String(query || "")), refs);
