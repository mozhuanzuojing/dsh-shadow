...
export const buildManifest = (version, nodes, failures = []) => ({
    version,
...
export const writeManifest = async (fs, ws, m) => {
    if (!fs || !ws)
        return;
    try {
        const t = await fs.resolve(`${ws}/${manifestRel()}`, { cwd: ws });
        await fs.writeText(t, JSON.stringify(m));
    }
    catch { /* best-effort */ }
};
export const readManifest = async (fs, ws) => {
    try {
        const t = await fs.resolve(`${ws}/${manifestRel()}`, { cwd: ws });
...
export const renderManifest = (m) => {
    if (!m)
        return "（shadow manifest：尚无（尚未构建投影索引））";
    ...
    if (failures) {
        lines.push("");
        lines.push("## 失败项");
        for (const f of m.failures.slice(0, 20))
