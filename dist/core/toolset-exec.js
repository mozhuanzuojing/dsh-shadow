...
import { capabilityOf } from "./toolset.js";
 ...
 * 其余走 PATH 上的可执行文件。
 */
export const probeCapability = async (id, timeoutMs = PROBE_TIMEOUT_MS) => {
    const c = capabilityOf(id);
    if (!c)
        return { available: false, detail: `未登记的能力：${id}` };
    if (id === "zg") {
...
export const resolveInstall = (id) => {
    const c = capabilityOf(id);
    if (!c)
        return { error: `未登记的能力：${id}（台账见 core/toolset.ts）` };
    const r = c.install;
    if (r.kind === "argv") {
...
export const installCapability = async (id, opts = {}) => {
    const c = capabilityOf(id);
    if (!c)
        return { id, status: "unknown-capability", detail: `未登记的能力：${id}。可用：见 mode:"toolset" 的台账。` };
...
/** 台账巡检（**只读**）：逐项探测 + 给处置。 */
export const surveyCapabilities = async () => {
    const out = [];
...
/** 渲染巡检结果（读侧输出）。 */
export const renderSurvey = (rows) => {
