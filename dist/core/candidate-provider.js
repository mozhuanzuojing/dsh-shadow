...
const okSet = (provider, sources, atoms) => ({ provider, state: "ok", sources, atoms });
/**
 ...
 */
export const fsCandidateProvider = {
    id: "fs",
    async provide(fs, ws, _cfg, keep) {
        const sources = (await listMemories(fs, ws)).filter((mm) => keep(mm.rel));
        const atoms = [];
        for (const mm of sources) {
            ...
            if (!text)
                continue;
            try {
                atoms.push(parseMemory(text, mm.rel, mm.name));
            }
            catch { /* 单条解析失败跳过（与今天同） */ }
        }
 ...
 */
export const createCandidateProvider = (cfg) => {
