...
export const targetNotIdentity = (t) => !TARGET_IDENTITY.test(String(t || ""));
export const assertTargetNotIdentity = (t) => ({ ok: targetNotIdentity(t), reason: targetNotIdentity(t) ? undefined : "Adaptation ≠ Identity Change（target 禁 identity/self：调整行为，不改变 Who I am）" });
const BETTER_SELF = /better self|improved myself|我变得更好|我是更好|我更优秀|i improved|self improvement|better person|变成一个更好的/i;
export const resultNoBetterSelf = (r) => !BETTER_SELF.test(String(r || ""));
export const assertResultNoBetterSelf = (r) => ({ ok: resultNoBetterSelf(r), reason: resultNoBetterSelf(r) ? undefined : "Successful Adaptation ≠ Better Self（Outcome matched expectation，非『I improved myself』）" });
