...
//
// §8 的纪律：**接线不重建** —— 血缘走既有 `AtomLineage` + `AtomEvidenceRef`（`core/lineage/index.ts`），
...
// （同 §5：候选与证据都来自**外部**）。
import type { AtomEvidenceRef, AtomLineage } from "../core/lineage/index.js";
 ...
 *
 * ⚠ `createdBy` 只有 `user | agent | tool` 三个值（`core/lineage/index.ts:11`），**没有「引擎」这一档**。
