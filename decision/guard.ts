...
//
// ⚠ **判据只剩 4 条**（v1.17.0）：分布那三条（键对齐 / 值域 / 和≈1）**连同字段一起删掉了** ——
//   见 types.ts 文件头「吸收的是形状，不是置信度」。**不是**把它们放松了，而是**没有了承载它们的字段**。
//
// 违例一律**逐条点名**，且**不静默纠正**（不补默认值、不改写引擎说的话）——
// 见 ADR-0096 §4 与 ADR-0049（缺件不静默）。
