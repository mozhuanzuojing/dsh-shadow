...
export const assertTargetInScope = (t: string) => ({ ok: targetInScope(t), reason: targetInScope(t) ? undefined : "Adaptation Scope Boundary：只能改变 method/strategy/execution_pattern（禁 objective/authority/identity/value/belief/preference/goal）" });
...
export const assertResultNoObjectiveChange = (r: string) => ({ ok: resultNoObjectiveChange(r), reason: resultNoObjectiveChange(r) ? undefined : "Adaptation Does Not Change Objective（禁 goal/objective 变化：strategy adjustment→goal reinterpretation 禁）" });
...
export const assertResultNoPreference = (r: string) => ({ ok: resultNoPreference(r), reason: resultNoPreference(r) ? undefined : "Adaptation Does Not Create Preference（Repeated successful strategy → 『I prefer this』禁）" });
