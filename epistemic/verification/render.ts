// dsh-shadow —— verification/render.ts：验证渲染（run + drift report）。
import type { VerificationRun, DriftReport } from "./types.js";

export const renderRun = (run: VerificationRun) => {
  ...
  for (const c of run.checks) lines.push(`  ${c.boundary} #${c.invariantId} ${c.status.toUpperCase()} · evidence ${c.evidenceRefs.join("、") || "—"}`);
  lines.push("（Verification 只答有无违反边界；无 score/quality/health；不产生 RealityClaim）");
  return lines.join("\n");
};

export const renderReport = (report: DriftReport) => {
  const lines = ["[Drift Report]"];
  for (const b of report.observedBoundaries) lines.push(`  ${b.boundary} drift ${b.drift ? "⚠" : "none"} · evidence ${b.evidence.join("、") || "—"}`);
