// dsh-shadow —— reality/claim/engine.ts：RealityClaim Engine。
// RealityObservation + ValidationHistory + TemporalContext → RealityClaim（只产 candidate/supported）。
...
export const claimOf = (opts: { observations: RealityObservation[]; validations?: { id: string; outcome: string }[]; id?: string }): RealityClaim | null => {
  ...
  const status: RealityClaimStatus = unstable ? "unstable" : supported ? "supported" : "candidate";
...
export const renderClaim = (c: RealityClaim) => {
