// dsh-shadow —— federation/contract.ts：Projection Contract（不是 Access 权限层）。
import type { FederatedObservationPacket, ExchangeableKind } from "./types.js";

export const packetOf = (opts: { sourceObserverId: string; observationClaim: string; lens?: string; visible?: string[]; hidden?: string[]; distortion?: string[]; hypothesisId?: string; validationId?: string; outcome?: any }): FederatedObservationPacket => ({
...
export const renderPacket = (p: FederatedObservationPacket) => {
  ...
  if (p.validationReference.hypothesisId || p.validationReference.outcome) lines.push(`validationRef ${p.validationReference.hypothesisId || ""} · ${p.validationReference.outcome || ""}`);
...
export const isExchangeable = (kind: ExchangeableKind) => ["ObservationClaim", "ValidationResult", "AlternativePerspective"].includes(kind);

export const assertPacketBarrier = (p: FederatedObservationPacket): { ok: boolean; reasons: string[] } => {
