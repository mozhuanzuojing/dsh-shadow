...
| ✅ 确定性取代 + 双时间账本 | Temporal Validity（[2606.26511](https://arxiv.org/abs/2606.26511)）：**余弦相似度分辨「被推翻的旧事实」与「换个说法」的 AUROC 仅 0.59（近随机）**；RAG 因此结构性有 **15–** 概率返回已被取代的值 |
| ❌ **LLM 判过期** | 同上 AUROC  —— 让 LLM 判「哪条失效」无依据 |
| ❌ **LLM 自动纠正** | Huang（ICLR 2024，[2310.01798](https://arxiv.org/abs/2310.01798)）无外部反馈时**性能反降**；Kamoi（TACL 2024，[2406.01297](https://arxiv.org/abs/2406.01297)）**无任何工作证明提示式自纠能成功**；误纠正率主导 **53–**（[2605.27559](https://arxiv.org/abs/2605.27559)） |
| ❌ **裸用 LLM 查文档漂移** | DocPrism（**ISSTA 2026**，[2511.00215](https://arxiv.org/abs/2511.00215)）：**flag rate **（> 的函数被标成不一致），加约束后降到  —— **无约束时基本是噪声** |
...
### 1. 先量事实：表面  的「证据失效」里，绝大多数是假的

对 `.shadow` 全库（记忆）跑 `evidencePathsOf → isPathLike → fsExists`（**即召回路径上真正用的那条链**）：
...
| 排除 | 依据 |
|---|---|
| 让 LLM 判「哪条知识过期」 | AUROC  近随机（2606.26511） |
| LLM 自动「解决」冲突 / 改写事实 | 误纠正率 53–；无外部反馈时自纠不可靠（三重否定） |
| 裸用 LLM 检测文档-代码漂移 | flag rate （DocPrism, ISSTA 2026） |
...
| 备选 | 否决理由 |
|---|---|
| 用 LLM 判「这条记忆的引用是否失效」 | **AUROC  近随机**；DocPrism 无约束时 flag rate  |
| LLM 自动修正过期引用 | 误纠正率主导 53–，且违反 ADR-0043 的 Mutation 边界 |
...
### 正
- **消掉一个静默的正确性缺陷**：绝对路径证据不再被误判失效 —— 量级 **1025 → 243**（约  的假「证据失效」消失）。
- 「引用漂移检测」有了**可引用的判据**（双条件），且与 FSE 2026 的方法同构。
- 明确了**不做什么**（自动纠正），并在 ADR 里留下依据，避免以后有人「顺手加个 LLM 修一下」。
- 修 bug 过程中**独立发现了两处 mock 不忠实**（见下）。

### 负 / 已知边界
- **残余  未解析**，主体是**跨项目相对路径**（如 `scripts\wslc-utils.ps1` 来自 wslc1 仓库）——它们相对本工作区根确实不存在。**不能靠猜基线解决**（见 Alternatives）；需要的是**跨项目根注册**这种带来源的能力，本轮不做。
...
> 通用教训：**mock 与宿主契约不符时，测的是 mock 不是系统**。本仓此前也踩过同类（CHANGELOG v1.15.2「建在编造形状上的测试」）。
...
- [x] 真实 fs 契约**读过源码**（`dsh-fs-local/lib/index.js:`），非假设。
- [x] 全套 测试文件通过；新增 `test/evidence-absolute-path.test.ts`（10 组断言，含「修复前为红」的回归）。
