...
3. **ObserverDifference 是核心产物**：`epistemic/federation/difference.ts` 输出 `projectionDelta{visibleDifference,hiddenDifference,lensDifference} + possibleBlindSpot + unresolvedQuestion`（科学过程=发现"原来不知道什么"），**非 winner**。
4. **Perspective Stability 不叫升级**：`epistemic/federation/stability.ts` `isolated → corroborated(≥2 Observer 引用) → validated(shared + future validation)`——**shared != correct**（两 Observer 可同时错）。
