...
### 正
- **投影与源头重新挂钩**：`_index.md` 不再随会话增长而漂移（实测  /  → 结构性消除）。
- **幽灵条目被清**：磁盘删除的文件不再留在投影里。
- **两个目标同时成立**：每次读索引多一次 `listDir` 级扫描（真语料 **62 ms / 目**），
...
- **未压测大库**： / 62 ms 是当前规模；10 万级时的对账成本未测。
...
- [x] 与 ADR- 一致：同属「判据/可见性/触发条件错」族，按同一纪律处置（先量证、再修、加锁）。
