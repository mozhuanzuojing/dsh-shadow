...
## 补记（`v1.21.33`）：**T1 的代码整层被删除** —— 正文不改，这里记结局

- **用户裁定（2026-09-25）**：在 `T26` 的三个候选切片（读侧渲染建议 / 写侧记录候选 / 不接）之外，
  选了**更强的那个 —— 直接删掉**。理由：本仓规矩「没使用者的机制要么用要么删」。
- **实测前提（不是推测）**：`grep` 全部产品面（`index.ts` / `core/` / `query/` / `persistence/` / `selfhood/` / …）
  **零处** import `decision/`；该层只被自己的测试与工具扫到 ⇒ **零生产消费者**。
  连 `choose()` 都只有测试调用（10 处），与 §12 记的「唯一引擎只报 `null`」互相印证。
- **删了什么**：`decision/` 全部 6 个文件 + `test/decision-primitive.test.ts`；
  并同步清掉 `tools/audit-layers.lib.ts` 里对该层的引用（`PURE_MODULES` 的 `decision/types.ts` 与
  `DIRECTION_RULES` 的 3 条 `decision` 禁令 —— 留着就是**永不命中的死规则**）。
- **本 ADR 的哪些结论不受影响**：§7（T1 不进受保护契约面）依然成立且更干脆 —— 面都删了；
  §11/§12 关于「不建型、不排序、不打分」的**取向**仍是对本仓的约束（那是对**未来**任何决策相关设计的约束），
  但它们的**可跑锚点没了**：本文引的实现与测试已不在 HEAD 上，要复核只能回到 `v1.21.32` 及以前的提交。
- **代价（如实记）**：`T19` 的证据指针（`test/decision-primitive.test.ts` ⑦）随之失效（已在 `BACKLOG` 就地补注）；
  将来若真要「引擎产出的选择」这条原语，**得照本 ADR 的协议重写**（协议部分仍在，实现没了）。
- **为什么不写「维持现状 + 注明」**：那条路径正是 `ADR-0087` 要评估的形状 ——
  一个没有消费者的机制，留着的唯一理由是「以后可能用」；本轮把它判为**不成立**。
