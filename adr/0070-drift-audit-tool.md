...
- 状态：**已接受（2026-09-11）**
- 决定日期：2026-09-11
- 关联 ADR：**ADR-0062**（接线审计工具 —— 本 ADR 是它的**姊妹工具**，同一「先标定再用」纪律）、**ADR-0059**（证据缺失的**双条件**判据 —— 本 ADR 把漏掉的那个消费者补齐）、ADR-//0069（本工具要抓的那一族缺陷）
- 关联待办：`../BACKLOG.md`
- 版本：`1.15.27`
...
| 轮次 | 缺陷 | 检测手段 |
|---|---|---|
| ADR-0063/D5 | 同一条规则**三份实现**，生效那份判据精度仅  | 手工 + 探针 |
| ADR-0067 | 命中数累积用了「展开了片段」的集合（ 永不命中） | 手工（`_meta.json` 不存在这一矛盾逼出来） |
...
**检测 B「同一条判据在 ≥模块被表达」（线索级）** —— 把 `字段 === "字面量"` 按
`字段=字面量` 归集，跨文件出现即报。
...
| NEG-件用**探针赋值的局部名**（ADR-0069 修复后的写法） | 不报 |
| NEG-件**直接含源探针** | 不报 |
| NEG-4 不含进程内集合 | 不报 |
...
就是 ADR-0069 的两处真缺陷，而同文件**当前版本**已修。检测器必须「**旧版报 、新版报 **」。
把历史编码进测试（而不是靠人记），是为了让结论**可复现**。

### 3. 工具的**第一次使用**就抓到一个新缺陷（第 ）
...
**实测影响**（真语料 有路径引用的记忆）：**（）**受影响 / 非具体 locator ****。
...
| 只做检测 A（已标定），不做 B | B 正是**本轮抓到新缺陷的那一个**（`not_found` 跨文件）。没有 B 就没有第  |
...
- **工具首次使用即产出**：抓到 `judgment.ts` 的双条件缺口（第 ），修复后
  记忆（）不再被误判「证据过期」、置信不再假降、rationale 不再谎报缺失。
- **跨消费者一致性被锁住**：`test/evidence-missing-criterion.test.ts` 的 ⑤ 直接断言
  「同一批引用，两个消费者的判定必须一致」—— 这正是病根形态的**不变量化**。
- **回归护栏**：真仓库上检测 A 必须 ****（ADR-0069 刚修完）。若将来有人写回「只看进程」的守卫，
  标定测试立刻红。
...
- **检测 B**：**只答「同一 `字段=字面量` 出现在多个模块」，答不了「两处口径是否一致」** ⇒
  **一律人工复核**。真仓库上当前 键 / ，其中多数是正当的分层表达（生产者/消费者）。
- **真机 `not_found` 语义未端到端验证** —— 复现测试用按契约写的假 Gateway。
- **影响面  /  是离线探针读数**（`_research/judgment-locator-drift.ts`），不放进单元测试。
...
- [x] 与 ADR-//0069 一致：本工具正是为抓它们那一族而造；A 的标定集用了 0069 的真缺陷。
...
- [x] `npx tsc --noEmit` clean；`npm run typecheck:tools` clean；全套回归 ****。
...
- [x] ~~**未做**：检测 B 的 键逐个人工复核（本轮只复核了 `not_found` 那一个）~~ → **已结（v1.15.32 第 4–复核，见下节）**。
...
| `res.status=not_found` | 第 真缺陷 | **v1.15.27 已修**（`observer/judgment.ts` 补 `isConcreteLocator`） |
| `c.kind=provider` / `c.kind=reference` | **正当分层** | `toolset.ts` **声明** `kind` ↔ `core/toolset/exec.ts` **消费**；并实测其边界不变量 **全满足**（v1.15.29 已复核） |
...
**结案**：**真漂移（已修）+ 待复核**。其余各键均落「正当分层」或「同形不同义」。
B 段小计 **11 键/ → 10 键/**。
...
v1.15.29 只做过一次**实测**（全满足），**没有断言** ⇒ 加条目时可能把边界写糊。
