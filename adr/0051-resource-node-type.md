...
- 决定日期：2026-09-10
- 关联 ADR：ADR-0001（不引向量库）、ADR-0042（Shadow Knowledge Graph，提议未实现）、ADR-0043（Shadow Contract）、ADR-/0046（Evidence Lineage / Validation Gate）、ADR-0049（缺件不静默）
...
但现有  `NodeType`（`memory|code|document|decision|concept`）**全部从记忆原子（Memory Atom）派生**——外部资源根本不存在对应的记忆原子。直接加一个枚举值会得到「没有任何生产者」的死类型。
...
分析时还确认了两条既有纪律可以直接复用，不需要新机制：**证据门**（ADR- 对 decision 的那道门）与 **缺件不静默**（ADR-0049）。

## Decision

1. **两层分离**：Resource Card = **source**（写在 `SHADOW_ROOT/resources/`，与其余 shadow 写入同受既有安全边界约束）；`resource` 节点 = **Projection**（派生、可重建、不覆盖卡片）。

2. **NodeType 增 `resource`**（第 ）；`shadow_query` 的 `scope` 收 `resource`；工具描述同步。
...
- **content 顺序固定为「投影段在前、固有层在后」**：读侧 `queryShadow` 只取前 ，倒过来会让「结论 / 引用证据」永远不可见。**取值上限由读侧决定，投影层必须保证高价值行在前**。
