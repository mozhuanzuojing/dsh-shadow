...
ADR-0050 硬切了 旧名，并在 §2 留下一份「**保留**」清单：`args.identity`、`mode:"verify"`、`recall-*`、`model-*`、Gateway `EvidenceRef`、`realityEvidenceRef` 字段名。当时的口径是靠**注释桥**消歧（「`recall` 一词仍多义……靠 mode 正名 `recovery` + 注释桥消歧，不改工具名」）。

保留清单里仍有 属于**同一个词承担两层含义**，注释桥只能让人不误用，不能让人**不误读**：
