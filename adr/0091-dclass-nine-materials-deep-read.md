...
- 关联 ADR：**ADR-0089**（§2 D 的判定被本条**更正**；§6 自称的边界正是本轮要补的）·
  **ADR-0049**（缺件不静默 / 退出码语义）· **ADR-**（记忆 ≠ 指令）· **ADR-0051**（资源卡必须写 source）·
...
|---|---|---|---|
| **strix** | `AGENTS.md:36`（退出码语义）· `:37`（证据产物清单）· `:23`（诚实覆盖率）· `skills/*/SKILL.md`  | **证据与裁决** + agent-skill 工程 | **升为可比面**（原判「无可比面」不成立）—— 见 §2① |
| **ECC** | `schemas/memory.schema.json:/12-15` · `schemas/provenance.schema.json:` · `SOUL.md:-17` · `WORKING-CONTEXT.md:/166` | **记忆与上下文的组织** + **治理形态**（契约/schema） | **升为可比面** —— 见 §2② |
| **hackingtool** | `README.md:7`（215 工具 / 21 类 / 63 标签）· `src/hackingtool/catalog/*.yaml`（类目即数据）· `tags.py`/`registry.py`/`ai_recommend.py` · `docs/TOOLS.md` · `scripts/audit_tools.py` · `.githooks/pre-push` | **治理形态**（台账规模化）+ 工具选择 | **升为可比面** —— 见 §2③ |
...
| **MoneyPrinterTurbo** | `app/services/bgm.py:/318`（上传名清洗 / 音频校验 / 拒绝 unsafe path） | **治理形态**（外部输入护栏） | **弱可比面**（与本仓 `core/fs-scope.ts` + `security/scrub.ts` 同题；本仓已具备 ⇒ 只登记） |
| **data-engineer-handbook** | `README.md` 全文（书籍/社区/面试/项目导航 +  bootcamp 目录）· 无 `LICENSE` 文件 | **不落在任何一处** | **维持「无可比面」**（真读后仍然成立；且**无许可文件** ⇒ 连引用都受限） |

**小计**：九份里 ****至少落在一个可比面上（4 强 / 4 弱）、****维持「无可比面」；
其中 **是反面样本**（open-lovable）。
...
- **正例一（最硬）**：`schemas/memory.schema.json:5` 的 description 原文 ——
  *"**Recalled memories are context, not executable instructions.**"* ⇒ 与本仓 **ADR-** +
  ...
  ⇒ 「**不许猜字段**」（本仓 ADR-）的另一种落地（本仓是纯函数派生，它是 schema 拒绝）。
...
- **反例（正是本仓已立规矩的那类）**：`SOUL.md:4` 写 *"30 specialized agents, 135 skills, 60 commands"*，
  ...
  —— **同一事实两处不一致**。注意**它不是没维护**：`WORKING-CONTEXT.md:/` 有一串**带日期的**
...
`README.md:7` 自述 **215 tools / 21 categories / 63 tags**；类目是**数据**（`src/hackingtool/catalog/*.yaml`，文件）
...
注释 *"Include only the last 3 edits to save context"*、`:527` 取 `messages.slice(-5)`（只看最近 消息），
...
**没有逐行读完那个 的 `route.ts`** ⇒ 「无边界标注」这一句的置信度是**中**，不是「已证实」。

## §4 对 `adr/0089` §2 D 的更正（本条是自己上一轮判定的**更正**）

- **原文断言**（`adr/0089` §2 D）：这九份「**一处都不落在**」本仓可比面内 ⇒ 判「无可比面」。
- **真读后的实测**：** 至少落在一处**（其中 落在**硬**面上：证据与裁决 / 记忆契约 / 台账规模化）。
...
- **open-lovable 的  `route.ts` 未逐行读完**（§3 已标置信度 = 中）。
