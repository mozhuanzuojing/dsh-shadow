...
| 测试 | `test/*.test.ts`（） | **TS**（Node 原生 type-stripping 直跑） |
| 工具 | `tools/*.mjs`（） | ❌ **JS** |
| 回放脚本 | `test/replay-*.mjs`（） | ❌ **JS** |

⇒ 仓库是「**核心 TS、工具 JS**」的混合体。而按 ADR-/0059 的教训，
**工具恰恰是发现问题的那一侧**（接线审计、引用漂移测量、检索评测），它却是**唯一没有类型门**的部分。

## Decision

### 1.  `.mjs` 全部改为 `.ts`（`git mv`，保留历史）
...
`package.json` 的  script 同步改指 `.ts`（script **名**不变，调用方无感）。
...
**字段名写错编译器不会响**。迁移后报  `TS2339/TS18046`，已补**显式接口**：
...
曾试 `tsconfig.tests.json`（`include: ["test/**/*.ts"]`），实测结果：**文件 类型错误**。
逐条看过后**决定不加**，理由：

- 报错集中在**故意喂畸形输入**的守卫测试上（`test/concept-guards-2.test.ts` 等）：
  它们的**目的**就是把 `{ status: "supported" }`（缺 必填字段）、
...
故 也一并改扩展名（`measure-metadata-leak` / `measure-metadata-quality` / `measure-path-visibility` /
...
| 给测试套也加类型门 | 见 Decision §3：错误里绝大多数是**故意的**；加门只能用 `as any` 消掉，反而削弱测试 |
...
- **审计工具现在会扫到它自己**。`tools/*.ts` 由 `.mjs` 变成 `.ts` 后进入
  ...
  **新增的 全部来自工具自身的字符状态机**（`c === "\\"`、`c2 === "*"` 这类**单字符局部别名比较**），
...
- [x] 与 ADR- 一致：核验器与评测器的**行为未改**（实测复跑：`--id jqlang.jq` → `status: ok`；
      `eval:retrieval` 复现同量级读数）。
- [x] **零新依赖**：仍只有 `typescript` + `@types/node`。
- [x] `npx tsc --noEmit`（生产）clean；`npm run typecheck:tools` clean；全套回归 ****。
