...
- 状态：**已接受（2026-09-11）**
- 决定日期：2026-09-11
- 关联 ADR：ADR-0049（缺件不静默）、ADR-0055（工具集台账两级 + 不代装）、ADR-0056（委派规模与复用）、ADR-0030（inv 182 Delegation Scope 不可扩大）、ADR-0029.1（inv ）
...
- 预检会**真的起进程探测**（每条需求最多探命中的条目数）。已有短路（任一可用即算就位）与超时（provider 15s / reference 3s）。`need` 限 。
...
- [x] 与 inv /184 一致：不给主体打分、不扩大委派范围、安装审批凭据仍是发起者。
- [x] F1 修复经**回档复验**（改动前 `findReadQuery({mode:"toolset"})` = MISS，改动后 HIT）。
- [x] **F1 在真实系统上原样复现**（v1.15.17 补验，本项原为「未验证」）：在运行中的 DSH 进程里调 `read_shadow({mode:"toolset", need:[...]})`，**返回的是 `_index.md` 目录**，不是台账——即 F1 的后果按原样发生。同批取证：`Tool.listTools` 给出**活的** `read_shadow` schema，含 v1.15.9 的 `install`/`survey`/`category`，**但不含本版的 `need`**，mode 描述里也**没有 `toolset`** ⇒ **运行中的插件是 v1.15.12**（即 F1 仍在其中的那一版）。这比代码阅读强：**缺陷在真实系统上复现，不是推断**。
- [x] 全套测试 文件全过（含新增 ）。
- [ ] **未验证**：**本版修复在真机生效后**的端到端返回 —— 需重启 DSH（插件代码**不随 config 热加载**，见下）。
- [ ] **未验证**：本机无 zg / semble，故预检的「多命中短路」在**已装**状态下未经实测（只在未检出状态验过）。

## 运维事实（v1.15.17 取证）

同一进程里观察到**两种加载行为并存**，这对部署很重要：

| 平面 | 是否热加载 | 证据 |
|---|---|---|
| **host 组合**（`cordis.patch.yml` 的行、新装的包） | ✅ **热加载** | 加 `agent-team` 行 + 装包后，`agentTeams` **立即出现在服务目录**、`team_task_list` **立即可用**（无需重启） |
| **插件自身的 `dist/` 改动**（本包） | ❌ **不热加载** | 改完 `dist` 后，live schema 仍是旧版（无 `need`、mode 描述无 `toolset`），行为仍是旧版 |

⇒ **改 dsh-shadow 的插件代码必须重启 DSH 才生效**；改 host 组合行不一定。
