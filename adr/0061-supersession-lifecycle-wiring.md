...
## 补记（v1.15.38，ADR-0077 D3）：把「错误方向不对称」写进本 ADR 的**判据面**

本 ADR 决定「取代由**读时裁决**给出、不持久化」，并在「负 / 已知边界」里承认取代是
...
可观察的噪音**（两条都在、都能被读到），代价远小于**错误关链**。
⇒ 故 `arbitrate.ts` 用**严格** `t < newest`（并列则不取代）**不是缺陷**，是这条原则的正确落法；
反之，「宁可并存」也不得被记为缺陷。

**它与本仓 ADR-0049（缺件不静默）是互补的一对**，不是同一件事：
一个管**缺失必须可见**，一个管**破坏必须保守**。

**本补记不改本 ADR 的任何决定**（取代仍不持久化、仍是单键时间序），只补判据依据；
细粒度取代仍在 `BACKLOG.md` **D3** 待用户拍板（对照材料见 ADR-0076 §1）。
