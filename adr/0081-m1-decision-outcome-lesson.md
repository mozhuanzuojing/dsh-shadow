...
### M1 Decision Memory —— **已有 ，缺的是一小块明确的边**
...
| 重复决策 / 重复结果的 tally（≥） | `reflection/patterns/decision-outcome.ts:15-16`（`repeatedDecisions` / `repeatedOutcomes`） | ✅ 已有 |
...
| 取代的**确定性**（不靠相似度/LLM） | ADR-；`adr/0080` 证明「阈值不可达」 | ✅ 已冻结 |
...
| ③ | **精度上界已被别家量过** | `adr/0080`（MemStrata）：语义判断在「是否相关」这类任务上 **AUROC **、**任何阈值 precision 上限 0.667**，「安全自动化的 0.95 floor 不可达」。而 M5 要拿归属算 `influenced_decision` / `caused_rework` ⇒ 这些**指标会带一个不可量化的偏差源**，且无法说清分母。 |
...
+ `test/decision-outcome.test.ts`（**11 组闸**）；`tsconfig.json` 显式纳入编译面。`npm run verify` = ****。
...
**输入**：决策（含 **故意构造**的边界）+ 观察。**输出**：归属 7 · 歧义 1 · 未归属 2 ·
原语记录 Proposal 7 / Confirmation 7 / **违规 0** · 事实 （全部 `fact-outcome-*`）。

### 9.1 验证到的（被真跑，不是被断言）

| 路径 | 真实输入 | 结果 |
|---|---|---|
| 正常归属 | `DEC-1 ← OBS-1`（verify  通过） | ✅ 归属，`rule same-key-window/v1` |
...
| **F4** | `DEC-4` 天然有 观察 | 事实层**一决策多事实是常态** ⇒ 读数必须按决策聚合（已如此） |
| **F5** | 真实数据里 0 歧义、0 超窗（本次 1+2 **全是构造的**） | `ambiguous` 通道**保留但不应常响**；常响 ⇒ key 设计或 `windowDays` 取值有问题 |
| **F6** ⭐ |  pending 里 `DEC-2`/`DEC-7` 都是「**刻意不做**」，schema **区分不出「刻意不做」与「忘了做」** | M1 需补 **`disposition`（`open` / `deliberate-deferral`）**，否则 age 读数把两者一起报成「积压」——**不是错，是缺维度** |
| **F7** | 归属**全是 `lag 0d`**，实际间隔 5 分钟 ~ 5 小时 | 整日粒度对「当天决策-当天结算」**丢失全部分辨率** ⇒ 若要支撑「结算得多快」，lag 需**小时级** |
| **F8** ⭐ | 「接受率 **1**」是**自产自销**：确认全由规则代码生成（`actor:"tool"`），**来自人的真实确认** ⇒  **零信息量** | 候选统计**必须按 `actor` 分层**（human / tool / ci），否则 tool 自确认能把接受率刷成满分——**这正是「candidate 统计只能用于待确认候选」最容易被绕开的地方** |

### 9.3 dry run **不能**回答的（不得据此设计）

- **确认是否高频、是否要批量、是否要 diff、批与批之间怎么排** —— 本次 **真实确认动作**（确认是代码生成的）。
