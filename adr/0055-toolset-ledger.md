...
4. **自动安装（缺件即装）** —— 否决：违反 inv （见 §5）。v1.15.9 的审批门是合规的替代。
...
- **已知缺口（v1.15.12 已闭合）**：`docs/toolchain-wsl.md` 是 Debian 包名口径，与 winget **无对应关系**，故初版**未做棘轮**。已在 v1.15.12 补上：棘轮只扫该文档的**「工具映射总表」**段（其余表格如「国内镜像」第 2 列是 URL，误扫会假阳性），按**命令名**匹配台账的 bin/id/label/provides 并带别名表（`fdfind→fd`/`batcat→bat`/`z→zoxide`/`sg→ast-grep`）；为此补登 台账条目（含 Windows 无可靠包的 `tmux`/`viddy`/`tig`/`ip`，其 `install` 留空、只陈述事实）。
...
- [x] 修复了本 ADR 的悬空引用（CONTEXT/README/源码 现已可解析）。
