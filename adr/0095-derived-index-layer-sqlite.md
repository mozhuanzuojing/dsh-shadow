...
`tFingerprint` = **2,452 / 7,286 / 7,088 ms**；`incremental.msFingerprintDetect` = **6,908 ms**。
⇒ **`shadowSourcesFingerprint` 今天要对 9,144 个文件做 per-file `listDir` 级枚举 ⇒ 它自己就 ~7 s。**
...
> - T17-A 组（`t17a/run-bench.txt`，13:46）：`tFingerprint` = **2,452 / 7,286 / 7,088 ms**；`msFingerprintDetect` = **6,908 ms**；
> - 父代理独立复核组（`t17a/parent-verify-fingerprint.txt`，13:54，直接打 `dist/core/projection-store.js`）：
>   **真实工作区 1,791（首跑）/ 2,258 / 3,382 / 3,199 ms**；**冻结副本 3,465 / 4,010 / 3,752 / 3,728 ms**。
...
**(2) 反向实验首轮 5/6 造红 —— 探针「尚不完全合格」，且原因要查清**
...
**(3) 首轮正面结果（供对照，别只记风险）**：canonical diff 在 **8,204 vs 8,203** 上只差 **1 条**，
...
在 `052d26ee` 上 `docs/concepts/memory-builtin.md` 整体**下移 3 行**（插入点在 `:24` 之后、`:117` 之前）：
...
它只判「行号越界」，而**下移 3 行不会越界**。⇒ 这类仍然只能人工回头核（同 `AGENTS.md`「材料」节硬边界 2）。
