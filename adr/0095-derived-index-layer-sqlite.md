...
| 场景 | Windows（本机 2026-09-25） | **Linux**（WSL2 Debian 13 · 同版本 node 26.8.2） |
|---|---|---|
| ① **cold rebuild**：8800 `.md` → SQLite | 512.6 ms（8800 行 · 索引 **6884 KB** · 801 B/条） | **251.8 ms**（同一体积 6884 KB） |
| ② **startup**：索引已存在 → ready | **68.9 ms** | **86.2 ms** |
| ③ **incremental**：+1 新、原地改 1 | 302.5 ms（8801 行 · 改动能被看见 = true） | **136.4 ms**（同） |

⇒ 「默认开是否可接受」在**量级**上有答案了：冷建**不到一秒**、启动**几十毫秒** —— **不是**「启动 DSH 要等十秒」那种形状。
Linux 的冷建/增量更快（ext4），启动略慢（首次 open + 校验）；两侧**都远在可接受量级内**。

⚠ **边界（不缩小）**：合成语料**内容分布均匀**（每条入口/主题长度几乎一致），真语料更偏 ⇒
耗时只能当**下界量级**；且是**单次运行、无重复取样**、各自单机。
⇒ 它回答的是**量级**问题（秒级 vs 分钟级），**不是**一个可比的性能基线。

### 三、二进程并发读数（§补记七 明列的「未测」之一）—— **同机 + 跨平台都测了**

- 做法：`tools/derived-index-bench.ts` 的 ④ 步 —— 两个**子进程**同时对同一份 `index.sqlite` 调 `provide`
  （两个都 `writable: true`；其中一个是 writer，每 3 轮新增一条）。
  ⚠ 关键更正（实测踩到）：**`writable: false` 不是「并发读」** —— 守卫②（只读会话）会直接返回 `unavailable`
  （那由 `test/derived-index.test.ts` ②b 覆盖），拿它当并发探针**等于什么都没测**。
- 读数（**四次运行，结论不同，都照记**）：

  | 运行 | Windows reader | Windows writer | Linux reader | Linux writer |
  |---|---|---|---|---|
  | 合成 **300** 条 | 12/12 `ok` | **出现 `query-error`** | **出现 `query-error`** | **出现 `query-error`** |
  | 合成 **8800** 条 | 12/12 `ok` | 12/12 `ok` | 12/12 `ok` | 12/12 `ok` |

  ⇒ ① **撞锁会发生，且是时序相关的**（同一份代码两次运行可能一次撞、一次不撞）；小语料更容易撞，大语料稳态不撞。
  ② **跨平台确实不同**：300 条那次 Linux **两个进程都撞**、Windows 只有 writer 撞。
  ③ 撞锁一律被表达成 `query-error`（本次回退 fs + **可见**），**不是崩溃、也不是静默空集**
  （第一原则 `error ≠ empty` 的形状成立）。
  ④ ⇒ **不能靠单次运行断言**（这就是「必须多跑几次才能下结论」的一类）。
- **为什么是 WSL2 发行版而不是容器**（如实记）：按用户指示本应先 `wslc pull` 一个 `node` 容器映像，但
  **容器引擎拉不到镜像** —— `registry-1.docker.io` 直接超时（`Get "https://registry-1.docker.io/v2/": ... Client.Timeout exceeded`），
  而 `wslc` 的设置文件（`settings.yaml`）**只有 session/凭据两类项，没有 registry 代理配置**；
  给 CLI 进程设 `HTTPS_PROXY` 也无效（请求由**引擎**发出，不是 CLI）。⇒ 改用**同一台机的 WSL2 Debian 发行版**
  （真 Linux 内核 6.18，ext4）+ 经宿主代理下载的官方 Linux 版 node 26.8.2 跑同一段探针。
  **可重放**：`../.docs/fix/2026-09-25/` 的 `t17c-env.sh`（环境与代理连通性）· `t17c-setup.sh`（下载 Linux node + 拷 dist/与探针）·
  `t17c-run.sh <N>`（跑探针）。
- **仍未测**：**容器**里的锁与 WAL（上面那条障碍）；长时间高并发下的退避策略（当前**无重试**，撞了只回退本次）；
  撞锁的**频率**（没有做成断言，只打印 states）。

### 四、决策：**默认仍是 `fs`**（本轮仍不改）—— 但理由从「没测」换成了「测出了一个具体缺口」

- §七 的判据原文是「**全部通过之后**才考虑把 `sqlite` 设为默认」。本轮之后，**9 项矩阵的证据落点已齐**（§一），
  第 8/9 项也有了跨平台读数 ⇒ 不再是「没测」。
- **但仍然不改默认**，因为并发那半边测出了**具体缺口**：**撞锁是会发生且时序相关的，而当前实现没有重试/退避**
  ——撞上就**回退本次**（正确性不丢，但那一轮走 fs 全量扫，且读侧会挂一条降级横幅）。
  在「默认开」的位置上，这会表现为**偶发的一次性降级 + 横幅**；这是产品取舍，**不是**「已验证通过」。
- ⇒ **改默认的前置条件（现在是可执行的，不是「等环境」）**：
  ① 给 `query-error` 做**有界重试 + 退避**（并保持「重试仍失败才回退」的可见性）；
  ② 用真实流量的 `query-log` 样本压一遍（真实并发形态与本探针的合成并发不同）。
- 其余「未核实」项不变：真实流量漏召回 / 外部进程原地改的实际频率 / 非本地后端 `processPath`。

### 五、本轮的诚实标注

- 上表第 4 项「表缺失」分支**没有单独断言**（与 schema mismatch 共用 `corrupt` 处理路径）。
- 三个数字是**单次运行**，且合成语料均匀 ⇒ 不当性能基线用。
- 并发**四次**运行里 `query-error` 的**分布**（哪几轮、多少次）没有做成断言，只打印 states（读者不能据此估频率）。
- **跨平台那条用的是 WSL2 发行版、不是容器**（原因见 §三）；两者的文件系统/内核相同但有差异（overlay vs 发行版根），
  故这条读数的适用范围应写作「Linux（WSL2 内核）」，不要读成「容器里也验过了」。
