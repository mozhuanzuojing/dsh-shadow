...
## 补记（2026-09-25，`v1.21.14`）：本 ADR 的核心实例 **`ChangeSet` 已被删除** —— 结论随之变化

- **本 ADR 的原结论**（标题即写着「确认 `ChangeSet` 未接线但非缺陷」）建立在「**保留它、标注未接线**」之上。
  **`v1.21.14` 改判为删除** ⇒ 该实例不再是「未接线的优化」，而是**已删除**。
- **工具本身不受影响**：`tools/audit-wiring.ts` 与它的标定（8 组断言）都还在，且 selftest **仍然全绿**。
- **⚠ 但标定的一处锚点失效（本补记最要紧的一条）**：selftest ⑧ 用 `ChangeSet` 作**真仓库 A 类已知答案**
  （`countCallSites(prod,"ChangeSet").sites === 0`）。符号删除后该断言**仍成立却不再有判别力** ——
  **空判据**。⑧ 里另外两条**反例**（`invalidateProjection` / `createJsonlProjectionStore` 必须 > 0）
  **仍有判别力**，故本块的净损失是「真仓库正例」这一半。
- **未做**：未给 ⑧ 找替代锚点（备选不足：A2a 其余候选与 A1 的零引用者**不测同一种判别** —— 类型位置提及 ≠ 调用点）。
---

## 补记（2026-09-25，`v1.21.20`）：`T7` 由「保留并注明」**改判为删除**

- **原判（本 ADR 正文）**：`relationForProposal` **保留并注明** —— 原文已写「留接口」（给未落地的 reflection 泳道），
  同时本 ADR §5 已点名**新风险**：它忽略入参（`(_n: any)`）⇒ **恒返回** `"evolved_into"`。
- **改判依据（三条）**：① **零消费方** —— 全仓仅定义自身一处，生产与测试引用皆 0
  （`ADR-0087`：**不允许建没有消费方的机制**）；② **恒返回常量** ⇒ 留着就是**假接口**，
  将来真接线时会**静默给出错答案**（本仓最忌的形态）；③ 「让它真的用入参」需要**尚未实现**的
  reflection 语义 ⇒ 那是**臆造**，不做。
- **执行**：删除 `selfhood/temporal/edge.ts` 里那两行（注释 + 导出）。
- **若将来 reflection 泳道落地**：**重新立条目 / ADR 再实现**，**不要**复活一个恒返回常量的占位。
- **同步更正**：`BACKLOG` 的 A 类分诊表里那行「保留并注明」已一并改为「已改判删除」——
  **写过的话必须回写**，否则台账与事实脱节（本会话已出现过一次同类：`D12` 标题）。
