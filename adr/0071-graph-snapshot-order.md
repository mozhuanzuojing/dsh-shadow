...
- 关联待办：`../BACKLOG.md` 的 **T4**（本 ADR 给其中 符号落了处置）、**T5**
- 版本：`1.15.28`
...
- 而 ADR- 把 `graph.json` 定义为**可重建的派生快照**（ADR-0003）——
...
修复后 ：① temporal 取最新 ② world 取最新 ③ **乱序插入**仍取最新（只看名字序，不依赖插入序）
