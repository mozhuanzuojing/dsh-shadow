...
|---|---|---|
| `agent-team`（域服务）`lib/index.js` | **逐字节相同** | MD5 `BBA2DB3A…` 一致（1834 行） |
| `agent-team-profile` 的 `cordis.patch.yml` | **逐字节相同** | 仍 disable 4 条 `tool-subagent*`；仍插 `agent-team`(`maxMembers: 8` + 4 键) / `tool-agent-team` / `ui-agent-team` |
| `client-ui-agent-team` `lib/index.js` | **逐字节相同** | MD5 `92BF033E…` 一致 |
| `dsh-web-app` 的 `presets/{standard,minimal,ptc,cordis}.patch.yml` | **逐字节相同** | 4 个文件全 SAME ⇒ **本预设的 F1 忠实性基线未动** |
| **`tool-agent-team` `lib/index.js`** | **唯一有语义变化的地方** | 549 → 557 行；MD5 `6C426893…` → `7B817E51…` |
...
**Agent Teams 的平面、工具数（9 个）、schema、名额语义在 alpha.2 上一个字都没变** —— 变的是「teammate 知道怎么回话」。
...
| 3 | alpha.2 的 teammate 身份提示 | 本仓**全库 0 处**提及 | 漏描述（alpha.2 的真变化） |
| 4 | `members.splice/pop/shift/filter/delete` → **0 hits**（`presets/README`） | 实测 **1 hit**：`members.filter(... phase === "provisioning")` 是**只读筛选**；`splice/pop/shift/delete` 仍 0 处 ⇒ 「无移除路径」结论不变，但「0 hits」这句不成立 | 断言口径过宽 |
| 5 | `installed = new Map()`「第 531 行」、`const scoped`「第 232 行」（`presets/README`） | 两个数都是 **0.1.5-rc.2 时代**的读数；alpha.1/alpha.2 上：`const scoped = agent.ctx` **L231**、`name: "team:policy"` **L238**（两版同）、`installed` alpha.1 **L530** → alpha.2 **L538** | 行号腐烂 |
...
⚠ **一条必须说清的时间线**：第 1、5、6 处**不是 alpha.2 造成的** —— `DEFAULT_MAX_MEMBERS = 16` 与 `push` 的新位置
...
实际上是把**整条 0.1.7 线**的读数补齐；alpha.2 自己只贡献第 3 处。
...
- 依据（按强度排序）：
  1. **实测：4 从未生效过**。本机 `~/.dsh/sessions` 现存 **23 个会话 / 51 715 325 字符**，按事件类型统计
     （`type == "tool/call"` 且 `data.name == "spawn_teammate"`）真调用为 **0 次**；`TEAM_MEMBER_LIMIT`
     的**运行时**抛出同样为 **0 次**（文本命中的那几处全是「把 ADR / 包源码读进上下文」的引文，逐条看过）。
  ...
  4. **消掉一条长期维护面**：ADR-0098 §2.2 规定「按 id override **必须重述该行全部 5 个 config 键**，漏写一个 = 静默回落
     schema 默认值」。删掉 override 后这条不再需要人工维系。
- **判据一个字不动**：「复用优先（会复用 ≥2 次才用 teammate）」与 persona 的「默认不派人」**全部保留**（它们是真正
  控制花费的那一层，见 §3.3）。4 → 8 只是把**从未生效的兜底**对齐上游，不构成任何一条要派门槛的放宽。
- 该决定**不改上游代码、不新增权限**（与 inv 178 / 182 无关）。
...
### 4.1 上限值：仍 4 / 8 + 删 override（采纳）/ 16 / 4 + 仪表

| 选项 | 内容 | 判 |
|---|---|---|
| 仍 4，只把描述写清 | 零配置改动 | **不采**：留着「与上游的部署偏离 + 5 键重述」这条**持续**维护面，换来的是一道**实测 0 次触发**的闸门 |
...
读数：`agent-team` / `agent-team-profile` / `client-ui-agent-team` / 4 个 shipped preset patch **全 SAME**；
`tool-agent-team` **DIFF**（549 → 557 行，差异即 §1 的 system-reminder 扩写）。

### 5.2 会话丈量：名额从未被触达

- 语料：`C:\Users\l\.dsh\sessions\**\session.v*.jsonl.zstd`，**23 个文件 / 51 715 325 字符**。
...
- 判据**必须按事件**，不能按字符串：`spawn_teammate` 字符串在最大的那个会话里出现 205 次，全部是
...
- 读数：`spawn_teammate` **0** · `team_task_*` **0** · `wait_agent` **0** · `interrupt_agent` **0** ·
  `send_message` **3**（三个 `origin:"subagent"` 的子会话各 1 条回话）· `subagent` **3** ｜
  **运行时 `TEAM_MEMBER_LIMIT`：0**。
- 交叉来源：`~/.dsh/.shadow` 全树对 teammate 只有 3 条命中，且全是分析文字，无使用记录 ⇒ 两个独立来源一致。
...
| 无移除路径 | 唯一写入是 `if (index < 0) state.members.push(member)`；`splice/pop/shift/delete` 0 处（唯一 `filter` 是只读筛 `provisioning`） |
...
| 面 | 验法 | 期望 |
|---|---|---|
| 组合（部署面删 override 后） | `dsh --profile web --dump-config` | `agent-team` 的 `config` 为 bundle 的 5 键、`maxMembers: 8`；`preset-projection` 仍组合出 27 行、`tool-subagent*` **0** |
...
- **`maxMembers` 的运行时拦截行为仍然没有被真机触发过**（BACKLOG 的 V3 不结案），但本次把它**量化**了：
  在 23 个会话里离 4 都还差 4 个 ⇒ 「没验过」与「不可能验」是两件事，本 ADR 只主张前者。
