...
ADR-0047 吸收了 PageIndex/zg 的第一批思想（免向量树 + 推理检索 + 语料树 + 多级管线）。再次深入源码，挖到**更深、能显著强化 dsh-shadow 索引/知识层**的 点（见下）。本 ADR 收紧**采用项**，其余留作候选。
...
## 自检
- [x] 深入 PageIndex/zg 源码挖出 更深设计，本 ADR 收紧采用 ①②⑤⑥。
- [x] 与 ADR- 自洽：结构确定性（无 LLM）、不向量化、检索不是系统大脑。


---

## 补记（2026-09-25，`v1.21.14`）：**⑤ change-set 增量索引 —— 已删除（改判）**

- **改判**：本 ADR ⑤ 的落地物 `core/retention/change-set.ts`（`ChangeSet`）与 `ShadowProjectionStore.invalidateFor?()`
  **连同测试一并删除**（用户决策）。**⑤ 条款自此不再是「已交付但未接线的优化」**，而是**撤回的候选**。
- **原判与理由（保留在 `adr/0086` §6，v1.15.74）**：**保留** —— 它无生产消费者、无外部契约，但**有验证价值**
  （充当 `tools/audit-wiring.selftest.ts` ⑧ 的**真仓库已知答案**），删它会把一条检测器变弱。
- **本次改判接受的代价（明文记录）**：⑧ 之后**仍然绿，但已不具判别力**（符号不存在 ⇒ `sites === 0` 恒成立）。
  即：**本仓 A 类检测器失去其唯一真仓库校准锚点**。这不是执行事故，是**被明确选择的损失**。
- **仍然成立的部分**：生产走`invalidateProjection` 的**粗粒度清空**（`invalidate()`），路径**正确**（缓存是可重建派生）；
  「只失效变更项」的优化**不再保留**，若将来要做，须**重新立 ADR**（而不是复活这段代码）。