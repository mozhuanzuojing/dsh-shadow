# ADR-0083: 对抗性审查 2026-09-12 —— 三类缺陷、四条新增纪律、以及**两批未修线索台账**

> **本文件的定位（与 `BACKLOG.md` 的分工，用于消除重复）**：
> **本 ADR = 决策与理由**（为什么这么判、判据怎么收、哪些是刻意的取舍）；
> **`BACKLOG.md` §六/§七 = 台账与状态**（每条线索的 `文件:行号`、当前状态、优先级）。
> **同一件事不在两处各写一遍**：本文件只保留「理由」，状态一律回指台账章节号。
>
> **本文件已跨 10 轮（v1.15.54 → v1.15.62）、共 14 节**，不是一次审查的快照 —— 初版标题只覆盖第一轮的三类缺陷，已改。

- **状态**：已生效，**范围 v1.15.54 – v1.15.62**（§1–§14；每节开头标注该轮版本）
...
- **纪律**：报告必须单列「**已排除（追下去发现是对的）**」。各轮报告合计排除了 **70 余条**可疑点并给了理由；
...
## 13. 第十轮（v1.15.62）：测试面类型检查**扩到全部**（83 → 0），并**独立核实**了它是怎么修的
...
批量机械修改测试文件是**高风险动作**：最容易的作弊方式是把断言改宽松、加 `@ts-ignore`、或删测试块。
故写了两支探针（都在仓外 `.docs/fix/2026-09-12/`）**逐项核对**：
...
② 剥完 cast 后 `{ … } )` 与 `{ … })` 的空格差异没归一化 ⇒ 又一批假报。
⇒ 改成**按括号深度消费 cast**（一直吃到深度 0 的 `,` 或 `)`）并归一化标点前空格。**先修探针，再信结论。**
...
## 14. 元审查（v1.15.63）：审**我自己这 10 轮的产出**，不是审代码

用户要求：找出遗漏、纠正因果、处理重复、解决干扰、填补缺失环节。逐条给出**我发现的**与**我改的**。
...
§六/§七 的表格里有「后果」一列，写法上是**断言**，但其中相当一部分来自只读审查的**静态推演**（报告者明确说未跑测试）。
**我此前只在报告结尾整体标注可信度，没有逐行区分** ⇒ 读者容易把「改坏这里会怎样」读成「这里正在造成损害」。已处置：

- §7.0 表下加**统一口径**：「标『已修』的以 `adr/0083` §12 为准，其余为**静态推演**（不是实测结论）」；
- §12.2 / §13.2 已分别标注本轮修的是「读到并核实过的」还是「报告者推演的」；
- **仍未做**：逐行加「证据等级」列（同上，属 14.5 的重构）。

**另一处因果要纠正**（我在 §9.2 写过、可能被误读）：`audit-wiring.selftest` ⑪ 的同义反复我写「**把产品侧改成任何东西它照样全绿**」——
准确说法是「**改产品侧的分桶判据**它照样全绿」（它测的是**分桶**这一条判据，不是整个工具）。已在此更正。
...
我当时的动作是**可恢复的**（因为每次都提交推送），但若那次改动混在一批未提交的编辑里，就会静默丢结构。
⇒ 记入操作纪律：**改大文件的章节顺序前，先 `git status` 确认工作树可回退，且不要用数组喂 `-NoNewline`。**
