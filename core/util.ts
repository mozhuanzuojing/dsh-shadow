...
export const isNotFound = (e: unknown): boolean => {
  ...
  if (/^write failed\b/i.test(msg)) return false;
  if (/^cannot (write|edit|mkdir|copy|move|remove|delete)\b/i.test(msg)) return false;
  ...
  if (/must be a non-empty string/i.test(msg)) return false;
  // ② 宿主的读侧形状：`cannot read|list|stat|resolve "<path>": not found`。
  if (/":\s*not found\b/i.test(msg)) return true;
 ...
 */
export const numOr = (v: unknown, dflt: number, min = 0): number => {
