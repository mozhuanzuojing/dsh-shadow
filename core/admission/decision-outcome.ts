// dsh-shadow —— core/admission/decision-outcome.ts：**M1 的确定性结果归属**（决策 → 结果；ADR-0081，走 ADR-0082 的原语）。
//
...
//   · **只有事实能进统计**：本模块产出的 `Proposal`/`Confirmation` 必须经 `core/admission/proposal.ts` 的
...
// 天/小时粒度**不再在本模块实现**：与 `core/admission/proposal.ts` 曾各写一份同样的判据（判据分叉风险），
