// dsh-shadow —— core/view/recall.ts：Shadow Usability Layer（v1.5，「记忆恢复」统一入口）。
...
export const renderRecoveryFor = (query: string, t: TaskView | null, refs: ContextRef[]): string => {
  if (!t) return `（未在投影空间中找到可恢复的任务或主题：${query ? D(query) : "空查询"}）\n> 下一步：① 换更短/同义的词再查；② \`read_shadow()\` 无参看 \`.shadow/indexes/_index.md\` 的主题索引与近期记忆；③ 跨「决策/代码/文档」找上下文用 \`shadow_query\`。`;
  const seg: string[] = [];
