// dsh-shadow —— core/view/task.ts：Task Lifecycle 视图（ADR-0039，派生式）。
...
export interface TaskDecision { text: string; source: string; reason: string; at: string; rel: string }
...
export interface TaskView {
  ...
  constraints: string[];
  status: TaskStatus;
  statusNote: string;        // 状态依据（观测信号），不是判断
  startedAt: string;
  endedAt: string;
  decisions: TaskDecision[];
  outcomes: string[];        // 观测到的结果文本（非 success 判断）
  evidence: string[];
  memoryRefs: string[];
}

const fmtT = (hhmmss: string) => { const s = String(hhmmss || "").padEnd(6, "0"); return `${s.slice(0, 2)}:${s.slice(2, 4)}:${s.slice(4, 6)}`; };
...
const atomAt = (p: ParsedMemory) => `${p.date || ""} ${fmtT(p.time)}`;

export const deriveTasks = (parsed: ParsedMemory[]): TaskView[] => {
  const groups = new Map<string, ParsedMemory[]>();
  ...
  for (const [key, atoms] of groups) {
    ...
    const decisions: TaskDecision[] = sorted.flatMap((a) => {
    ...
    if (ABANDON_RE.test(all)) { status = "abandoned"; statusNote = "（观测到放弃/停止/搁置信号，非判断）"; }
    else if (COMPLETE_RE.test(all)) { status = "completed"; statusNote = "（观测到完成/通过信号，非「方案正确」判断）"; }
    out.push({
...
export const renderTasks = (tasks: TaskView[], topic?: string): string => {
  ...
  const filtered = needle
    ...
    : tasks;
  if (!filtered.length) return needle ? `（无匹配任务：${topic}）` : "（暂无 Task 派生结果）";
  const parts = filtered.map((t) => {
    ...
    if (t.constraints.length) seg.push(`- 约束：${t.constraints.slice(0, 6).join("、")}`);
    if (t.evidence.length) seg.push(`- 证据/材料：${t.evidence.slice(0, 6).join("、")}`);
    ...
    if (t.outcomes.length) {
      ...
      for (const o of t.outcomes.slice(0, 8)) seg.push(`    · ${o.slice(0, 70)}`);
    }
