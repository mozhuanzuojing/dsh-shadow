 ...
 */
export const PROCESS_LOCAL_VISIBILITY =
  "宿主进程的 PATH 是启动时快照：**本进程内新装的工具通常要重启宿主才可见**（同进程内的子 Agent 同样看不见）。故预检后**不要**按「先装再派」做计划——处置是**降级**或**告知用户需重启**。";
 ...
 */
export const precheckCapabilities = async (needs: string[], timeoutMs?: number): Promise<PrecheckRow[]> => {
  ...
  return Promise.all(list.map(async (need) => {
    ...
    if (!hits.length) return { need, hit: false, hits: [], available: null, detail: "台账未登记该能力（不编造命令）" };
    // 多命中（如 coreutils 三变体）→ 逐条探测，任一可用即算就位。
    let firstMissing: { available: boolean; detail: string } | null = null;
    for (const c of hits) {
      const p = await probeCapability(c.id, timeoutMs);
      if (p.available) return { need, hit: true, capability: c, hits, available: true, detail: `${c.label} 可用（${p.detail}）` };
 ...
 */
export const renderPrecheck = (rows: PrecheckRow[], platform: string = process.platform): string => {
  ...
  for (const r of rows) {
    ...
    if (r.available !== true) {
      ...
      if (rem) lines.push(`    - 装法（**需用户显式要求 + 宿主审批**，见下）：\`${rem.cmd}\``);
