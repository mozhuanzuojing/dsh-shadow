// dsh-shadow —— core/candidate/semble.ts：Semble（MinishLab）候选生成。ADR-0054。
// 定位（与 zg 逐字同构）：**Semble 是检索层，不是裁决层**。
//   - 只做 Index Engine 的「候选生成」，产出 AtomEvidenceRef 交回 Shadow Core 裁决；
 ...
 */
export const stripBracketedNoProxy = (value: unknown): string =>
  String(value || "")
    .split(",")
    .map((s) => s.trim())
...
const sembleEnv = (): Record<string, string | undefined> => {
  const env: Record<string, string | undefined> = { ...process.env };
  for (const key of ["NO_PROXY", "no_proxy"]) {
    if (env[key] !== undefined) env[key] = stripBracketedNoProxy(env[key]);
  }
  return env;
};

/** 起 semble CLI。返回 { unavailable } / { stdout }，不抛异常（与 runZg 同形）。 */
export const runSemble = async (args: string[], timeoutMs = DEFAULT_TIMEOUT_MS): Promise<any> => {
  try {
    const cp: any = await import("child_process");
    const { execFile } = cp;
    return await new Promise((resolve) => {
      execFile("semble", args, { timeout: timeoutMs, maxBuffer: MAX_BUFFER, env: sembleEnv() }, (err: any, stdout: string, stderr: string) => {
        if (err) {
          if (err.code === "ENOENT") return resolve({ unavailable: true, reason: "semble_not_installed" });
...
export const parseSembleRefs = (stdout: string, ws = "", limit = 20): AtomEvidenceRef[] => {
 ...
 */
export const sembleCandidates = async (query: string, ctx: any, timeoutMs = DEFAULT_TIMEOUT_MS): Promise<{ unavailable?: boolean; reason?: string; refs: AtomEvidenceRef[] }> => {
  const q = String(query || "").trim();
  const ws = String(ctx?.ws || ctx?.workspace || "").trim();
  if (!q || !ws) return { refs: [] };
  const res = await runSemble(["search", q, ws, "--top-k", "20", "--content", "code"], timeoutMs);
  if (res.unavailable) return { unavailable: true, reason: res.reason, refs: [] };
