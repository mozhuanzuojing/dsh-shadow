// dsh-shadow —— core/retention/lifecycle.ts：记忆生命周期状态机 + 热度（从 meta 信号派生）。从 index.ts 迁出。
 ...
 * `subject/observer/arbitrate.ts` 的 `verdictOf`（按同 `entry` 是否存在更新记忆判定，**无 LLM、无相似度阈值**）。
 *
 * **为什么必须由参数传入，而不是读 meta**：`meta[rel].status === "superseded"` 这条**在生产中永不可达**——
 * 已核实生产代码只写 `"active"`（`core/retention/memory.ts`、`query/query.ts`）与 `"compacted"`
