// dsh-shadow —— core/retention/collect.ts：采集辅助（消息提取 / 目标文本 / 用户消息分类）。从 index.ts 迁出。纯函数。
...
// 决策类型（v1.1.1 扩展）：真实用户"关键决策"往往短促，且不只"选择类"——
//   ① selection：选择/保留/删除 X（旧判定）
//   ② scope：范围/聚焦声明（"资产同步"= 当前做哪块）
//   ③ anchor：锚点/定位声明（"…这是 openapi 的 U8 工作区" = 事实基准在哪）
// 三者都属「原文明确存在」的决策；不引 LLM、不补写理由（Evidence≠Interpretation）。
...
export const decisionClass = (text: unknown): DecisionKind => {
  const t = String(text || "").trim();
  if (!t) return "";
  if (SELECTION_RE.test(t)) return "selection";
  if (ANCHOR_RE.test(t)) return "anchor";
  ...
  if ((short && !/\s/.test(t)) || (short && focusLead)) return "scope";
  return "";
};
...
export const classifyUser = (text: unknown) => {
  ...
  if (decisionClass(t)) return "decision";
