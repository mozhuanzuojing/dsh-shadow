// dsh-shadow —— core/retention/change-set.ts：ChangeSet（zg `daemon/change-set.ts` 思想，ADR-0048 ⑤）。
...
//   生产侧 `core/view/projection-store.ts` 只在**类型位置**提到它（`invalidateFor?(set: ChangeSet)`），
