...
**`D1`** `ChangeSet` ⇒ **已于 v1.21.14 改判为「删除」**（用户决策；见 `adr/0086` §6 与 `adr/0062` 补记）。
原判（v1.15.74）是**保留**，理由是它充当 `tools/audit-wiring.selftest.ts` 的**真仓库已知答案**，删它会把一条检测器变弱 ——
⚠ 本次改判**接受该损失**：⑧ 自此**仍然绿，但已不具判别力**（符号不存在 ⇒ `sites === 0` 恒成立）·
