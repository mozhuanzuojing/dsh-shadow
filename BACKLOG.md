...
>
> 最后整理：2026-09-11（`v1.15.34`）—— **现存 19 条**（T 3 / D 7 / V 6 / G 4 + **B3** + **T7** + **T8**）；
> 已结案 **16 条**（B1 / B2 / D4 / D5 / 命中数累积 / `_meta.json` 并发 / `_index.md` 投影漂移 /
> 漂移审计工具 / 图快照顺序 / 台账版本出处 / **T5 漂移键复核** / **T1 A 类分诊** / **T4 零引用定性** /
> **D8 能力矩阵补三列** / **W1 审计工具漏报** / **countInconsistency 接线**）；**D6** 已决策待实现。
...
> **D8 已结案**（v1.15.34）：README「默认开关」表**补齐三列**（成熟度 / 降级行为 / 晋级标准），
> 并**修一处硬缺陷**（`knowledgeEngine.enabled` 生产零读取 ⇒ 那个闸门**不存在**）；
> 顺带产出 **T8**（7 条**静默降级** + 2 处开关缺陷）。
