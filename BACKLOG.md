...
### T12. **时间炸弹 fixture** —— ⚠️ **原判「已闭环（v1.15.43）」被 v1.15.97 推翻**：第 1 颗（引爆日 **2027-01-02**）已修，**第 2 颗（`recall-envelope`，引爆日 2026-09-20）本版已修**；其余 21 文件需按新默认重判
...
### V10. 基线抬到 `0.1.7-rc.2` 只抬了「声明」：四个包的 `lib/**` 变化未逐行核对
...
- **完成判据**：逐包 diff 判定对本仓的影响（本机 5 代 dlx 树可用：`alpha.1`×2 / `alpha.2` / `rc.1` / `rc.2`）；
  或明确写下「按注入服务消费 ⇒ 不核」的判据。
