...
### T20. `MATERIALS.md` **自称「由磁盘枚举生成」却在本仓没有生成器**，且实测**缺 18 项**

- **内容**：`MATERIALS.md` 的文件头把该表定为「本地全部材料的**单一来源**」并给了枚举口径，但实测两头都不成立：
  ...
  （清单与复核命令见 `MATERIALS.md` §2.9 末尾）。
- **依据**：`MATERIALS.md` §1 的注（「换盘前」快照；旧枚举根 `G:\project\dsh1` 已不存在）· §5「更新方法」
  （明文要求重跑枚举命令才能改数）· `MATERIALS.md` §2.9 的实测与复核命令 ·
  **同族先例**：模块归属表的生成器已按**同样理由**重建进 `tools/module-ownership.ts`（v1.16.0）。
- **为什么现在没做**：本轮指令只要「加一份材料」（`jev-ultrafast`，已完成）；代填那 18 行需要逐个枚举
  HEAD / 许可 / 规模，且**该表的权威口径得先定** —— 是**补生成器**，还是**废掉**「由磁盘枚举生成」这句自述。
- **完成判据**：二选一，且**都**要满足后半条 ——
  (a) `tools/` 下有一个生成器，能由磁盘重算名册与规模，`MATERIALS.md` 与 §5 指向它；**或**
  (b) 删掉「由磁盘枚举生成」的自述、明确改成手写台账，并写明**谁在何时复核**；
  并且：那 18 个目录**要么入账、要么被显式记为「不收」**（ADR-0049：**缺件不静默**）。
