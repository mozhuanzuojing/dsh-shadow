...
### T26. `decision/` 的 `choose()` / engines **是否接进读侧或写侧** —— 先定**切片**（`T18` 的残余，`v1.21.30` 拆出）

- **由来**：`T18` 的完成判据① 原文即「若要把 `decision/` 原语也接线，**另切片**」；`v1.20.6` 只接了 **outcome 半边**
  （`mode:"decision"` 调 `attributeOutcomes` / `outcomeReadout` / `renderOutcomeReadout`）。⇒ 本条就是那个「另切片」。
- **约束（不可违反）**：`adr/0096` §7 —— **不新增工具 / 不新增 mode（mode 面保持 62，受门保护）/ 不新增 config 键**
  ⇒ 任何接线只能挂在**既有**读路径或写侧内部。
- **三个候选切片（各自代价，需拍板）**：

  | 切片 | 做什么 | 代价 / 风险 |
  |---|---|---|
  | ① 读侧渲染建议 | `mode:"decision"` 在**本回合有候选**时用 `choose()` 给出确定性建议（对齐 outcome 半边的形状） | 读侧输出多一段；而 `choose()` 的引擎当前**只报 `null`**（`adr/0096` §12）⇒ 多半渲染成「无可用引擎」，**收益待评估** |
  | ② 写侧记录候选 | 决策落盘时把 `choose()` 的候选集一并写进记忆 | 改**写侧语义**（记忆文件内容变）⇒ 要过 `adr/0086` 的契约登记册 |
  | ③ 不接（维持现状） | 原语只由测试/公开面消费 | 与本仓「不造没有消费者的机制」冲突 ⇒ 要么接，要么按 `ADR-0087` 评估**删除** |

- **完成判据**：三选一落定 + 落定后 `npm run verify` 全绿；若选 ③，须同时给出「留着的理由」或删除决定。
- **依据**：`adr/0096` §7 / §12.5；本台账的 `T18`；`test/decision-primitive.test.ts`。
