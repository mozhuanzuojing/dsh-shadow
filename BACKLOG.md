...
### T27. **审计材料进 lineage / evidence 聚合面** —— 卡点是「**审计记录与哪条记忆同属一条线索**」的键（`T21` 的残余，`v1.21.30` 拆出）

- **由来**：`T21` 的三条判据**已全部落地**（`v1.19.1`），台账自己写明再往前一步是「**另一件事**」：
  材料只到「可见可查」（`read_shadow({debug:true})` 的诊断面），**没有**进 lineage / evidence 的聚合面。
- **卡点（必须先回答，否则不写代码）**：审计流与记忆之间**现在只有 `agent` + 日期两个弱键**
  ⇒ 「这条审计记录属于哪条线索」**判不出来**。可选方向：
  ① 用记录里出现的 `rel` / `comp` 路径当键；② 时间窗 + `agent` 归并（弱，易错配）；③ 明确**不接**（审计流只做诊断面）。
- **约束**：`adr/0097` D2/D4 —— 审计流**不进** `_meta.json` 与索引（该不变量不得破）；证据派生只能用**可复核**的键
  （`AtomEvidenceRef` 那层的口径）。
- **完成判据**：键选型落定；若接，附「同一批审计记录 → 同一条线索」的断言；若不接，写明理由（并据此给 `T21` 的终态定性）。
- **依据**：`adr/0097` D2/D4 与 §7.3；`test/granularity-e2e.test.ts` ④⑤；`persistence/audit-stream.ts`。
