...
>
> 最后整理：2026-09-12（`v1.15.45`）—— **现存 22 条**（T 11 / D 6 / V 5 / G 4 + **T7**–**T11**、**T13 后半**–**T15**；
> **B3 / T12 / T16 / V6 已闭环**，**T13 前半 部分完成**）；
> 已结案 **21 条**（B1 / B2 / **B3** / **T12** / **T16** / **V6** / D4 / D5 / 命中数累积 / `_meta.json` 并发 / `_index.md` 投影漂移 /
...
|---|---|---|---|
| **M1** | **Decision Memory**（决策 → 结果 → 经验） | 🟡 **契约草案已出（`adr/0081`）**；**用户已拍板 3 处**，其中第 1 处**我有异议 ⇒ 待确认后开工** | **已有 50%**：`DecisionEvent`/`DecisionReason` 的**分离**与「**绝不生成理由**」**已由 ADR-0037 冻结**（`adr/0037:29-42`）；缺的只有三件 —— **① 决策→结果这条边**、**② 结果结算状态机**、**③ `alternatives`（可选）**。**用户拍板**：① 结果来源＝**允许 LLM 判断归属**（⚠ 我提出**两层合成设计**并列出四条代价，见 `adr/0081` §7.1）；② **引入 `subject`，与 `entry` 并存**（附来源纪律：只接受显式提供，绝不从文本推断）；③ **不设结算窗口，永远 `pending`**（附可见性要求：读数必须印 pending 年龄分布与最老一条）。**开工前置**：确认第 1 点走「两层」（LLM 只进候选层）还是「LLM 直接进事实层」（后者须改契约禁令并登记「归属不可复现」）。 |
