 ...
 *
 * **已知边界 ②（v1.20.2 实测，值得单独记）**：本函数**不认正则字面量**，而正则里可以出现引号 ——
 * `line.match(/…\s*">=([^"]+)"/)` 这种写法会让它把 `"` 当成**字符串开始**，于是**一路错配**，
 * **把该文件从那一行起的其余部分全部空白化**。实测：`tools/docs-consistency.ts` 里**一行**这样的正则，
 * 让文件尾部的 CLI 全被抹掉 ⇒ `countCallSites` 数出 0 个调用点 ⇒ **同一行上的 6 个导出函数一起**
 * 变成 A1 线索，`audit:ratchet` 报 `a1 30 → 36`。
 * ⇒ 两条可操作结论：**① 在被扫描的 `.ts` 里，正则字面量内不要写裸引号**（用 `\x22` / `\x27`）；
 * **② 这道门报「线索变多」时，先怀疑掩码错配，再怀疑真漏接线** —— 本例是掩码的锅，不是代码的锅。
 */
