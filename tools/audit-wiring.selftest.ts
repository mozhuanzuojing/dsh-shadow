...
//
// ⚠ **v1.21.14（D1 改判）之后，本块已不具判别力 —— 空判据，明文记录**：`ChangeSet` 模块已按用户决策删除，
//    于是 `sites === 0` **仍然成立**，但成立的原因是**符号不存在**，而不再是「只在类型位置被提到却从不实例化」。
//    这正是 `adr/0086` §6 当初**拒绝删除**该符号的**唯一理由**，本次改判接受了该损失（见 `adr/0086` §6 与 `adr/0062` 补记）。
//    **保留本块而不删**：删掉它不会让检测器变强，只会让这条**损失**失去书面记录。
// ─────────────────────────────────────────────
