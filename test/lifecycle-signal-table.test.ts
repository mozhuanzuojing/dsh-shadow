...
// ─────────────────────────────────────────────
{
  ...
  // 参数信号的生产者是调用点：主题召回打分（query/topic-score.ts）必须真的把 `v.superseded` 传进 lifecycleOf。
  const q = prod.find((f) => f.file === "query/topic-score.ts");
  assert.ok(q, "应存在 query/topic-score.ts");
  assert.ok(/lifecycleOf\([^;]*v\.superseded/.test(stripComments(q.text)), "query/topic-score.ts 必须把读时裁决 v.superseded 传给 lifecycleOf（参数信号的生产者）");
