...
## [v1.21.14] `D1` 执行：删除 `ChangeSet` / `invalidateFor`（改判，代价已明文记录）

**代码面 + 决策改判；无产品行为改动**（生产一直走 `invalidateProjection` 的粗粒度清空，那条路径未动）。

- **改判（用户 2026-09-25 决策）**：`D1` 原判（`adr/0086` §6，v1.15.74）为「**保留**」，本次改判为「**删除**」。
  触发点：本次会话把 D1 重新摆上台面时，**我最初给的选项漏了原判理由**，补披露后用户仍选择删除。
- **删了什么**：`core/retention/change-set.ts` 整个模块 · `ShadowProjectionStore.invalidateFor?()`（接口 + 实现）·
  `test/change-set.test.ts` · `test/projection-store.test.ts` 的 change-aware 用例 · `adr/0048` ⑤ 的条款地位。
- **⚠ 代价（本条目最要紧的一段，不缩小承诺）**：`adr/0086` §6 当初拒绝删除的**唯一理由**，是它充当
  `tools/audit-wiring.selftest.ts` ⑧ 的**真仓库已知答案**（「删它会把一条检测器变弱」）。**该损失已经发生**，
  且形态是本仓最忌的一种 —— **门仍然绿、判别力消失**：断言 `countCallSites(prod,"ChangeSet").sites === 0`
  依旧成立，但成立的原因是**符号不存在**，不再是「只在类型位置被提到却从不实例化」。
  ⇒ **本仓 A 类检测器自此没有真仓库正例锚点**（⑧ 里那两条反例仍在，仍有判别力）。
  **保留 ⑧ 而不删**：删掉它不会让检测器变强，只会让这条**损失**失去书面记录。
- **改判记录（6 处）**：`adr/0086` §6 补记 · `adr/0062` 补记（本 ADR 的核心实例已被删除，结论随之变化）·
  `adr/0048` 补记（⑤ 条款撤回）· `adr/0095`（引用已删文件的句子改写）· `docs/maintainers.md` 的 D1 行 ·
  `README` 默认开关表那一行 · `BACKLOG` D1 置 ✅。
- **未做（如实列出）**：**没有给 ⑧ 找替代锚点** —— 备选不足：A2a 其余候选与 A1 的零引用者**不测同一种判别**
  （「类型位置提及 ≠ 调用点」才是 ⑧ 的判别点）。
- **棘轮基线（有意更新，非静默漂移）**：删掉该符号后 A 类命中数**下降**；本仓棘轮是**单向**的
  （不许上升、下降须收紧锁住），故用 `node tools/audit-wiring.ts . --update-ratchet` **显式**录制新基线 ——
  `findingsA` 下降 **1**（即那条 A 类命中）、`findingsB` 下降 **1**、corpus 文件数与指纹随之更新。
  ⇒ **基线 diff 本身就是「检测器锚点消失」的收据**。
- **测试总数下降 1 项**（被删的 `test/change-set.test.ts` 所带检查），`verify` 的检查项随之少 1。
...
## [v1.21.11] 四条决策落账：`D1` 删除 · `D2` 做 · `D3` 不做 · `D12` 删数（执行口径待定）

**台账面；无产品行为改动**。用户 2026-09-25 对「三、决策」里四条待拍板项给出选择（先给推荐 + 备选与代价，再选）。

- **`D1` 🟡 已决策：② 删除** —— `ChangeSet` / `invalidateFor` 连同测试与 `ADR-0048⑤` 相应条款一起删。**待执行**。
  ⚠ 已知连带：删除导出会改动 `audit-wiring` 的**棘轮基线**，须**有意**更新它 —— 不许让基线静默跟着漂
  （那正是本仓「能推出来的字段要么别手写、要么配一道门」要防的形态）。
- **`D2` 🟡 已决策：做** —— 「跨项目根注册 + 可信度设计」。**待设计，且须先立 ADR**：它要新增一处**配置面**与
  「哪些根可信、谁有权限加根」的授权模型（涉 **inv 178** 边界）—— 这正是本条原标注的「不是小改」。
- **`D3` ✅ 已决策：不做** —— 维持现有取代机制（`newestByEntryOf` + `verdictOf`：无 LLM、无相似度阈值）。
  依据：带方向的真病根是「同一 `字段=字面量` 出现在多个模块」，而「**口径不一致**」需**类型/语义分析**，
...
- **过程说明**：本条只把**决策本身**落账（对话不算存档）。`D1` 的执行、`D2` 的 ADR + 实现、`D12` 的边界确认
  都是独立轮次 —— 不把「已决策」写成「已执行」。
- **验证**：`SHADOW_EVAL_ROOT=D:\project\net1 npm run verify`。

## [v1.21.10] 台账闭合两项：`T12` 时间炸弹扫清（672 次 0 红）· `V10` rc.2 消费面判定
...
**待办台账面；无产品行为改动**。用户「继续」= 执行上一条提出的那件事：把本次会话产生的未核项写进台账 ——
...
- **验证**：写盘用一次性脚本（**逐条打印匹配数、任一条 ≠1 即拒绝写盘**）+ 结构不变量
  **`## ` 9→9 · `### ` 77→82（+5：T23 / T24 / D12 / V10 / V11）**；新增条目**逐条 grep 落位确认**
  （T23 L1116 · T24 L1124 · D2 补注 L1164 落在 D2 与 D3 之间 · D12 L1378 · V10 L1590 · V11 L1600）；
  `SHADOW_EVAL_ROOT=D:\project\net1 npm run verify`。

## [v1.21.8] `docs/` 历史报告产物：留仓者移出发布面、本地产物删除（包 1.8 MB → 621.5 kB）
...
**工具面；无产品行为改动**。这是 `v1.21.6` 那条「可复现性说明」承诺的落地。

- **问题**：`v1.21.6` 量「归档层有多少**越界**」用的是一次性脚本（未入库）⇒ 结论**不可复现**。
  而默认门按设计把归档层排除（`currentStateDocs` 不含 `CHANGELOG.md` 与冻结 ADR）⇒ 此前**没有任何工具能回答**这个问题。
- **做法（关键：判据不许两份实现）**：把判定本体抽成 **`judge(ROOT, docs, MATERIALS)`** ——
  「什么算引用 / 什么算越界 / 什么算未判定」**只此一份**；`checkCitations`（门）与新增的
  `checkArchiveCitations`（归档层度量）**共用它**，差别只在**呈现与退出码**。
  另加 `archiveDocs(root)` = `CHANGELOG.md` + **全部** `adr/*.md`（**含冻结**）。
- **边界（写进输出，不只写进文档）**：归档层**只报不判** —— 行号是「**当时**」语义，越界是**历史事实**，
...
因为当时的门只比**版本号**（检查①）与 **`HOST_BASELINE` vs `engines.dsh`**（`test/host-probe.test.ts` ⑥），
**没有一处**守「文档里的基线 = `engines.dsh`」。按本仓那条一般化规则
（**任何能推出来的字段 —— 版本号 / 计数 / 链路清单 —— 要么别手写，要么配一道门**），补门。

### ① 新检查

`tools/docs-consistency.ts` 的 `checkBaselineConsistency`（检查 ⑦）：**三处**必须逐字等于
`package.json` 的 `engines.dsh`（去掉 `>=` 前缀后）。

| 处 | 行锚点 | 判据 |
|---|---|---|
| `README.md` | 行首 `\| 验证基线 \|` | 该行必须含内联码 `` `<基线>` `` |
| `README.md` | 行首 `\| 声明 \|` | 该行必须含 `engines.dsh: ">=<基线>"` |
| `CONTEXT.md` | 行首 `\| 验证基线…` | 同上 |

- **只按行锚点定位**，不做全篇搜索 —— 检查① 在 v1.15.66 就因「全篇第一处」被散文抢走匹配而收紧过。
- **结构缺失报 2**：读不到 `engines.dsh`，或 `README` 连基线表都没有 ⇒ **不是「通过」**（ADR-0049）。
- **刻意窄**：不判正文里对历史基线的追述（「为什么从 A 抬到 B」那段**本就该**含旧版本号），
  也不判 `presets/README` 的「(current as of X)」这类**上游状态**叙述。

### ② 标定（`tools/docs-consistency.selftest.ts` ㉔–㉘）

正对照（三处一致⇒0）· README 落后⇒1 · **CONTEXT 落后⇒1**（三处里最容易漏的一份）·
缺 `engines.dsh`⇒2 · 缺基线表⇒2。

⚠ **并给所有既有 fixture 补了本检查的前置**（`engines.dsh` + README 基线表 + `CONTEXT.md`）——
本仓自己的纪律：**新加一条门，就要给所有 fixture 补它的前置**（v1.15.82 加检查⑤ 时记下），
否则每个 fixture 都会以结构缺失(2) 退出、把别的组的断言全短路。

### ③ 真缺陷回档复验（不是只跑自造 fixture）

把 `README.md` 的基线改回 `0.1.7-alpha.1`（复现 `0706f4c` 的形态）⇒ ⑦ 变红，且**点名两份文档**：
「`README.md` 的 `| 验证基线 |` 行未含内联码 `0.1.7-alpha.2`」+「`README.md` 的 `| 声明 |` 行写的是 `0.1.7-alpha.1`」；
`git checkout` 复档后回到 0。

### ④ 文档同步

`AGENTS.md` 的 `audit:docs` 检查表加第 ⑦ 行；`README.md` 的闸门块注释里点名 ⑦。
...
- **已把边界写进 `maskStrings` 的「已知边界 ②」**（含这次的可重放读数 `30 → 36`），并记下两条可操作结论：
  ① 在被扫描的 `.ts` 里，正则字面量内不要写裸引号（用 `\x22` / `\x27`）；
  ② 这道门报「线索变多」时，**先怀疑掩码错配，再怀疑真漏接线**。

**插件代码零改动**（只动工具面与文档）。

## [v1.20.1] 基线抬到 DSH `0.1.7-alpha.2` + 补全 `v1.20.0` 遗留的当前态口径

`0706f4c`（chore）把 `engines.dsh` 与 `HOST_BASELINE` 抬到 `0.1.7-alpha.2`，但只动了
`index.ts` / `package.json` / `dist/index.js` —— **当前态文档仍写着 `0.1.7-alpha.1`**：
`README` 的验证基线表与「为什么」段、`README` 当前版本行、`CONTEXT` 的「验证基线」术语、
`presets/README` 两处。本版补全这部分，并给出这一变更应有的**版本号 + tag**。

### ① 抬升的理由与证据

理由同 ADR-0098 §1/§2.4：抬的是**形态**（预设只能是声明行），不是接口。**alpha.2 上的实测
（用户 live 环境，2026-09-23）**：宿主 `dsh --version` = `0.1.7-alpha.2`；
`dsh --profile web --dump-config` 里 `agent-preset-registry` / `preset-standard` / 团队三行 /
`dsh-shadow` / **`preset-projection`** 全在；本会话持续落盘（最新记忆原子 2026-09-23 09:01:38）
⇒ 这条声明**有证据**，不是「抬完就算」。

### ② 为什么这次单独立一版

`0706f4c` 改的是**声明面**（`engines.dsh` + 插件描述 + `HOST_BASELINE`），却与 `v1.20.0` 共用版本号与
tag —— 于是 `v1.20.0` 那个 tag 指向的提交里 `engines.dsh` 是 `alpha.1`，而 main 上已是 `alpha.2`：
**同一个「v1.20.0」对应两个不同的基线声明**。按本仓两条仪式（「发版三处一起改」「每发一版打一个 tag」）
这一变更必须落成一个版本 ⇒ **v1.20.1**。

### ③ 有意不改的东西

- ADR-0098 正文里 `0.1.7-alpha.1` 的叙述**保留**（那是**决策当时**的事实，不是当前态引用）：按本仓口径
  「已冻结的正文不改写，只写补记」，本轮在它末尾加补记指向本版。
- **插件代码零改动**：本版只动文档 + 版本号（与 `dist` 无关，故 `dist` 也不变）。
...
- **不碰**：`_index.md`（派生件；日期目录令牌变了，下一次读自会重建）、合并件（`ep-`）、`query-log/`、
  `resources/`、任何非「纯动作」的记忆文件。

### ② T21：审计流接读侧（**只到「可见可查」**，不越线）

- 新增 `persistence/audit-stream.ts`：`readAuditStream`（records / materials / dates / **tornLines** / `reason`）
  + `renderAuditStreamDiag`。材料 = `materials` 字段 ∪ 从 `改/读 <path>` 推断（回收来的记录两种都有）。
- 挂在 **`read_shadow({debug:true})` 的诊断区**。**刻意不进召回正文** —— 整个 `adr/0097` 就建立在
...
- 新增 `test/granularity-e2e.test.ts`：**假宿主 + `createShadowCollector`** 驱动**真实 `flush`**，
...
- 门的边界文案更新（原文写着「§5.3 历史回收**未做**」——已成过期陈述）；`adr/0097` 新增 **§7 补记**
  （回收 / T21 / T22 的落地与证据），并更新 §4（效果从「推算」变**实测**）与 §5（边界逐条改成现状）。
- `BACKLOG`：**T22 结案**；**T21 → 部分收口**（写清「只到可见可查、聚合面未做」）。

### 未做 / 边界

- 审计材料**未接进** lineage/evidence 聚合面（T21 继续挂着）。
- 跨进程并发追加审计流**未测**（与 `query-log` 同一边界）。
- 门的覆盖仍要从**起点之后的第一次采集**才开始非零（回收后读数：判定 0 · 豁免 5,423）。

### 验证

`npm run verify` 全绿（新增一个测试文件与两个工具自检都被 `run-tests` 自动收录）；回收的完整性由**独立对账**
与**逐字抽查**另行证明（见上，不依赖工具的自我报告）。


## [v1.19.0] 记录粒度：**动作回声降级为审计流**（`adr/0097`）+ 粒度门 + 两处「判据收一处」

用户质疑「一条记忆 = 一个文件」这个设计（原话「这个设计不是很好，重新思考」）。
...
（实测 **11,834 原子 + 2,508 收口件**）⇒ 文件数**一个没少**。

### ② 改了什么（`adr/0097` D1–D7）

- **D1 判据（可机械化，且不新增字段）**：一批 `pending` **全是 `action`** ⇒ 审计流；
  **出现任何别的 kind**（`user` / `decision` / `assistant`（思维落点）/ 将来新增的未知 kind）⇒ 记忆文件。
  复核判据是**现成的** `core/memory.ts:60` 的 `> 证据链：来源(动作·用户·agent·决策)` —— 「只有动作」= 纯动作回声。
- **D2 落点**：`.shadow/audit/<date>.jsonl`，**一行一条 JSON**（`at` / `kind` / `comp` / `text` / `source` / `sub` / `agent` / `project`）
  ⇒ **一字不丢**，只是不再各占一个 inode、不再各带一份样板头。属**系统派生记录层**（同 `query-log/`）。
- **D3 读侧不受影响（逐条证明）**：`persistence/files.ts:50` 只枚举 `^\d{4}-\d{2}-\d{2}$` 的目录 ⇒ **整个 `audit/` 不被收**；
...
- **JSONL 追加收一处**：抽 `persistence/jsonl-append.ts`（原 `query/observatory.ts` 内联的那一份），
  `query-log` 与审计流共用；`errText` 同时提到 `core/util.ts`。
  **顺带修掉一类丢行**：原来的「读回 → 拼接 → 写回」没有排队 ⇒ 同进程内两次并发追加会各自读到同一份 `prev`、
  后写覆盖先写（`query-log` 一直有这个窗口）。现在同一 `rel` 串行（**跨进程仍未测**，边界写在文件头）。
- **语料根解析收一处**：抽 `tools/eval-root.lib.ts`（原 `retrieval-eval.ts` 内联），粒度门与评测共用。
...
`npm run verify` 全绿（`audit:docs` 的 ② 已自动认成 **10 步**；`audit:ratchet` 与 A1/B 桶与基线逐桶相等 ——
新判据一律用**私有常量**比较、导出只留被别处 import 的，避开了「比较点新字面量」与「导出但生产无调用点」两个桶）。
...
| 引用（`adr/0095` 正文） | `c1c870a4`（正文写作时） | `052d26ee`（现在） |
|---|---|---|
| `:17-19` FTS5 + 向量 + 混合 | ✔ | ✔ `:17-19`（插入点在它之后） |
| `:24` sqlite-vec optional | ✔ | ✔ `:24` |
| `:117-119` / `:138` / `:141` | ✔ | `:120` / `:141` / `:144` |
| `:212-215` / `:249` / `:259` / `:285-288` | ✔ | `:215` / `:252` / `:262` / `:288` |

`memory-architecture.md` / `memory-config.md` **一字未动**（已复核）。
⇒ **旧 pin 上全部命中、新 pin 上全部失配** ⇒ **是我这次更新造成的**，不是我记错。

### ② 处置：**冻结 ADR 不改正文**（写补记）+ 立 **pin** 规则
...
- `adr/0081` 同一张表把**注释**当判据引（`reflection/types.ts:22`、`reflection/engine.ts:20`）——
  同一个错在 `adr/0096` 自审时已经改过一次，**那一份修只顾了一条**（本仓点名的反面教材）
  ⇒ 订正为 `reflection/types.ts:35`（`reflectionEligible: hasDecision && hasOutcome`）
  与 `reflection/engine.ts:22`（`reflectTraces` 里的 `filter`）。

### ④ 新门：`audit:docs` 检查 ⑥（`tools/citation-audit.{lib,ts,selftest.ts}`）
...
⇒ 本次更新**没有**造成引用腐烂；**但这个核对步骤留下了**（下次换材料仍要跑）。
...
⇒ 「最新」永远是**检查时刻**的读数，**不是持久事实**（这也正是缓存必须有 TTL 的原因）。

**验证**：`SHADOW_EVAL_ROOT=D:\project\dsh1 npm run verify` → exit 0，末行 `[run-tests] ALL PASS ✅`。


## [v1.18.2] 新规则：材料（`vendor/_src`）的 git **访问前先查最新** + 工具 `tools/materials-freshness.ts`

用户 2026-09-20 指令：「**材料里面的 git 每次访问先检查最新，并更新到最新**」。

### ① 规则落在 `AGENTS.md`（本仓的「清单」）+ `MATERIALS.md` §5

**检查是默认动作**（只读、便宜、离线可降级）；**更新是显式的**。

### ② 但「更新到最新」有两个意思 —— 这条必须先说清，否则规则会**骗人**

| 做法 | 效果 | 代价 |
|---|---|---|
| `--update` | 只把 **HEAD** 移到最新（`git reset --mixed`） | **工作树不动** ⇒ **读文件读到的仍是旧内容**；台账只能说「上游现在是哪个 sha」 |
| `--update --hard` | **连工作树一起更新** | **覆盖 vendored 拷贝** —— 本机的裁剪/改动会丢；未跟踪文件**不会**被清（工具**不做** `git clean`） |

⇒ 默认走**非破坏**那条；**要真读到最新内容才用 `--hard`**，且工具会**先把将被覆盖的差异数打出来**。

### ③ 三条硬边界（都是本仓已有纪律的推论，不是新发明的限制）

1. **冻结快照不动**：名字带 `--snapshot-<版本>` 的（`langextract--snapshot-v1.6.0`）**按设计**停在那版；
   更新它＝它**不再是那个快照** ⇒ 必须 `--include-frozen` **显式**要求。
2. **先看爆炸半径再更新**：工具会列出「**哪些文档里有该材料的 `文件:行号` 引用**」，
   更新后必须回头核对 —— 「**行号是最易腐烂的引用形态**」是本仓 `AGENTS.md` 的头号纪律。
3. **它不进 `verify`**：因为**要联网**。本仓的门**刻意不联网**（同族：`audit:docs` 检查⑤ 宁可读 ref 文件也不 spawn git）。
   把要联网的东西塞进 `verify` ⇒ 离线时的「全绿」会变成**假红 / 假绿**。
...
**18 最新 / 6 落后**：`langextract` · **`langextract--snapshot-v1.6.0`（冻结）** · `openclaw` · `PageIndex` · `rtk` · `zvec-grep`。

⚠ **本轮不执行更新** —— 按 ②，`--hard` 是**破坏性**的（要覆盖 vendored 拷贝），该由人来选；`--update`
虽然非破坏，但它**不会**让你读到最新内容，单独做只会把台账的「工作树 vs HEAD」列变大。
...
- `langextract--snapshot-v1.6.0` 取的是 **tag `v1.6.0`** —— 它是**版本快照**，
  取 `main` 会把 v1.7.0 的溯源**贴到 v1.6.0 的拷贝上**，那是**错的**。
- 其余取各仓的**默认分支**（实测 `hackingtool` 与 `marker` 是 `master`，不是 `main`）。

### ② 上游怎么确定的：**判据分级** + **接错即回滚**

不敢凭「看起来像」就接 —— **接错比没有更坏**：它会让一个**假的**溯源**看起来像机器核过的**。分四级：

| 判据 | 用在 | 强度 |
|---|---|---|
| **仓内自指**（`package.json.repository` / `pyproject` 的 `Repository =` / README 的 shields 徽章自指本仓） | `marker` · `langextract--snapshot-v1.6.0` · `system_prompts_leaks` | **强** |
| **旧枚举根记录**（`MATERIALS.md` §1 的远端列） | `archify` · `OpenViking` | 中 |
...
**不按状态码分桶** —— 既够用，又避免再引入「内联字面量比较」（那已被棘轮抓到过两次）。
...
## [v1.18.0] `MATERIALS.md` 的生成器进本仓（T20 选 (a)）：24 项全入账 + 实测出「18 份材料**没有版本溯源**」
...
- 一条命令重算名册：**目录 / 远端 / HEAD / 许可 / 含 `.git` 与不含 `.git` 两栏规模 / 台账状态**。
- **口径自己打印**（§5 的纪律 ①）：文件与字节递归、**含**隐藏文件；许可取
  `LICENSE|LICENCE|COPYING*` 中**第一个**的首行 + 关键词识别（**识别不出就原样打首行，不猜**）。
- **「无 `.git`」与「git 降级」分开报** —— 前者是说「这份材料**没有版本溯源**」，
  后者才是「工具取不到」。两者混为一谈就会把一个**事实缺口**读成一次**工具故障**。
- **结构缺失不是通过**：枚举根不存在或为空 ⇒ **`exit 2`**，拒绝输出空表
  （否则空表会被读成「本机没有材料」—— 与 `audit-layers` 拒绝输出「假绿」同款）。
- 脚本式、**不导出任何符号** ⇒ 与 `tools/module-ownership.ts` 同款，**不给棘轮添 `a1`**。
- ⚠ **门又抓到一次**（同一纪律的**第二次**）：初版把 `name === ".git"` **内联**，`audit:ratchet` 立即判红
  `b_keys 95 → 96（+1）`（键 = `name=.git`）；收成一处 `isGitDir()` 后回到 **95**。
...
v1.16.x 的 `adr/0096` §12 与 `BACKLOG` 的 T19 把下一程写成「**概率型后端（LLM / jev）落地前**必须重审 0037」
—— 那是「把一个外部服务接进来，再审它合不合规」。真正的差别是结构性的：

| | **接入** | **吸收** |
|---|---|---|
| 对象 | 一个**外部服务** | Jev 那套 **Typed Decision 的思想** |
| 原语的自洽靠什么 | **靠后端行为**（「等它落地再判」） | **靠原语的形状本身** |
| 0037 那道门 | 每换一个后端重审一次 | **一次定死、与后端无关** |

### ② 症状就藏在 v1.16.0 那四条承重理由里

其中一条是「**生产里根本不存在分布**（唯一引擎报 `null`）」。那是**吸收没做完的自白**：
如果一个原语合不合 0037，要靠**某个后端当前的行为**来判断，被吸收的就只是**调用约定**，不是原语。

### ③ 修法：**吸收形状，不吸收置信度**

| | 该不该吸收 | 与 `adr/0037` |
|---|---|---|
| Typed Decision 的**形状**（候选集 / `selected` / 原始依据 / 血缘） | **这才是要吸收的** | **无冲突** |
...
### ④ `adr/0037` 的「❌ Confidence」**结构性成立**，`T19` **结案**

- 现在不是「我们保证不用置信度」，而是 **`EngineDeclaration` 的字段集恰好是那四个**
  —— **类型里没有那个槽**，所以**任何后端都引入不了**，与谁做后端无关。
- 由新测试第 ⑦ 块**静态断言 + 标定**守着：加任何字段即红；检测器本身也用**合成样本**自证
  看得见非法字段（否则那条断言是恒真的假绿）。
- ⇒ **`BACKLOG` 的 T19 结案**。⚠ **这是「设计改了」而不是「风险被证伪」** ——
  0037 的顾虑本身没有被推翻，只是**不再有承受它的结构**。
  残余边界如实写在 `adr/0096` §12.5：`rawOutput` 里的数字会渲染给模型看，
  模型要不要据此行动**不在本层管辖内**。

### ⑤ 棘轮**下降**，并按工具提示收紧基线

删除导出后 `audit:ratchet` 报 `a1 31 → 30` · `a_total 46 → 45`（**下降不违规**，
工具提示「建议 `--update-ratchet` 收紧」）⇒ 两个 `--update-ratchet` 都跑了，两段都报「与基线逐桶相等 ✅」。
⇒ 相对 T1 之前的净增量变为 **`a1 +7` · `a_total +7`**（`adr/0096` §9.2 第 5 步已同步）。
...
- **最弱的一环没写下来**（实质问题，不是笔误）：`reportedDistribution` 与 `adr/0037` 的「❌ Confidence」
  ...
  v1.16.0 用的是**最弱的论据**（命名）。⇒ 新增 **`adr/0096` §12**：写明真正承重的四条
  （谁算的 / 怎么留的 / 谁消费 / **生产里根本不存在**），并明说
  **「T1 的一部分安全性来自『还没有概率后端』这个事实本身」** ⇒ 那一版**必须重审、另立 ADR，不得继承结论**。
...
- **纪律与 `tools/run-tests.ts` 同款**：**每个文件一个子进程**（插件在模块级注册表上有全局副作用，同进程串跑会互相污染）；
  `stdio` 走**文件描述符而不是管道**（受约束沙箱下 pipe 会被拒 EPERM）；假日期 shim 用 **`data:` URL** 交给 `--import`，
  **不落任何临时文件**（本仓禁止手写 `.js` / `.mjs`）。
- **退出码**：`0` = 全过；`1` = 有红（列出日期 × 文件 + 断言首行 + 按文件归并）；`2` = **结构性缺件**（找不到测试文件）——
  按 ADR-0049，**缺件不静默、不得当成「通过」**。
- **刻意不进 `verify`**：一次全量是分钟级，而 `verify` 要的是**快速缝合线**。触发条件写进 `AGENTS.md`：
  **凡改 `forget` / `retention` / `compact` 这类默认值的变更，跑一次 `npm run sweep:timebomb`**。
- 用法（复审者能重放）：`npm run sweep:timebomb` · `node tools/timebomb-sweep.ts --dates 2026-09-21,2027-01-01` ·
...
- 并在 `## 它做什么` 章首补一段导读：本章是**能力分组**，带「可跳」标记的小节**整段跳过不影响使用**，日常入口仍是首屏那三个。
- ⚠ **已知边界（不假装有门）**：这三块**至今没有任何自动检查盯着**（已 grep 确认）⇒ 这次的标注靠人维持。要机械化，得先有「维护者面」的**可判定标记**，本仓暂无此判据 ⇒ 记为下一步，**不顺手造约定**。

**② `CONTEXT.md` 术语表：「投影」由「三样并列」改成「有向」**

- 旧写法把 agent 的思维 / 上下文 / 灵魂**并列**成「落到文件系统上的具象化」—— 读不出**谁是源**。
- 新写法**有向**：**灵魂**（身份 / 价值观 / 原则 / 品味 / 边界）**投出思维**（判断与决策），思维再**落盘**成文件树，落点 = 记忆树 `.shadow/`；**三者不是并列关系**。
- 两条机制依据写进定义本身（便于复核）：`query/query.ts` 的 `projectContext(..., soul, identity.observerLens, …)` = **把灵魂当透镜**；`identity/evaluator.ts` 的「读反思 → 生成候选 → 三道闸门 → 推进 identity，**不自动改 `soul.json`**」= **回流必须过闸，源头只有人能改**。
- 这是 `v1.15.92` 首屏那句「**思维是灵魂的投影**」的**同一口径**，此前只落在 README、**没落到术语表** ⇒ 同一件事两个说法，读的人会以为定义本身错了。
...
- **投影三层按用户的口径写**：**灵魂**（它是谁、在乎什么、不干什么；很少变，只有人能改）→ **思维**（按人设做的判断，
  **人设投出的影子**）→ **笔记长成人设的新版本**（要反复复盘、过门槛）。用户原话「**思维是灵魂的投影**」；
  机制依据两条：`query/query.ts` 的 `projectContext(..., soul, identity.observerLens, ...)`（**把灵魂当透镜**）·
  `identity/evaluator.ts` 的注释「读反思 → 生成候选 → 三道闸门 → 推进 identity；**不自动改 `soul.json`**」。
- 顺带记一处**文档与机制不符**：`CONTEXT.md` 的「投影」定义把三样写成**并列**，而机制是**有向**的（灵魂是源）。
  **本轮不代改 `CONTEXT.md`**（它另有归属），列为下一步。
...
- **门必须跟着挪，而不是去掉**：`audit:docs` ④ 原先盯的是「版本历史表里当前版本那一行」——
  表一删，旧锚点**永久消失**，该检查只会报「结构缺失」、不再回答任何问题。⇒ 锚点换成**每次发版都会更新**的
  当前版本行（行首 `**当前版本：`）：**判据一字未改**（README 的当前状态摘要与 `CHANGELOG` 同名条目之间
  不得有 ≥40 字逐字重复）、**负对照仍在**（标定 ⑮ 整段复制 ⇒ 1）、**正对照仍在**（⑯ 短片段 ⇒ 0）。
  **这不是「改门掩盖失败」**：被删的是**制品**、不是**判据**；且这次改门**在同一版内被门自己验证**过
...
## [v1.15.91] 两件：省略披露开关（`adr/0092`）+ 标准格式出口的评估（`adr/0093`）

用户 2026-09-15 在本轮末尾给的两个选项，**两件都做**。

**① 召回路径的省略披露做成默认开关（`adr/0092`）**

- **键** `recall.lossDisclosure`（`core/types.ts` 的 `recall` 段，扁平布尔，与同段 `debug` 同形）。
  **默认 = 开**，判据**只有一处**：`core/util.ts` 的 `onByDefault`（`undefined` = 开，只有显式 `false` 才关），
  与 v1.15.85 的三个量控开关**共用同一份判据**。
  **为什么默认开**：默认值即产品 —— 默认关等于默认放弃 `adr/0090` 甲-1 的判据（「读侧能区分『无内容』与『被省略』」）。
- **语义边界（本版最要紧的一句）**：**只改「说了什么」，不改「给了什么」**。降档与丢条目的判据与开关**无关**；
  关掉后返回的**条目标识与内容逐字不变**。⇒ **做成了断言**：`test/recall-envelope.test.ts` ⑦ 现在比较
  「开 / 关」两侧的 `[.shadow/…]` 条目标识序列**必须逐字相同**，且关闭侧更短。
...
- **边界**：**未实测** SARIF 的消费链路（未装扫描平台、未跑 strix）· **未读** SARIF 2.1.0 规范 ·
  **未做**全仓「机读产物」普查（§1 的表来自 `tools/*.json` / `docs/*.json` 的枚举 + `package.json` 的 `dsh` 段）。
...
`adr/0087` §C 当初不实装的理由是「每一条都有一个必须先拍板的前置」，本条把前置写成决定（`adr/0090` §A）。

**① 甲-1（D9）：省略必须声明损失形态 + 交出恢复句柄**

- **恢复载体 = 已有的源文件**（句柄 = 路径 + 段名/入口名）。rtk 必须造存储（SQLite + gzip + 内容哈希去重 + 老化），
...
- 新文件 `retrieval/loss.ts`：`Lossiness{none,tail,whole}` · `RecoverHandle` · `handleText` · `lossLine` ·
  `excerptWorthwhile`（区分「本来就短」）· `tierLossNote`（召回路径**条内**损失，一次披露、O(1) 输出）。
  索引路径在 `retrieval/render.ts` 的披露里给句柄；召回路径在 `query/query.ts` 记录「本是够长却没给片段」的条目。

**② 甲-2（D10）：never_worse 守卫，单位 = 字符**

- 口径：**比字符数**（与预算 `max_tokens × 4` **同单位**），**不用** rtk 的「字节 / 4」——
  ASCII 1 字节/字、中文 3 字节/字 ⇒ 比字节在**中英混排**下会把方向判反。
- **把它做成断言而不是注释**：`raw = "中"×60`（60 字符 / 180 字节）vs `longer = "a"×62`（62 字符 / 62 字节）
  ⇒ `neverWorseChars(longer, raw) === raw`（判出更长），而 `byteLength(longer) < byteLength(raw)`（字节口径漏判）。
- 出口唯一：`renderIndexBudgeted` 的返回（正文 + 披露）与 `query/query.ts` 的降档判定（`degraded.length <= render.length`）。
...
1. **`audit:ratchet` 一度变红（B 键 97 → 100）**：新写的三个字符串判等（`kind === "measured"` / `kind === "authority"` /
   `loss === "none"`）被接线审计读成「只读不写的判断值」。**没有去改基线**（改基线 = 让门更松），
   改成**查表**后回到 97 —— 顺带把「枚举 → 标签」的映射收成唯一一份。
2. **披露会被段名撑爆（实测）**：`renderIndexBudgeted("## " + 4000 字, 20)` 把**整行**当段名抄进「未返回的段」
   ⇒ 披露自己 4000+ 字 ⇒ 守卫退回原文（假象：以为硬截断没自报）。**修**：披露里的段名**限长 48 字**。
3. **`claimOf` 的调用点不止一处**（`ledgerMismatch` 内部也调）⇒ 改签名必须同步 CLI 入参类型，否则
   「棘轮走新判据、CLI 走旧形状」= 同一判据两份实现。

**⑤ 一处行为变更（留档，不是隐藏）**：`test/recall-envelope.test.ts` 的「连第一节都放不下 ⇒ 硬截断必须自报」
原先用**单行 400 字**语料；加守卫后，该用例**必然**退化成「退回原文」（403 字原文 vs 400+ 字披露），
于是断言**必然**失败。处理：**把语料放大到 4003 字**（用例原意不变）+ **新增**一条断言钉住
「披露比原文长 ⇒ 逐字退回原文」。⇒ 两件事各自有断言，新行为是 D10 的**规格**。
...
用户 2026-09-15 的指令是「这个项目和参考资料对比 **一一对比，深层次**」。按本仓口径这句话拆成两半：
**先证明「参考资料」是哪一套**（否则比的是清单、不是材料），**再逐份判定**。
**本轮不重读任何材料的内容**（已读的见各 ADR 与 `references.md` §6.3 覆盖率台账）；交付物 = `adr/0089` + `references.md` §14。
...
**② 逐份判定：四问量尺**（同题？权威源在哪？结论从哪来？有没有本仓缺的形态？）

- **A 同题（3）**：`OpenViking`（只取概念 / AGPL）· `hl_mem`（只取治理与体例；两处硬冲突：LLM 提取 + 常驻服务）·
...
**被吸收的只有判据与门禁、被拒的是运行时与算法**（痕迹前四名里没有一条把它们的检索算法搬进来）；
本仓把「证据」拆成**记忆 ≠ 证据 ≠ 指令**三层分别判，而材料里它多只是一个字段；
**手写清单必漂移** ⇒ 由此立判据：**清单类字段登记时必须带「枚举根 + 枚举时刻」**。

**④ 一处自审：差一点写进去一个自失真的数**

「痕迹」列是**跟踪文档里的提及次数**，而 `adr/0089` **自己**就反复提到这些名字 ⇒ 同一条命令
**不排除本条**时 `hl_mem` = **137**、排除后 = **123**（`OpenViking` **91 → 83**）。
口径已写进正文与 §5 的重放命令（**排除 `adr/0089` 自身**）。

**⑤ 重放性自审（写完之后逐条跑 §5 的命令，抓到三处，都不是顺手改的）**

1. `references-agents` 的**上跳级数写错**：`.docs` 在 `vendor/`（一级），而 `references-agents` 在 `dsh1/`（**两级**）
...
改数必须重跑枚举命令，而那个根已不可用）。**未改任何源码、未动 `dist/`**；`verify` 全绿（检查条数由 `run-tests` 自己打印）。
**未做**：材料内容一律未重读 ⇒ §2 的 D 类九份「无可比面」仍是**基于定位的判断**，要改成有判据的结论必须真读；
`hl_mem` / 评测分数 / 两篇综述与 Scrub Jay 全文**都还欠着**（§6 已逐条写明）。
...
「允许 ts py ps1 等脚本 但是不要 js 和 mjs 如果是 mjs js 迁移到 ts」。⇒ 规则收成三句，写进 `AGENTS.md`：
...
⇒ 该目录**此前从不在语料里**，这条排除是**精确还原**，不是放闸；`audit-wiring` 则 `files 888`（基线 887，+1 是 v1.15.85 新增的 `test/volume-defaults.test.ts`）、`b_keys 97` = 基线 ⇒ **两个棘轮都通过**。

⚠ **边界（写在两处注释里）**：这是**按名字**排除 —— 若哪天 `_research/` 被 `git add`（`git ls-files _research` 非空），这条排除**必须删掉**。
⚠ **同一判据现在有两处**（两个工具各自持有一份 walker）—— 与它们本来就重复的 walker 同源，未合并不是漏做，是这轮不动它。
**本版没做（如实标注）**：`_research/` **不在版本控制内** ⇒ 这次迁移**不进版本库**、只在本机工作区生效；
要让它可复核得按 `AGENTS.md` 的判据（**能复现的东西放 `tools/`**）搬进去 —— **未做**。
**给这条规则配门也未做**：工作区扫描（排除 `dist/`、`node_modules/`）只要十几行，但新加一个 `verify` 步骤要同时改
`package.json`、`README` 的闸门块与 `AGENTS.md` 的链路行（否则 `audit:docs` 检查 ② 的**反向半边**会红）⇒ 留给下一版。



## [v1.15.85] 「默认全开」：三个量控开关的默认值翻成开 + 无参 `read_shadow()` 的索引预算（`adr/0088`）
...
⇒ 判据收成**一处**：`core/util.ts` 新增 `onByDefault(v) => v !== false`（`adr/0084`「显式 0 ≠ 未传」的**布尔版**），
四处读取点全部改走它。**第三处必须单独点名**：`maxActive` 闸门**不在**三个「开关」的名单里、只**读** `forget.enabled` ——
不改它就会出现「开关开了、它的一个子能力默认睡着了」这种**半开**形态。

**四条边界（都是决定，不是顺带发现）**：

① **默认开 ≠ 不写工作区** —— 三件套都会写（`_meta.json` / 移出活跃集 / `-consolidated.md`）⇒ `README` 原先那句
「会写、会烧 token 的增强默认都关」**不再成立**，已改写（「只读工具不写工作区」是另一句话，仍成立）。
② **不减磁盘占用**（被反复问到的正是这个）：`Forget ≠ Delete`（ADR-0031）⇒ 被遗忘的记忆**文件仍在**；
收口**不删原子**。三件套缩的是**活跃集 / 索引**，不是文件数、不是盘上占用 —— 要减盘上占用必须另有动作，本版**不做**。
③ **风险面被放大，如实标注**：`ADR-0067` 的 `minHits` 保护链与 `ADR-0068` 的 `runCompact` delta **真机未验**，
此前挂着「默认关 ⇒ 不执行 ⇒ 风险为零」；翻成默认开后它们**从不执行变成在生产路径上**。本版不撤回默认全开（用户决定），
但**不给它任何「已验证」的说法**。
④ **`README` 表里 `retention` 那格的 ⚠️静默语义随之变化**：默认关时它与 `adr/0085` 的裁定「用户显式 `enabled:false`
不留痕」**互相矛盾**（关是默认态，不能说是用户的选择）；默认开后它自动成立 ⇒ 该格**不再是候选缺陷**，`README` 已就地改口径。

**B —— 无参 `read_shadow()` 的索引预算**。`max_tokens` 此前**只作用于带 `topic` 的路径**，而无参路径（agent 的**第一跳**）
把 `_index.md` **整篇原样**返回 —— 按上面的实测那是 **2.2 MB** 的返回值，而同一个入口在带 `topic` 时既有预算又有截断信封
⇒ 属 `adr/0084` 那类判据分叉的变体：**同一处的一个分支忘了接预算**。修法：`max_tokens` 的解析与钳位**提到分支之前**
...
冻结 ADR 正文（按「归档不改写」留原样）。



## [v1.15.84] 发版：把「证据入口在仓库之外、不受版本控制」写进 `AGENTS.md`（上一个**无版本提交**并入本版）

`AGENTS.md` 的「证据的入口」原先**写死**指向 `../.docs/fix/2026-09-12/meta-review-four-rounds.md`。检查阶段实测：
**本机没有这个目录**（只有 `2026-09-14` 与 `2026-09-15`）—— 根因是 `../.docs` 在**仓库之外、不受版本控制**
⇒ **换机 / 新克隆必然断**（`CHANGELOG` / `adr/*` 里若干「对账探针」路径同理）。

- **改法：规则 + 命令，不再钉日期目录** —— 那一条改成「`../.docs/fix/<年-月-日>/` 下的入口文件」+ 找本机最近一份的命令：
  `Get-ChildItem ../.docs/fix -Directory | Sort-Object Name -Descending | Select-Object -First 1`。
- **顺手落一条判据**：**能复现的东西放 `tools/`**（进版本控制、进 `verify`；如 v1.15.83 的 `tools/contract-surface.selftest.ts`），
  `.docs` 只放**当时**的取证脚本 —— 否则复审者拿到的是一串**只有某台机器**才存在的路径。
...
用户 2026-09-15 先下「检查当前 dsh 插件设计」，给完四条发现后再下「fix 1 2 3 4」。本轮**只修这四条**，
每题都在本机实测过、命令可重放；**没碰**的东西照旧列在末尾（不借机扩范围）。

### 1. 给两族契约补「名字面」的门（`tool-schema-v1` / `config-keys-v1`）

此前这两族的 `verification` 一直是「参数名无人枚举」「没有一处枚举键名」⇒ **删参数 / 改名 / 删键不会有任何东西变红**
（`README` 的受保护契约面自己写着这条，`adr/0086` §8.6 的结论也是「八族里有两族其实没人守」）。本轮按「判据收一处」补上：

| 层 | 落点 | 语义 |
|---|---|---|
| 判据 | `tools/contract-surface.lib.ts` | `diffSurface`（`missing` = 改名/删除 ⇒ 禁；`added` = 新增 ⇒ 允许）· `extractConfigKeys`（按**大括号深度**抽顶层键） |
...
⚠ `audit:docs` 的检查 ③ **只管** `README` 默认开关表的行数 ⇒ **抓不到这一处** —— 它是人工读到才发现的，此处留档。

### 3. 评测门的语料根：由「只往上推一层」改成**候选三级**

`tools/retrieval-eval.ts` 的默认语料根（`CANDIDATE_ROOTS`）：`<repo>` / `<repo>/..` / `<repo>/../..`，取**第一个存在 `.shadow`** 的。

- 修前实测：本仓布局是 `<工作区>/vendor/dsh-shadow` ⇒ 推导落到 `<工作区>/vendor`（**没有 `.shadow`**）⇒ `npm run verify` 第 5 步 **exit 2**，
  **后 3 步（`audit:ratchet` / `tsc --noEmit` / `test:all`）根本不跑**；而 `README` 的评测门表当时把它标成「✅ **恒可用**」（**被实测证伪**）。
- 修后实测：**不带任何环境变量** ⇒ `基线完整性检查：通过 ✅`（exit 0）；把 `SHADOW_EVAL_ROOT` 指向不存在的根 ⇒ 仍 **exit 2**，
...
- `AGENTS.md` 记下两条**判真假**的命令，避免下次有人把假脏当真漂移、或反过来把真漂移埋进噪声里。
...
两件都做了，**并且各带一条自我更正**（下面 §1 与 §2 各一条）。

### 1. 门：`audit:docs` 新增检查 ⑤（`tools/docs-consistency.ts` 的 `checkVersionTags`）

- **判据**：`CHANGELOG.md` 里所有 `## [vX.Y.Z]` 中，**≥ 仪式起点（`1.15.77`）且版本号 < 当前版本**的每一条，
  必须存在同名 tag；缺一个就红（退出码 1）。
- **为什么要有它**：tag 这半边连续两次是**用户提醒驱动**的（`v1.15.77`/`78` 属补打，`v1.15.80` 与 `v1.15.81`
  都由用户先说「tag」）。按 `AGENTS.md` 自己那句「凡『每次发版都要做一次』的动作，**要么写进清单、要么配一道门**；
  靠记性 = 迟早停」—— 同族的前两条门（①三方版本一致、④README 抄 `CHANGELOG`）都是**先有门才停住**的。
- ⚠ **落地时更正了我自己上一轮的原话（本条最要紧）**：我上一轮写的是「版本已变、但同名 tag 不存在」——
...
- **实现选择：刻意不 spawn `git`** —— 用**读 ref 文件**（`.git/refs/tags/**` 的松散 ref + `.git/packed-refs`，
  ...
  **读不到 `.git` ⇒ 报结构缺失(2)、不是「通过」**（ADR-0049 缺件不静默）。
- **它刻意不做的两件事**：**不查远端**有没有这个 tag（门里不联网）、**不查** tag 指向哪个提交（那要读对象库）
  ⇒ 发版时那句 `git rev-parse --short vX.Y.Z` 仍是**人工核对**的一步。
...
## [v1.15.81] 登记新资料 `Vincentwei1021/video-shotcraft`（`references.md` §13）+ 发版 tag

用户 2026-09-15 指令「tag」。按 `AGENTS.md`「发版 tag 约定」（名字 = `v<package.json 的 version>`、**轻量 tag**、精确推送）先核实：
`v1.15.80` 已在远端并指向**上一提交** `53045e0` ⇒ 本轮**唯一的新东西就是那条资料登记**，它落在纯文档提交 `90599bd` 里。
**本条目不重复登记内容**（是什么 / 读数 / 机制 / 可借鉴四条 / 与 dsh-shadow 的关系 / 未核实边界，全文见 `references.md` §13），
只记「发版这一半」与本轮新核出来的两件事。

### 1. 本版做的两件事

| # | 动作 | 落点 |
|---|---|---|
| 1 | 资料登记（**已在上一提交**） | `references.md` §13 + 清单指引行 §7–§9 → §7–§13（提交 `90599bd`，纯文档 +84/−1） |
| 2 | **为它发版并打 tag**（本版新增） | 三处版本号 `1.15.80 → 1.15.81`（`package.json` / 本文件首条 / `README` 当前版本行）+ 轻量 tag `v1.15.81` |

**为什么不是「把 `v1.15.80` 挪到新提交上」**：约定写的是 tag 指向**该版本的发布提交**；挪一个已推送的 tag 要 force push，
且会让「tag ⟺ 版本」这条对账直接失效。⇒ 本仓口径是**新增一版**（与 v1.15.80 那次同款：用户说「tag」，
落地成「补上 tag + 一版」）。

### 2. 顺带核出：那条核对口径**在全历史上不成立**（口径写窄了，不是新缺陷）
...
- 处置：**不追溯改建历史 tag**（与 v1.15.78「`v1.15.4`…`v1.15.76` 的缺口不补」同一个决定），
  ...
  改它属另一次文档改动（且要连带想清楚门怎么跟着走），不在本版范围。

### 3. 一条观察：tag 这半边目前是**用户提醒驱动**的

`v1.15.77` / `78` 是补打，`v1.15.80` 与**本轮**都由用户先说「tag」才发生。而 `AGENTS.md` 自己写着：
**凡「每次发版都要做一次」的动作，要么写进清单、要么配一道门；靠记性 = 迟早停**。
⇒ 按这条，下一个可做的动作是**把「该打的 tag 打了没有」变成可机械判定的门**。
难点已想清：HEAD 永远可能是下一版的起点，所以门**不能**问「HEAD 有没有 tag」，
只能问**「`package.json` 的 version 已变、且 `v<version>` 的 tag 不存在」**这类组合（即「版本已发布但没打 tag」）。
**本轮未做**，只登记这个判断（避免又变成一句没人执行的建议）。
...
- **安装副本未同步**：WSL `~/.dsh/profiles/web` 里用 SHA 固定的 `dsh-shadow` 仍指旧提交 ⇒ 它的 `package.json`
  读数仍是 `1.15.80` 而仓库已是 `1.15.81`（`dist` 逐字节相同 ⇒ **行为零变化**）。要不要重新 `pnpm install` 由用户定。
- **`eval:retrieval:check` 在本机需 `SHADOW_EVAL_ROOT=D:\project\dsh1`**：不设时它按工具文件位置推导出
  `vendor\.shadow`（本机 checkout 在 `vendor/dsh-shadow` 下，该目录不存在）而退出码 2。
  **既有布局环境问题、非本版引入**（已单独复现：设 env 即通过），本轮只记明。
- 资料本身**未克隆、未安装、未渲染**；能否以 `~/.agents/skills/` 或 DSH 预设形态装载**未验证**（见 `references.md` §13）。
...
**拉取 + 学习 + 判定**，**实装按待办走**（`D9` / `D10` / `D11`）。**纯文档：无代码 / 配置 / 行为改动。**
...
- **`langextract` 本机原有副本是「无 `.git` 的 v1.6.0 快照」** —— 落后一个小版本且**不是克隆**。
  按「可逆改动先备份」改名为 `langextract--snapshot-v1.6.0` 后重拉正规克隆。
- 首次克隆被代理掐断（`early EOF`），按「网络受限换可达通道」降到 `--depth 1` 通过。

### 2. 学习（两路独立只读深读，**未安装、未运行**）

要求：每条结论带 `文件:行号`、**禁止把 README 宣传当事实**、按「一旦错了就得返工的 / 其余的」两栏交付。
本机**无 Rust 工具链**，langextract 未 `pip install`（会出网且撞 ADR-0049 的审批边界）
⇒ **结论止于源码阅读，行为未经运行验证**。

### 3. ⚠ 本轮最值钱的单点结论：**一处对自家文档的更正**

`references.md` §8 此前只写了 langextract「自动检测：定位不到原文的抽取项其 `char_interval = None`」，
**读起来像一道门**。**读源码后推翻**：
...
- `langextract/resolver.py:1021-1038` 收 `unaligned_extractions` **只为跑一次 fuzzy 重试**，
  **之后无计数、无 warn、无 raise**；`align_extractions` 在 `:1073` 直接 `return`。
  （父代理**独立复核**：全文件仅两处 `logging.warning`，`:311`/`:319`，**都是解析错误**，与对齐失败无关。）
- 唯一的硬断言 `tests/test_live_api.py:279-302`，而默认门 `tox.ini:25` 是
  `pytest -m "not live_api and not requires_pip and not integration"` ⇒ **被排除**。

⇒ **它不是「有门的先例」，它恰好是 ADR-0049 要防的形态：降级了，但不可见。**
这是 `adr/0078` D1 更正① 那条纪律的**第二次**生效（第一次是 hl_mem 的 `assert_transition()`）。
...
1. **不允许建没有消费方的机制** —— 本仓已八次记录「**机制存在、没接线**」这一族缺陷
   （`audit:wiring` 的 A/B 两类就是为它造的）。langextract 的全部可吸收项在本仓今日都没有消费方。
2. **每一条可实装的都带一个「必须先拍板的前置」**：甲-1 要先定**恢复存储策略**（不许 rusqlite）、
   甲-2 要先定**中文 token 口径**（rtk 用字节/4，中文偏差方向相反）、甲-3 会动**已签入的离线棘轮**。
   **静默替用户选一个就是越权** —— 已被写成 `D9`/`D10`/`D11` 三条待办，各带完成判据。
3. **`adr/0065` 的先例成立**：只做「对标 + 勘误 + 待办」的 ADR 在本仓是正当的，**且不实装是有意的**。

### 文档改动（本次全部改动）

| 文件 | 改动 |
|---|---|
| `adr/0087-absorb-rtk-langextract-verdicts.md` | **新增**。沿用 `adr/0065` 体例；含 A（rtk 判定）/ B（langextract 判定）/ C（**为什么不实装**）三节 |
| `references.md` | **新增 §10–§12**：两仓一手事实（写死 `HEAD`）+ **对 §7/§8 的更正** + 结论摘要 |
| `BACKLOG.md` | **新增 D9 / D10 / D11**（四项格式齐备，各带完成判据）；**未写复合计数**（表头已改成「判据 + 命令」口径） |
| `CHANGELOG.md` / `package.json` / `README.md` | 本条目 + 版本号三处一起改 `1.15.78` → `1.15.79` |
...
- **两仓均未安装、未运行**（本机无 Rust 工具链；langextract 未 `pip install`）⇒ 所有结论**止于源码阅读**。
- **甲-2 的中文 token 偏差是深读者的推断**（编码事实 + 源码事实推出），**rtk 源码里没有任何 CJK 口径讨论**。
- 两路专家的「未核实清单」已并入 `adr/0087`「负 / 已知边界」，**含专家自己纠正的一处初判**
  与原报告被截断后补发的部分 —— 未核实项**如实登记，不靠猜补齐**。
...
⇒ **判据（已写进 `AGENTS.md`）**：凡「**每次发版都要做一次**」的动作，
**要么写进那份清单，要么配一道门**；**靠记性 = 迟早停**。

### 3. 为什么不给 tag 配门（说明这个「没做」不是漏了）

检查① 能配门，是因为它**纯读文件**（`package.json` / `README` / `CHANGELOG` 三方比对）。
而「有没有 tag」要**读 git 引用**，`tools/docs-consistency.ts` 目前是**纯文件**判据（不碰 git）。
把 git 引入那道门会**扩大它的职责面**（还得处理：浅克隆、无 remote、detached HEAD、CI 只 checkout 一个 commit）。
⇒ **本轮只写成文**，并**明确记下「配门」是一个可选的后续**（而不是忘了）。
...
**这不是「数错了」，是这类数根本不该这么写** —— 本项目里**第一个「写下来就必然错」的数**。
...
- `stability` = **`soft`**（骨架/措辞可改，须写 `CHANGELOG`）/ **`hard`**（**「不静默丢内容」**：截断必须自报）
- `verification` = **强**：`test/recall-envelope.test.ts`（逐字断言信封四要素）+ `test/recall-attribution.test.ts:440`

### 4. A 段 #3/#4/#5 —— **现在可以判了**（这就是补齐它的全部意义）

| # | 判定 | 依据（机械：查 `tool-output-v1` 的两半边） |
|---|---|---|
| 3 | `renderIdentityModel` **允许接线** | 改的是 `mode:"identity-advance"` 的返回内容 ⇒ `soft` 半边 ⇒ 允许，**须写 `CHANGELOG`**，且不得让原有内容静默消失 |
| 4 | `progressiveDisclosure` 进默认读路径 **允许接线** | 同上 ⇒ `soft`：允许，须写 `CHANGELOG`；**但它会折叠内容 ⇒ 必须由信封披露被折叠的部分**，否则违反 `hard` 半边 |
| 5 | `refineTree` | 与 #4 同一处决定，同一判定 |

⇒ **判据不再是我的意见，是登记册里那两行。**
⚠ **判定 ≠ 已实施**：本轮**只给判定**，没有接线这三处（接线要写 `CHANGELOG` 并摊开折叠披露，属独立任务）。
...
| # | 实测（代码自带说明） | 结论 |
|---|---|---|
| 3 | `identity/timeline.ts:68` 写明 `mode:"identity-advance"` 会**写** `IdentityModel`；`renderIdentityModel` 生产**无调用点**（A1 桶，测试引用 0） | **接线它 = 改某 `mode` 的返回内容** |
...
v1.15.75 加了检查 ④，于是 `AGENTS.md` 写着「查**三条**」、`tools/docs-consistency.ts` 文件头写着「**两条**检查」——
真值是 **4**，而**没有任何门在守它**（检查 ② 守的是 `verify` 步骤，不是本工具的检查数）。
...
| 次 | 版本 | 形态 |
|---|---|---|
| 1 | v1.15.66 §5 | 整表复制 `adr/0085` §8.6 |
| 2 | v1.15.72 | `README` 版本行整段复制 `CHANGELOG` 条目 |
| 3 | v1.15.74 | **又**整段复制（那一行 **881 字**，v1.15.73 那行 **1222 字**） |
| 4 | 本轮 | 又写了一次 ⇒ **拦住** |

`AGENTS.md` 的文档归属表早写了「同一条事实出现在多层时，**每层只回答它自己那个问题**」，**但它只是散文**。
⇒ 按本仓自己的先例（检查 ②③ 都诞生于「同一处手工纪律反复失效」）：**反复失效的手工纪律要变成门。**

**新增 `audit:docs` 检查 ④**：`README` 版本历史表里**当前版本那一行**，与 `CHANGELOG` 同名条目之间
**不得有 ≥40 字的连续逐字重复**（阈值 40：版本号与条目标题这类必然重合的短片段要放行）。
**只查当前版本那一行** —— 历史行是**归档叙述**（v1.15.71 起按「归档不改写」保留），
拿新门去要求重写历史行等于用门改写归档。
...
`BACKLOG` 要求 `| Module | Owns | Reads | Writes | Must not own |`，而 v1.15.73 的探针给的是
`Module / Reads / 入边 / Must not own / 纯模块 / 副作用信号` —— **既没有 `Owns` 也没有 `Writes` 列**。
⇒ 已改为**正好那 5 列**（`Owns` 未核的层写「未核」、`Writes` 指向两条契约），
「入边 / 纯模块占比」挪到**第二张辅助表**，不挤进要求的形状。

### 3. `D1`：**保留** —— 理由是「**现时的验证价值**」，不是「将来可能有用」

`BACKLOG` D1 判据：「若 `ChangeSet` **没有生产消费者 / 没有验证价值 / 没有外部契约** ⇒ **删**」。
...
删它会**打掉一条检测器的已知答案**，把那条棘轮变弱。（`adr/0062` 早已确认它「未接线但非缺陷」，与此判定一致。）

### 4. 纠正我自己的夸大：「`T15` 让 `D1/D2/D3` 机械判定」

`D1` 的三个条件里**前两条可机械判**（且都成立），**唯一决定性的第三条「有没有验证价值」是主观的** ——
**而它恰好推翻了前两条导向的结论**。
⇒ **诚实表述**：`T15` 让 `D1` 的**证据收集**变机械（该查谁、查到什么），**没有**让**判定**变机械。
这条留在册，免得下一个人以为可以照条件表自动删。
...
v1.15.72 在 `adr/0086` §8.6 写：不给 `config-keys-v1` 加门，是因为「删键是否真的危险
**还没有生产证据**（**没有生产消费者**，见 §7）」。

**这句把两件事混了**：

    index.ts:44   export function apply(ctx: CtxLike, rawConfig: ShadowConfig = {}) {
    index.ts:46-50  // 配置读取：测试直接传 rawConfig；live 走 Cordis 的 ctx.config（插件行 config，经 cordis.patch.yml 注入）

§7 说的「没有生产消费者」指**这份登记册**没人读；而**配置键恰恰是被消费的** ——
用户在配置里写下的键名**就是外部契约**，删掉它会让那份配置**静默失效**。

⇒ 正确表述：**不加门是因为本轮的范围是「填完登记册」而不是「扩工具」**；
**危险本身是有证据的，只是还没被测试固定下来**。这两句差别很大 —— 前者等于说「危险不存在」。

### 2. 重复（两处，都是我自己立过规矩的那一种）
...
- 它 **直接 import `tools/audit-layers.lib.ts`** 的 `DIRECTION_RULES` / `FORBIDDEN_TARGETS_EVERYWHERE` / `PURE_MODULES`
  —— **判据收一处，绝不写第二份禁向表**；
- 再用**真实 import 图**算出每个模块的实测出边/入边，把「它实际在读谁」与「它不许读谁」并排。
- 五列里**只有 `Reads` 与 `Must not own` 可机械推**；`Owns`（语义）与 `Writes`（副作用）**推不出来** ⇒
...
### 4. 遗漏：**`D1` 的再框定**从「`T15` 之后」的清单里丢了

`BACKLOG` T15 涉及 **D1 / D2 / D3 三条**，而 v1.15.72 的「未做」只写了 **D2 / D3**。
`D1` = 「若 `ChangeSet` **没有生产消费者 / 没有验证价值 / 没有外部契约** ⇒ **删**」。已补回 `README` / `BACKLOG` / `adr/0086` §6。

### 5. 干扰：`adr/0086` 的节序错了（**第三次同病**），以及一处轮次编号错误

| 症状 | 处置 |
|---|---|
| 文件头顺序是 `§8 → §8.6 → §8.7 → §8.5` —— **`§8.5` 排到了 `§8.6` 之后** | 移到数值位置。**与第一轮记的 `### 6.11`、v1.15.69 记的 `adr/0085` `§8.7/§8.8` 是同一个病** |
| `§8.6` 标题把「**`T15` 切片 2**（v1.15.72）」称作「**第八轮**」 | 改为「切片 2（v1.15.72）」。**「轮次」与「切片」是两种计数**，混用后「第几轮审了谁」无从复核（第八轮元审查是 v1.15.73） |

**根因**：我每次都在**文件末尾追加**新小节（最省事），而**小节号是语义顺序、追加位置是物理顺序**。
...
本轮目标是**把登记册填完**，不是扩工具。加门就要维护它，而「删键是否真的危险」**还没有生产证据**
（这份登记册**没有生产消费者**）。⇒ 先**如实登记为未设防**，等真有删键需求时再决定 —— 这比先造一道门更省。
...
## [v1.15.71] **第七轮元审查：把「62」的因果正过来**（受门保护的数不该被没有门的探针推翻 · 面族数 6→7）（`adr/0086` §8.5）—— `verify` **56/56**

**一句话**：同一条指令第七次。本轮审 `32573b6`（v1.15.70）本身，发现**我上一轮把「有门保护的数」和「没有门的数」的待遇弄反了**，
并顺手改正一处「数字与列表互相矛盾」。

> **勘误指向**：v1.15.70 的条目与 `README` 版本历史表里那句「`mode` 面 **实测 61**」「`CONTEXT.md` 的 62 来源不可考」
> **按「归档不改写」保留原样**（那是版本当时的状态）；**正确的数与因果以本条目为准**。
...
**61 才是没有门保护、由我的探针现算出来的数**。我把两者的待遇**弄反**了：
拿一个没有门的数去推翻一个有门的数，并给后者贴上「来源不可考」。

### 2. 根因（机械可复现，不靠推理）：探针把判等形态写死成 `===`

    query/planning.ts:15   if (String(args?.mode || "") !== "plan") return undefined;

本仓有**一处**用 `!==` 判 mode，而我的探针两条判等形态都写死 `===` ⇒ 漏掉 `plan` ⇒ 61。
判据改成 `[!=]==` 后即为 **62**，与门一致。

**新对账探针（可重放）**：`../.docs/fix/2026-09-12/t15-mode-count-reconcile.ts`
（A=门抽法 **62** / B=我的原样抽法 **61** / B′=只修 `===`→`[!=]==` **62**，**B′==A**）。

**探针顺带否掉了我自己的第一版解释**：我原以为差在「B 排除了 `.selftest.ts`」——
实测 `query/` 下 **0** 个 `.selftest.ts`，该解释**当场不成立**。探针把它写成一条显式的反例分支，
而不是让我把这个错解释写进文档（这正是 v1.15.69「先看一个具体实例再下结论」的延伸：
**这次要看的是「被测量那一侧的写法」，不只是「探针自己的实现」**）。
...
| 类型 | 例子 | 有人要改它时怎么判 |
|---|---|---|
| **受门保护的数** | `mode` 总数 **62**（`test/recall-envelope.test.ts:96` 断言） | **先假定它对**。要改必须**同时改门**并说明门为什么错 —— **不许拿一个探针去推翻它** |
| **没有门的数** | 探针/脚本现算出来的任何计数 | 改它不必连带改别处，但**写进文档必须带口径与命令** |

⇒ **「来源不可考」这句话本身要有证据**：说它之前先 `grep` 这个数在仓库里是否已被某道门断言。
⇒ 报「实测」时，**探针的实现**与**被测量那一侧的写法**都要看过。

### 5. 未做 / 诚实边界

- 本轮**只**纠正这一处因果与计数：`CONTEXT.md`（恢复为「62，且有门」）· `README` 面清单那一行 ·
  `adr/0086` §6/§8.1/§8.2/§8.3/§8.4 + 新增 §8.5 · `AGENTS.md` 一条。
...
## [v1.15.70] **第六轮元审查 + `T15` 切片 1 补完**（mode 面 **61** 可复核 / 工具 schema 面 / 参数级废止）（`adr/0086` §8）—— `verify` **56/56**
...
⇒ 就地改写 `CONTEXT.md`：保留 62 的历史记录，**写明实测 61 与差 1**，指向探针与 `adr/0086`。
...
- **十字段未填**（同上）；**这份登记册仍无生产消费者**（是给将来的人与 agent 读的政策）。
- **探针的边界已写在它自己的输出里**：它认不出 `switch` 或其他判等形态 ⇒ **61 是「按该探针」的数，不是神谕**。
- **`CONTEXT.md` 的 62 仍留在文里**（作为历史记录）—— 若将来有人能考证出它的来历，应补进 `adr/0086` §8。
## [v1.15.69] **第五轮元审查 + `T15` 切片 1**（受保护契约面 / 稳定性分档 / 最小弃用流程，`adr/0086`）—— `verify` **56/56**

**一句话**：同一条指令第五次下达。这一轮做了两件事：**① 审 `99cb55b`（v1.15.68）本身**，
查出**我在「修计数腐烂」的同一段里又写了一个错的计数**；**② 不再只报告「T15 未开工」，而是开始做它** ——
交付受保护契约面清单、稳定性分档口径与最小弃用流程（`adr/0086` + `README.md` 新节）。
...
### 2. 干扰：`adr/0085` 的节序错了 —— **与第一轮我记为缺陷的那个错同型**
...
前四轮的产物散在 `CHANGELOG` 四条 + `adr/0085` 三节 + `AGENTS.md` 五段，
而**没有任何地方指向那份证据文档**（`meta-review-four-rounds.md`）—— 复审者看不到「怎么重放」。
⇒ 在 `AGENTS.md`「脚本一律 TypeScript」节**显式写出证据入口**，并要求**每轮审查结束时更新它**。

### 4. `T15` 切片 1（**这才是本轮的主要产物**）

用户 T15 判据三条，本轮完成 ②③、部分完成 ①：
...
- **T15 的十字段未填**（`allowed`/`forbidden changes` / `verification` / `ratchet` 三列基本空缺）——
  按用户规则**这不等于登记册完成**。
- **`mode` 全量枚举未完成**（12 显式 + 13 模块内联分派）。
- **`adr/0085` 的标题仍只覆盖前半**（改名会打断引用，故只加显式标注）。
- **这份登记册当前没有生产消费者**（没有代码读它）—— 它是**给将来的人与 agent 读的政策**，
  必须现在说清楚，否则它会变成本仓清理过的那类「写好了但从不执行」的东西。
- **元审查已第五轮**：我在 §4 里开始做 T15 而不是继续只审查 —— 这是本轮与前三轮最大的不同。
## [v1.15.68] **元审查（第四轮）：审 v1.15.67 本身** —— 同一句立法**第二次只落一半** + `audit:docs` 从两条扩到**三条** + 一条**关于测量的教训**（`adr/0085` §8.8）—— `verify` **56/56**

**一句话**：同一条指令第四次下达。这一轮审 `d92d6f3`（v1.15.67）本身 ——
查出我**同一句立法第二次只落了一半**（只给链路清单配了门，**计数**仍只是规则），
以及**门自己有三处缺陷**，其中一处差点让我**把对的文档改错**。

### 1. 遗漏：同一句立法，第二次只落一半

v1.15.67 的立法是「**版本号 / 计数 / 链路清单** —— 要么别手写，要么配一道门」，
而它**只给链路清单配了门**，**计数**那半仍停在规则。当天实测两处：
...
| # | 缺陷 | 修法 |
|---|---|---|
| 1 | **检查②是单向的**：只查「`verify` 的每一步是否被文档点名」。**门被从 `verify` 里摘掉**时 `required` 少一项 ⇒ 检查反而更宽松、照样全绿，而文档里那行成了**死指针**（历史同族：v1.15.22 的「三方版本一致」仪式停掉后无人发现） | 检查②改**双向**：文档点名的每个 `npm run` 也必须在 `verify` 里（selftest ⑫/⑫b） |
| 2 | **反向检查第一版假阳性**：把块头的 `npm run verify` 自身、以及内联的 `= npm run build && node tools/run-tests.ts` 都当成闸门 | 只看 `^[+=]\s*npm run X`（块内**逐条列举**的行）；⑫b 组专锁 |
| 3 | **检查③报假警报** | 见第 3 节 |
...
`AGENTS.md` 被我在前两轮堆成了「事故叙述 + 规矩」混排，且与 `adr/0085` §8.7 逐字重复
（我**刚刚**立过「同一件事只写一处」）。现按分工收口：**`AGENTS.md` 只留规矩与指向**
（「事故经过见 `adr/0085` §8.7/§8.8」），数据与叙事留在 ADR。
...
- **检查③只覆盖 `README` 那一处计数**（结构稳定、可机械化）。`BACKLOG` 的复合计数改用「判据 + 命令」，
  **D / V / G 的条数本轮未复核** —— 明确写出来，不抄旧数。
- **没有做全仓「计数字段」普查**：只处理了已知的两处。
- **① ② ③ 都只是必要条件**：答「有没有 / 一致不一致」，答不了「顺序与描述对不对」。
- **T15 仍未开工**：`adr/0083` §14.1 记的路线漂移（`T8 → T15 → …`）在本轮之后**依然存在** ——
  这四轮元审查本身也是「线索越查越多」的一种表现，我把它记在这里而不是假装没有。
## [v1.15.67] **元审查（第三轮）：审 v1.15.66 本身** —— 一处**我自己说错的因果** + 一处**差点丢掉 34 个版本条目** + 文档派生字段门扩成两条（`adr/0085` §8.7）—— `verify` **56/56**
...
v1.15.66 §7 的原话是：「那是**人工核验过一次**，不是门」。我据此造了一个叙事：
...
**这比原来那个故事更值得警惕** —— 习惯会在无人察觉时失效；`v1.15.22` 那条记录本身就是
「它曾经是发版仪式的一步」的证据，我却把它读成了「只做过一次」。
**教训**：**看到一条历史记录，先问它是「一次性事件」还是「某个仪式的一次快照」。**
（数字也顺带更正：66−40=26 是**版本差**，**漏掉的发版数**是 25 —— 数字要带**范围**才可复核。）

### 2. 遗漏：门只覆盖了半类问题

`audit:version`（v1.15.66 建的）只管**版本号**。但同一轮我**刚立下**的一般化规则是
「任何能由其他数据推出来的字段 —— 版本号 / **计数** / **链路清单** —— 要么别手写，要么配一道门」，
而我**只给版本号配了门**。实测代价（本轮的 `audit:docs` 一条命令就抓出来）：
...
三处文档的**组成**补齐、**条数一律删掉**（改为「以 `verify` 输出为准」）。
selftest 从 5 组扩到 **11 组**，新增的三组都是被实测逼出来的：

- **⑥ 诱饵**：正文里提一句「当前版本：」不得抢走匹配（旧判据用「全篇第一处」）。
- **⑪ 假绿对照**：把一个步骤名放到**闸门块之外**（比如版本历史表里）必须**仍然红** ——
  第一版判据是「名字在这份文档里**出现过**」，而 `typecheck:tests` **恰好**在版本历史表的一行里，
  于是**闸门块明明没列它、检查照样过**。
...
**根因不是「手滑」，是三类操作习惯缺失**（已逐条写进 `AGENTS.md`）：
...
（`--update-ratchet` 打印桶、`run-tests` 打印条数、`audit:docs` 打印链路），
**我却一直没给自己做文档改写时配这样一道门。**

### 4. 重复内容：一处**该指向**而非复制的表

v1.15.66 §5 把「过期行号分层表」整表贴在 CHANGELOG 里，与 `adr/0085` §8.6 逐字重复。
⇒ 按本仓分工（ADR 拥有判据与表 / CHANGELOG 拥有历史归档的**数字**），
**新增的 `adr/0085` §8.7 记录本轮增量**，本条目只留**数字**与**指向**，不再复制表。
（v1.15.66 的原文按「归档不改写」保留不动。）
...
`CONTEXT.md`（冒烟门格的组成补 4 道闸、删手抄条数）· `adr/0085`（§8.5 勘误 + §8.7 本轮增量）· `dist/**`。

### 6. 未做 / 诚实边界

- **`audit:docs` 第二条是必要条件而非充分条件**：它答「**有没有点名**」，答不了「顺序 / 描述对不对」
  —— 那种只能靠人读。**不得据它宣称文档正确**（已写进 `AGENTS.md` 与工具注释）。
- **`README` 版本历史表里的旧行号仍留着**（归档层，按 `adr/0085` §8.6 的分层**故意不动**）。
- **未对全仓做一次「计数字段」普查**：本轮只修了 `verify` 链路的计数与组成；
  其他手写计数（如各表行数、台账条数）**未逐个核对**，只立了规则。
- **`v1.15.66` 条目内的两处错误按分层不作就地改写**，只在本条开头写勘误 —— 若认为应在原条目内加指针，
  那是一处**取舍**（本仓尚无明确规定），我选了「归档只追加勘误、不改正文」。
## [v1.15.66] **元审查（第二轮）：审我自己 v1.15.64/65 的产出** —— 1 处漏项 + 1 处过度声明 + 1 处统计口径 + README 表把已修项写成现状（`adr/0085` §8.5/§8.6）—— `verify` **56/56**

**一句话**：用户在 T8 收口后又下了一次同一条元审查指令。这一轮审的**不是代码，也不只是文档** —— 而是**我上一轮刚做完的 T8-B/T8-A 本身**。查出四类问题，其中一类是**真漏项**（漏了一个 `catch`），一类是**我自己写的因果说错了**。
...
我曾在 `adr/0085` §5 与 `core/projection-store.ts` 写：「同一份 ADR 的自检清单要求**每一个**可选增强都有可见信号，表里却给了豁免 ⇒ **ADR 内部张力**」。
...
| 层 | 处数 | 处置 |
|---|---|---|
| 归档文档层（`CHANGELOG` 34 · 已冻结 ADR 正文 28 · `adr/0083` 审查快照 6） | **68** | **一处不改** —— 行号是「当时/发现时」的，改写 = **伪造历史** |
| 当前态文档内的归档区间（`README` 版本历史表 · `BACKLOG` §〇 已结案 / §六§七 审查线索） | **34** | **一处不改** —— **只按文档分层不够，必须精确到行区间** |
| 当前态 + 受本次改动影响 | **14** | 改写成**不依赖行号**的形态 |
| 当前态 + 未受影响 | **10** | 不动（引用仍正确，改了反而丢信息） |
| 权威表格（`adr/0084` 缺陷表 / `adr/0085` 自身引用） | **10** | **手工**改成**符号引用** |
| `AGENTS.md` | 2 | **故意保留**（是「过期行号」的举例） |
...
判据是三方**逐字相等**（`package.json.version` / README 第一处「当前版本」行 / `CHANGELOG` 首个 `## [vX.Y.Z]`），
退出码 `0` 一致 / `1` 不一致 / `2` 结构缺失。配套 `tools/version-consistency.selftest.ts`（**spawn 真 CLI** 的标定，
5 组：一致放行 + README 落后 + 两种结构缺失 + **归档层隔离**）。
**只查 README 那一行、不查全篇最大版本号** —— README 底部的版本历史表本来就该包含全部历史，
拿「全篇最大」当判据会在归档与当前态之间来回误报（本轮的过期行号清扫刚吃过「只按文档分层不够」的亏）。
...
用户 2026-09-12 定的泳道是 `T8 → T15 → D1/D2/D3 → A段 → T2(B段) → T9/T10/T7 → T14 → V/G/T6`（并行 Memory Track `P1 → M1 → …`）。
我在 M1 之后连续 **8 轮**做「审查 → 修」，**T8 / T15 / D1-3 / A段 / T2 一项未动** ——
而「review fix all」每一轮都能自洽地生出下一轮（线索越查越多）⇒ **这是一种会自我延续、不会自己停下来的活动**。
**我此前从未把这件事写下来** —— 那才是最大的遗漏。已记入 `adr/0083` §14.1 并置于最高优先。

### 2. 因果：把「静态推演」当成了「当前生效」

§六/§七 表格的「后果」列写法上是**断言**，但相当一部分来自只读审查的**静态推演**（报告者明确未跑测试）；
此前只在报告结尾整体标注可信度，**没有逐行区分**。⇒ §7.0 表下加**统一口径**（标「已修」的以 §12 为准，其余为静态推演）。
另**更正我自己的两处措辞**：⑪ 的同义反复应说「改**分桶判据**照样全绿」（不是「改成任何东西」）。
...
最容易的作弊方式是把断言改宽松、加 `@ts-ignore`、或删测试块。故：
...
与 `{ … })` 的空格差异没归一化 ⇒ 又一批假报。⇒ 改为**按括号深度消费 cast**。**先修探针，再信结论。**

### 2. 顺带修掉一处**生产侧**类型缺口（探针顺带发现的）

`core/knowledge-retrieval.ts:60` 的声明比**真实契约窄**：实现读 `(h as any).__path`，而生产调用方
`query/reads.ts:162,167` 确实构造并传入 `__path`。生产没被 `tsc` 拦，是因为 `.map` 回调返回值**没有上下文类型**
⇒ 不触发 excess property 检查。⇒ 声明补 `__path?: string`、去掉 `as any`。
**教训**：声明写窄了，类型检查会反过来逼调用方去 cast —— 那是把缺口搬到下游。

### 3. 一处**为兼容而保留的旧标签**（诚实标注）

`world/explain/explain.ts` 的 `Validation History:` 前缀**没有改名**：`test/recall-attribution.test.ts:3230`
...
- **`retrieval-eval` 的第二份语料判据**（内联块、**零标定**，旧 footer 还误称它走 `classifyCorpus`）⇒ 抽成 `corpusFloorVerdict` + selftest **⑬** 锁边界。
  **两份判据刻意保留**：一个是**绝对下限**（拦工作区指错），一个是**相对基线容许带**（拦工具坏了）—— 拦的是不同故障。
- **`isProductionPath` 分叉**（wiring 把 `tools/**` 当生产面、drift 排除）⇒ **不擅自统一**（会改基线口径），改为**锁住**：新增 ⑭ 断言「差异**恰好**是 `tools/`」，任何一侧改动即红，**逼出那个决定**。
