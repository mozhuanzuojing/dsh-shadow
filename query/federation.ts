...
export async function runFederation(deps: ShadowQueryDeps, args: any, ctx: FederationCtx): Promise<string | undefined> {
  ...
  if (mode === "federation-perspective") {
    const p = perspectiveOf({ observerId: String(args?.sourceObserverId || agent?.id || "unknown"), temporalReference: String(args?.temporalReference || ""), observationClaim: String(args?.obsClaim || ""), lens: args?.lens as string, visible: args?.visible || [], hidden: args?.hidden || [], observationConfidence: confidenceOfInput(args?.obsConfidence), validationConfidence: confidenceOfInput(args?.valConfidence) });
