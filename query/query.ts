...
import { createKnowledgeEngine, renderKnowledgeTree, buildCorpusTree, retrieveKnowledge, renderRetrieved, renderKnowledgeRetrieval, sectionPath, flattenSections } from "../core/knowledge-engine.js";
...
export async function runReadShadow(deps: ShadowQueryDeps, args: any, exec: any): Promise<string> {
  ...
  if (String(args?.mode) === "context") {
    ...
    memories = memories.filter((mm: any) => !isForgettable(mm.rel, metaC, forgetC) && !isCompacted(metaC, mm.rel));
    const parsed: any[] = [];
    for (const mm of memories) {
      const text = await readRel(fs, ws, mm.rel);
      if (!text) continue;
