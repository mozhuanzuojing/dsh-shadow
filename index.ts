      ...
      toolsService.register({
        ...
        parameters: {
          ...
          properties: {
            ...
            mode: { type: "string", description: "模式开关：reflection 时从历史 ObservationTrace 发现候选规律（旁支，非 Memory 查询）；配合 from/to 限定周期。agency-context/agency-select/agency-event 时是 Agency Boundary Kernel（可解释行动能力，非 Autonomous Agent）。delegation-context/delegation-check/delegation-event 时是 Delegated Execution Boundary。recall-forget/recall-event/recall-validation 时是 Recall Continuity（Access Transition，非 Reality Reconstruction；非 Memory Kernel）。否则按布尔参数分派。" },
