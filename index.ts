...
export function apply(ctx: CtxLike, rawConfig: ShadowConfig = {}) {
  ...
  // v1.20.0：基线由 0.1.5-rc.1 抬到 0.1.7-alpha.2；**v1.21.4 再抬到 0.1.7-rc.2**（ADR-0098 §6/§7）。抬升的**唯一**理由是
  ...
  const HOST_BASELINE = "0.1.7-rc.2"; // 与 package.json 的 engines.dsh 同步维护
  ...
  const verifyEvidence = (ref: GatewayEvidenceRef, ctx: any): Promise<EvidenceResult> => routeVerify(ref, ctx, config.evidenceProvider || "fs", config.evidenceProviders);
  ...
  const makeQueryDeps = (exec: any): ShadowQueryDeps => {
    ...
    return {
      ...
      derivedIndexClearDirty: (ws: string, rels: Iterable<string>) => collector.clearDerivedDirty(ws, rels),
