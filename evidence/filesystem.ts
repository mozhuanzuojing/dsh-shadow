...
export const fsExists = async (fs: any, ws: string, rel: string) => {
  if (!fs || !rel) return true; // 无法判定时视为存在，避免误伤
  ...
  try { await fs.readText(resolved); return true; } catch { /* 读不出：可能是目录，或真不存在 */ }
  ...
  try { await fs.listDir(resolved); return true; } catch { return false; }
};
