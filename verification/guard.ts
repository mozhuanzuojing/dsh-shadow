...
export const assertRunHasNoEvaluationField = (run: any) => ({ ok: runHasNoEvaluationField(run), reason: runHasNoEvaluationField(run) ? undefined : "Verification 只读只报：禁 confidence/trust/score/quality/health/adaptationRef/permissionChange/identityChange（237/238/239）" });
...
export const assertEvidenceNoOptimization = (evidence: string[]) => ({ ok: evidenceNoOptimization(evidence), reason: evidenceNoOptimization(evidence) ? undefined : "Verification ≠ Optimization（Verification→Adaptation/Permission/Identity 禁；禁自我优化/调整自己）" });
...
export const assertReportNoRealityClaim = (report: any) => ({ ok: reportNoRealityClaim(report), reason: reportNoRealityClaim(report) ? undefined : "Drift Report ≠ Reality Claim（验证报告不得成为 RealityClaim/知识断言；只答有无违反边界）" });
