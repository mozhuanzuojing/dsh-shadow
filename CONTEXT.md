...
| 收口归档（v1.2.2，compact） | 对齐参考"会话级聚合+只留摘要+原始归档"：一个 episode 结束时把其 turn 原子**合并成 1 个 consolidated 文件**（保留决策/动作/材料/结果），个体原子 mark `compacted` 并移出活跃热集（文件保留可回放，Forget≠Delete）。活跃树"每 turn 一文件"→"每 episode 一 consolidated 文件"，热集文件数大降 |
