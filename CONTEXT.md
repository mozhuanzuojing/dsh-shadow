...
- **多粒度检索层的形态（ADR-0060，v1.15.16）**：**现有实现已经是「路由」**（`indexEngine.provider` 是**单值** `"fs"|"zg"|"semble"`，工厂只路由到一个），故「多库全量扇出」是**退步**而非加能力。实测（`tools/retrieval-eval.ts`，真 `.shadow` 语料 + 按 ADR-0054 实测性质建模的检索器 + 同候选预算 + 多种子）：**离题噪声** 有阈值单库 **0.000** vs 无阈值 **1.000**（扇出只是把噪声乘以库数）；扇出含互补来源**仍不升召回**（0.347 vs 0.358，对单·二元组仅 +0.002，落在 ±0.053 极差内）。**多粒度若做，形态是「单索引 + 层级表示（`level`/`parent_id`，只索引最细层，命中沿 parent 上取）+ 路由」**（学术等价物 RAPTOR / HeteRAG / UMG-RAG，**不要引「small-to-big 论文」——那没有原始论文**）；**加判别层（重排器）优先于加库**；多来源时**每个来源各自标定阈值**（实测阈值强弱因检索器而异）。**产品方向待用户裁决**，本 ADR 未单方面推翻任何既有 ADR。
...
- **`pinned` / `archived` 是「已文档化但无入口的能力」（ADR-0063，v1.15.22；T3 结案）**：生产只写 `pinned: false`（`core/memory.ts:74` / `core/writer-materialize.ts:88` / `query/query.ts:401`），**`pinned: true` 全仓零处**；`status: "archived"` 只见于读点与权重表。**不是「接线断了」**——① `_meta.json` 是 Derived Artifact（ADR-0003），手工改会被 `rebuild-index` 抹掉；② 没有任何命令/工具/元数据约定能置这两个状态；③ `MEMORY.md` 声明「不做写侧硬状态迁移」，而 `lifecycleOf` 两条最前置判断读的恰是**写侧** `rec.status`/`rec.pinned`。**处置待裁决（待办 D4）**。
