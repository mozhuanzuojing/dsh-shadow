...
export const buildAdaptationContext = (args: any): { ok: boolean; reason?: string; ctx?: AdaptationContext } => {
  const sourceExperience = String(args?.sourceExperience || "");
  if (!sourceExperience) return { ok: false, reason: "AdaptationContext 须 sourceExperience（调整依据是什么）" };
  const adaptationScope = String(args?.adaptationScope || "method");
  const g = assertTargetInScope(adaptationScope);
  if (!g.ok) return { ok: false, reason: g.reason };
  const ctx: AdaptationContext = { sourceExperience, validationRefs: (args?.validationRefs as string[]) || [], adaptationScope, createdAt: today() };
  return { ok: true, ctx };
};
...
export const buildAdaptationChange = async (fs: any, ws: string, args: any): Promise<{ ok: boolean; reason?: string; change?: AdaptationChange }> => {
  ...
  const basedOn = (args?.basedOn as string[]) || [];
  const sourceExperience = String(args?.sourceExperience || "");
  if (!basedOn.length || !sourceExperience) return { ok: false, reason: "Adaptation Lineage Required（Adaptation→Experience→Observation→Validation；basedOn/sourceExperience 必须）" };
  const after = String(args?.after || "");
  const g3 = assertResultNoBetterSelf(after); if (!g3.ok) return { ok: false, reason: g3.reason };
