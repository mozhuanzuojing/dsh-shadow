...
const nearestNodeAt = (graph: TemporalGraph, at: string): TemporalNode | null => {
  const t = String(at || "").slice(0, 10);
  const sorted = [...graph.nodes].sort((a, b) => String(a.timestamp).localeCompare(String(b.timestamp)));
  let best: TemporalNode | null = null;
  for (const n of sorted) if (String(n.timestamp).slice(0, 10) <= t) best = n;
  return best;
};

export const queryTemporal = (graph: TemporalGraph, q: TemporalQuery) => {
  if (q.type === "replay") {
    const n = nearestNodeAt(graph, q.at);
    if (!n) return { type: "replay", found: false };
    return {
  ...
  if (!a || !b) return { type: "compare", found: false };
  return {
...
export const renderTemporalGraph = (g: TemporalGraph) => {
  ...
  for (const n of g.nodes.slice(0, 8)) lines.push(`  node ${n.id} · ${String(n.timestamp).slice(0, 10)} · ${n.stateSnapshot.identityVersion} · visible=${n.perceptionSnapshot.visible.join("、") || "—"} · hidden=${n.perceptionSnapshot.hidden.join("、") || "—"}`);
  for (const e of g.edges.slice(0, 8)) lines.push(`  edge ${e.from} ${e.relation} ${e.to} (${e.derivation.rule})`);
  return lines.join("\n");
};

export const renderReplay = (r: any) => {
  const lines = ["[Replay]"];
  if (!r.found) { lines.push(`（无 ${r.at} 附近的观察）`); return lines.join("\n"); }
...
export const renderCompare = (r: any) => {
  const lines = ["[Compare]"];
  if (!r.found) { lines.push(`（无 ${r.from}/${r.to} 附近的观察）`); return lines.join("\n"); }
