...
+ npm run test:all             = npm run build && node tools/run-tests.ts
                               （43 个行为测试 + 2 个工具自检 = 45 项，串行、每文件一个子进程）
```
