...
**当前版本：`v1.16.2`**（把上一版留下的「仍未证明」清单逐条做掉：两个**可逆实验**分别实证了 `include` 的真实失败形态、并**推翻了 §7.1 关于配置键的整格**（两个方向都错）；由后者顺手查出并修掉一处**既有**缺陷 —— `derivedIndex` 一直不在冻结清单里，改名或删键原本**无门可守**；另补**端到端投影门**与 **T19 绊线**两个门）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
