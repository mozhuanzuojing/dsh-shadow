...
- **描述前置 + persona 的 Team 纪律（v1.20.4）**：预设描述改成「身份 → **Agent Teams** → 纪律」的顺序 —— 预设选择器的卡片把描述**截到 4 行**（`dsh-client-ui-agent-preset` 的 `cardDesc { -webkit-line-clamp: 4 }`，约 90 字），原先写在**末尾**的那句 Agent Teams 在界面上**根本看不到**（用户 2026-09-23 报的正是这个）；persona ⑤ 补上 0.1.7 `team:policy` 的执行口径：**只有用户显式要求才建 teammate**、写作用域**互不重叠**（写进共享任务的 `write_scopes`）、要排序用 `blocked_by`、任务板走 **list → get → claim（带 revision）→ complete**、**任务就绪不会唤醒 owner**、`FS_STALE_VERSION` 就重读 + rebase 重试。两者由 `test/preset-projection.test.ts` **④** 锁住（描述前 90 字必须含 `Agent Teams` + persona 六个锚点）。⚠ 上游 `team:policy` 仍是**权威且每会话自动注入**；预设这一份买的是「决定要不要开 Team 那一刻」的口径一致，persona 长度**实测 3047 → 3583 字符**（+536，代价记账见预设 README）。
...
**当前版本：`v1.20.4`**（**投影模式的可读性修复 + 团队纪律补全**：预设选择器的卡片只渲染约 4 行，Agent Teams 的硬事实已挪到前置窗口；persona 补上上游 `team:policy` 的团队纪律；本机那份冻结的旧副本备份后删除；并给这处加了一道测试闸门 —— 依据、实测与未复核项见 [`adr/0100`](./adr/0100-projection-description-window-and-team-policy.md) 与 [`CHANGELOG.md`](./CHANGELOG.md)；插件代码零改动）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
