...
**当前版本：`v1.15.98`**（**T12 重判：又抓出 6 个到期会炸的 fixture** —— 把 `Date` 钉到 2030-01-01 后逐文件跑，51 个测试里 6 个红（最早两个次日就引爆）；根因相同：fixture 写死旧日期撞上 `forget` 缺省开（`staleDays` 14），记忆被移出活跃集。修法不是改日期，而是按既有模式**显式声明这 6 个场景与遗忘无关**（`forget: { enabled: false }`），让前提不再随默认值漂移）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
