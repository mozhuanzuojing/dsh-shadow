...
判定落点见 `adr/0086` §8.10。其中 #3/#4/#5 **待核**（要读代码确认它们是否某 `mode` 的**返回内容**），
#6 **归 `T7`**。⚠ **用它们判出了登记册的一个缺口**：**没有一条契约覆盖「工具的返回内容」**
（`mode` 只决定「读哪一类」，不约束「读出来长什么样」）—— **本轮只登记缺口、不入册**。
...
**当前版本：`v1.15.75`**（「A 段 6 条」首次被枚举 —— 泳道指着它、却没有任何地方写下它是哪 6 条；新增 `audit:docs` 检查 ④）—— 最新几版摘要：
