...
**当前版本：`v1.18.1`**（把缺的 `.git` **补上**：§1.1 里那 18 个「无版本溯源」的材料逐个接回上游（**只加元数据、不动工作树文件**；快照取 **tag** 而非 main；接错即回滚），于是 **24/24 都有 HEAD**；并给生成器加了「**工作树 vs HEAD**」一列 —— 因为「有 HEAD」**不等于**「拷贝等于那一版」，实测 **10 个干净 / 14 个有差异**）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
