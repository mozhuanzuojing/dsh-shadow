...
**当前版本：`v1.18.0`**（`MATERIALS.md` 的**生成器进本仓**：一条命令重算名册，24 个目录**全部入账**（原先 18 个连名字都搜不到），`BACKLOG` 的 **T20 按 (a) 结案**；并由它实测出一条更要紧的事实 —— 这批材料里 **18 个根本没有 `.git`**，即**没有版本溯源**，所以旧表给它们记的 HEAD 在本机**复核不了**）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
