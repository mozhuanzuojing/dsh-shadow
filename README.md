...
**当前版本：`v1.17.0`**（按「**吸收**而不是**接入**」的口径收口：把「置信度」从原语里请出去 —— 删掉那个字段、删掉随附的三条分布判据，连排序视图也一并取消；`EngineDeclaration` 的字段集因此**恰好是四个**，于是 `adr/0037` 的「❌ Confidence」变成**结构性成立**（类型里没有那个槽，任何后端都引入不了），`BACKLOG` 的 **T19 随之结案**；棘轮下降并已收紧基线）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
