...
**当前版本：`v1.18.3`**（让「**材料访问前先查最新**」这条规则真的用得下去：实测实查 24 个仓 ≈ **25s**、且**调并发没用**（对端限流）⇒ 改为 `--only <目录名>` 只查要用的那个（**~1s**）+ **带 TTL 的缓存**（默认 600s，系统临时目录，第二次起 **~0.7s**）；并按指令执行了 `--hard` 更新 5 个材料（**动前先验工作树差异为 0** ⇒ 没覆盖任何本机改动），爆炸半径**核对后 0 处腐烂**，台账已重生成）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
