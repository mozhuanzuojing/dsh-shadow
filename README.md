...
**当前版本：`v1.15.78`**（发版 tag 约定成文 —— 第二条「仪式悄悄停掉」：73 个版本没打过 tag）—— 最新几版摘要：

| 版本 | 主题 |
|------|------|
| v1.15.78 | **发版 tag 约定成文**。用户要求打 tag，一查发现远端最后一个 tag 是 `v1.15.3` 而版本已到 `v1.15.77` ⇒ **`v1.15.4`…`v1.15.76`（73 个版本）从未打过 tag**。为当前版本补打 `v1.15.77` 并推送；**用户决定不追溯补打**，**从 `v1.15.77` 起往后每版都打**，约定写进 `AGENTS.md`（含历史缺口与「不补」的决定，免得下一个人发现跳号又去补）。**这是本仓第二条「靠习惯维持的仪式悄悄停掉」**（第一条是 `README` 的当前版本行，停了 25 个版本，立了检查①）—— **两条都没报错也没腐烂，它们只是停了**。细节见 `CHANGELOG`。 |
| v1.15.77 | **补齐登记册缺的那条契约（`tool-output-v1` = 工具返回内容），并因此第一次做到「按契约机械判定」**：A 段 #3/#4/#5 从「缺产品决策」变成「查契约的两半边」—— 三条**都允许接线**，但 `progressiveDisclosure` / `refineTree` 折叠掉的内容**必须由信封披露**，否则违反该契约 `hard` 半边。入册前先按上一轮的承诺**删除了散在各处的契约条数**（**条数＝下表行数**，不再手写）。**另一处教训**：上一轮我写的「16 处」同一条命令实际返回 **21 行 / 26 次**，根因是**自指** —— 写下「有 N 处写了 8」这句本身又添了几处，**这个数在写下的瞬间就不对了**。细节见 `CHANGELOG` 与 `adr/0086` §8.12。 |
| v1.15.76 | **把 A 段的「3 条待核」读码读成「1 个阻塞」**：`renderIdentityModel` / `progressiveDisclosure` / `refineTree` 接线后都会改**某条读路径的返回内容** ⇒ 它们缺的不是三个产品决策，而是**同一条契约**（登记册没有一条覆盖「工具的返回内容」）。**但该契约不能直接入册** —— 实测「8 条契约/八族」散在 **16 处**手写，须**先把计数收一处再加第 9 条**（本行只留结论，论证见 `CHANGELOG` 与 `adr/0086` §8.11）。另修三处「同一件事三个数」：`audit:docs` 的检查条数（`AGENTS.md` 写三条、工具文件头写两条、真值四条）⇒ **改为不写数**；以及一处我随手写下的「这一族工具有 7 个」（实测 **5**）。 |
| v1.15.75 | **补齐泳道下一项的「目标」**：它一直被叫做「A 段 6 条」，但**没有任何地方写过这 6 条是什么** ⇒ 本轮首次枚举（掩在一个 ✅ 已结案小节里，距引用处约 700 行），并给出各自的判据落点与一个**登记册缺口**（见 `adr/0086` §8.10）。**同时把第 4 次复发的文档重复变成门**：`audit:docs` 新增检查 ④。另外更正上一版一处口径（那处写「全仓」，实为「生产侧」）。细节见 `CHANGELOG`。 |
| v1.15.73 | **元审查（第八轮）：审 v1.15.72 本身**（`adr/0086` §8.7/§8.8）；`verify` **56/56**。**① 因果**：v1.15.72 称不给 `config-keys-v1` 加门是因为「删键**没有生产证据**（**没有生产消费者**）」—— **错**：配置键**是被消费的**（`index.ts` 的 `apply(ctx, rawConfig)`，live 走 `ctx.config`）⇒ 真实理由是**本轮范围**。**② 重复**：`adr/0086` §8.6 曾抄一张 8 行 `verification` 表与 `README` 表 B **逐行重复**（已删，只留结论）；`README` 的 v1.15.72 版本行曾**整段复制** `CHANGELOG` 同名条目（已压成摘要 + 指向）—— 与 v1.15.66 §5 同病，**而我在 v1.15.65 就为它立过规矩**。**③ 遗漏**：**模块归属表**（`BACKLOG` T15 明写「**用户要求**」）**从未做过、也从未在状态里提过** ⇒ 已补，且**用「生成」代替手写**（新探针 `t15-module-ownership.ts` **直接 import `audit-layers.lib.ts` 的判据**，不写第二份禁向表）；五字段里如实标注**只有 `Reads`/`Must not own` 可机械推**，`Owns`/`Writes` 不可推 ⇒ 只对 ADR 定过的层给结论、其余**标未核**。**④ 遗漏**：**`D1` 的再框定**从「`T15` 之后」的清单里**丢了**（此前只写过 D2/D3）⇒ 已补进 `README`/`BACKLOG`/`adr/0086`。**⑤ 干扰**：`adr/0086` 的 `§8.5` 被追加到 `§8.6`/`§8.7` **之后**（节序错 —— 与第一轮 `### 6.11`、v1.15.69 `adr/0085` 的 `§8.7/§8.8` **第三次同病**）⇒ 已移到数值位置；同轮还改了 §8.6 把「`T15` 切片 2」误称「**第八轮**」（**轮次与切片是两种计数，混用后「第几轮审了谁」无从复核**）。**⑥ 第 8 次「测量错」**：新探针把 `PURE_MODULES`（**文件级**）聚合成**层级**，于是 `core` 行同时印出「纯模块 是（0 import）」与「import 了 evidence/persistence/security/node:fs」—— **一行之内自相矛盾**，靠**读自己刚生成的那张表**拦住（改口径后为 **4/43 文件**）。**未做**：切片 3（D1/D2/D3）· 登记册仍无生产消费者 · 两处未设防仍只登记不修。 |
| v1.15.72 | **`T15` 切片 2：十字段填完（8 条契约 × 10 字段）**（`adr/0086` §8.6）；`verify` **56/56**。`README` 的「受保护契约面」改为**两张对齐的表**（表 A 结构语义 / 表 B 治理，`id` 相同）。**净发现由 `verification` 一列逼出**：八族里 **6 族有强门 · 1 族部分设防（`tool-schema-v1`：**参数名无人枚举**）· 1 族完全未设防（`config-keys-v1`：顶层 19 键**没有一处枚举键名**）**。**`ratchet` 一列 8 条均「无桶覆盖」** —— 棘轮桶按**缺陷类**分、不按契约面分，**没有为凑这一列新造桶**（唯一例外：`read-mode-v1` 的「`CONTEXT.md` 必须覆盖全部 62 个 mode」**本身就是棘轮**）。四个数由一个命令一次算清（`t15-counts-verify.ts`）：工具名 **3** · 参数 **132** · mode **62** · 配置键 **19**。**详细记录见 `CHANGELOG` v1.15.72 条目**（此处按「每层只回答自己那个问题」只留摘要）。 |
...
| v1.15.68 | **元审查（第四轮）：审 v1.15.67 本身** —— 同一句立法**第二次只落一半** + `audit:docs` 从两条扩到**三条** + 一条**关于测量的教训**（`adr/0085` §8.8）；`verify` **56/56**。**① 遗漏：同一句立法只落一半** —— v1.15.67 立了「版本号 / 计数 / 链路清单 要么别手写、要么配门」，却**只给链路清单配了门**，**计数**仍停在规则；当天实测两处：`README` 的「⇒ 现 18 行」（数字恰好对，但**没有任何东西在守它**）⇒ 新增**检查③**（声明 = 实际）；`BACKLOG` 表头「主台账现存 **20** 条（T 11 / D 6 / V 5 / G 4）」**已过期**（`T8` 于 v1.15.64/65 结案）⇒ 改为「**判据 + 命令**」（现存＝标题不带 ✅ 的条目；实测 **T 10 条**，**D/V/G 明确声明本轮未复核**，不猜不抄旧数）。**② 门自身三处缺陷**：`检查②是单向的` —— 只查「`verify` 每一步是否被文档点名」，于是**门被从 `verify` 里摘掉**时 `required` 少一项、检查反而更宽松照样全绿、而文档那行成**死指针**（历史同族：v1.15.22「三方版本一致」仪式停掉后无人发现）⇒ 改**双向**（文档点名的每个 `npm run` 也必须在 `verify` 里）；`反向检查第一版有假阳性`（把块头的 `npm run verify` 自身与内联的 `= npm run build && node …` 都当闸门）⇒ 只看 `^[+=]\s*npm run X` 那些**逐条列举**的行。**③ 关于测量的教训（本轮最重）**：检查③首跑报「声明 18 / 实际 19」，我的第一反应是**去改 README 的数字** —— 幸好先手工数了一遍（真值就是 **18**），才没把**对的文档改错**；根因是**本仓工作副本是 CRLF**，分隔行 `|----|\r` 的 `\r` 让 `$` 锚定的正则失配 ⇒ 分隔行被当成数据行。⇒ 立两条规矩：**解析前先归一化行尾**；**「工具报不一致」时默认怀疑对象是工具，尤其当检查是刚写的**（本仓已有三次同型先例，**都是测量错**：`adr/0084` §4 探针写窄判据 · §8.6 把 `src/index.ts` 当本仓 `index.ts` · 本条）。**④ 重复/干扰**：`AGENTS.md` 被我前两轮堆成「事故叙述 + 规矩」混排且与 `adr/0085` §8.7 逐字重复（我**刚**立过「同一件事只写一处」）⇒ 收口为**只留规矩与指向**。selftest **11 → 17 组**（⑫ 反向 / ⑫b 反向假阳性 / ⑬a-c 行数 / ⑭ CRLF）。**未做**：检查③只覆盖 `README` 那一处计数（`BACKLOG` 用「判据 + 命令」代替）· 未做全仓计数字段普查 · 三条检查都只是**必要条件**（答不了「顺序/描述对不对」）· **`T15` 仍未开工 —— `adr/0083` §14.1 的路线漂移在这四轮元审查之后依然存在**，我把它记在这里而不是假装没有。 |
| v1.15.67 | **元审查（第三轮）：审 v1.15.66 本身** —— 一处**我自己说错的因果** + 一处**差点丢掉 34 个版本条目** + 文档派生字段门扩成两条（`adr/0085` §8.7）；`verify` **56/56**。**① 纠正因果（我上一轮往「更可信」的方向写错）**：v1.15.66 说三方版本一致性「**只在 v1.15.22 被人工核验过一次**」—— 实测（`git log -G`，**不能用 `-S`**：它只认出现次数、同一行改值看不见）推翻：该行自 `44bcc8b`（**v1.7.0**）起**随每次发版更新**，一路维护到 `0be762e`（**v1.15.40**），之后**仪式静默断掉** —— `v1.15.41`–`v1.15.65` 共 **25 个版本**没人更新它（原来写的「26」是版本差，不是漏掉的发版数）。⇒ 诊断不是「只检查过一次」而是「**一条靠习惯维持的仪式悄悄停了**」，**比原故事更值得警惕**：习惯会无声失效。**② 遗漏：门只覆盖半类问题** —— 我上一轮刚立法「能推出来的字段（版本号 / 计数 / 链路清单）要么别手写、要么配门」，却**只给版本号配了门** ⇒ 门扩成两条（`tools/docs-consistency.ts` / `npm run audit:docs`），第二条＝**`verify` 每一步都必须被「该列它的那一块」点名**。一条命令抓出三处真缺陷：`README` 闸门块**漏 `typecheck:tests` 与本门** · `CONTEXT.md` 冒烟门格写着「**45 项（43+2）**」且漏 4 道闸 · **`AGENTS.md` 自己手抄「56 项」**（我刚立法别手写，转身又抄）。selftest 5 → **11 组**，新增三组都是被实测逼出来的：**⑥诱饵**（正文提一句「当前版本：」不得抢匹配）· **⑪假绿对照**（步骤名放到闸门块**之外**必须仍红 —— 第一版判据「文档里出现过」时，`typecheck:tests` 恰好躺在版本历史表里就照过）· **⑨结构缺失**（锚点消失报 2、漏一步报 1，修法不同不能混）。**③ 差点丢掉 34 个版本条目（留档）**：改 `CHANGELOG` 时用 `indexOf("### 6. 验证")` 当结束锚点，而该标题在更早条目里也有 ⇒ 实际删掉 `v1.15.65`–`v1.15.32` 共 **34 条**（`## [v` 104→70、−1985 行），**`Set-Content` 照常成功、零报错**；靠**事后数结构不变量**发现（且未提交 ⇒ 一条 `git checkout` 恢复）。根因不是手滑而是三条习惯缺失，已写进 `AGENTS.md`：**①锚点必须先算行区间、绝不 `indexOf` 全篇找同名标题；②`Replace` 不匹配会静默返回原文 ⇒ 必须打印匹配数、0 就拒绝写盘；③批量文档改写后先核对一个可数的不变量再提交**。⇒ 一般形态：**「命令成功」≠「改动按意图发生」**。**④ 重复**：v1.15.66 §5 整表复制了 `adr/0085` §8.6 ⇒ 本轮增量只写进 `adr/0085` §8.7，CHANGELOG 只留数字与指向（**v1.15.66 原文按「归档不改写」保留**，勘误写在新条目开头）。**未做**：`audit:docs` 第二条**只是必要条件**（答「有没有点名」，答不了「顺序 / 描述对不对」）· 未做全仓「计数字段」普查，只修了 `verify` 链路那一处 · `README` 版本历史表里的旧行号按分层**故意不动**。 |
...
| v1.15.62 | **测试面类型检查扩到全部**（**83 → 0**）+ **独立核实**「批量改测试」没偷偷放宽断言；`verify` **53/53**。`tsconfig.test.json` 的 `include` 从「三个文件」扩到 **`test/**/*.ts`** ⇒ `typecheck:tests`（已在 `verify` 内）现在守**整个测试面**，行为未变。**为什么必须核实**：批量机械改测试最容易的作弊方式是把断言改宽松、加 `@ts-ignore`、或删测试块 ⇒ 两支护城河探针：`verify-assertions-unchanged.ts`（取每个被改文件在 `HEAD` 与工作区的**所有 assert 行**，**按括号深度剥掉类型 cast** 后比较多重集）⇒ **断言逐字相同**（唯一残差 5 处「补必填字段」+1 处「多一对括号」）；`verify-added-fixture-fields.ts`（补字段理论上可能改变**会扫全部字段的守卫** ⇒ 旧/新输入喂同一编译产物）⇒ 3 处结果全同。另核 `@ts-ignore`/`@ts-expect-error` **0 处**、新增 `as any` **1 处**（带注释）。**⚠ 我自己的探针第一版有两个 bug**（正则剥 cast 遇泛型 `as Parameters<typeof x>` 剥不干净；`} )` 与 `})` 空格差异未归一化）⇒ 假报一批「文本不同」；改成**按括号深度消费 cast** 后修正 —— **先修探针，再信结论**。**顺带修一处生产侧类型缺口**：`core/knowledge-retrieval.ts:60` 声明比真实契约窄（实现读 `(h as any).__path`，而 `query/reads.ts:162,167` 确实传）⇒ 补 `__path?: string`、去掉 `as any`（**声明写窄了，类型检查会反过来逼调用方 cast**）。**一处为兼容保留的旧标签**：`world/explain` 的 `Validation History:` 未改名（`test/recall-attribution.test.ts:3230` 钉住该串），改为**保留旧前缀 + 追加纠正从句** |
| v1.15.61 | **认识论层「损坏/失败必须出声」6 处** + 三份**从未读过的目录**的审查台账；`verify` **53/53**。三个**只读**审查覆盖了全部剩余目录（`federation/`·`world/`·`reality/` · `long-horizon/`·`simulation/`·`soul/`·`planning/` · `adaptation/`·`agency/`·`continuity/`），共约 **60 条**线索记入 `BACKLOG` §七（逐条带 `文件:行号`）。**一条推翻既有假设的判定**：`adaptation/`/`agency/`/`continuity/` **三层全部已被生产接线**（→ `query/query.ts` → `index.ts:297`）⇒ 此前记成「未接线/潜在」是**错的**，那些缺陷是**当前生效**（**「未读」不等于「未接线」**）。**本轮修的 6 处**（同一族）：① `readObservations` 单 `try` 包整个循环 ⇒ 第 k 个坏文件让后续**永不读**且**静默返回前 k-1 条**，而下游 `supported` 判据**就吃 `obs.length`** ⇒ `readObservationsDetailed` 单条丢失 + 计数 + 留痕；② `readClaims` 同型但**更危险** —— 一份坏 claim ⇒ `mode:"world"` 用**残缺图不可逆覆盖**落盘 `graph.json` ⇒ 坏件时**不覆盖** + 出口披露；③ `readRealityEvidence` 同型（坏件 ⇒ 误报 `isolated`）；④ 三处**写失败渲染成成功**（`registerObservation`/`writeClaim`/`registerRealityEvidence`）⇒ 返回 `persisted`/`boolean`，mode 输出显式「**未落盘**」；⑤ `referenceEvidence` 把「读不出」压成 `null` ⇒ 输出成「（无 reality evidence X）」= **把报错当不存在** ⇒ 返回 `{evidence, reason: not_found\|unreadable}`；⑥ `world/explain` 两处失真（`Validation History: N supported claim(s)` 数的却是 **claim 条数**；subject 不匹配时**静默换成** `objects[0]`）⇒ 标签改准 + 回退**出声**。**顺带修的判据分叉**：`Number(x) \|\| 0.5` vs `x ?? 0.5` ⇒ **显式传 `0` 被静默改成 0.5** ⇒ 默认值收进 `CONFIDENCE_DEFAULT` + `confidenceOfInput`（只认「没传 ⇒ 默认」）。**三条最该先处理但未擅自改**（都需先定契约）：工具 schema 与四处读点不一致（`validations`/`visible`/`hiddenA/B` 未声明 ⇒ 要么该层恒空、要么可声明 `outcome:"validated"` 造 `supported`）· `claimOf` 的 `supported` 从不与 ValidationTimeline 交叉核对 · `planning`/`sim-action` 三处守卫永不失败。**可信度**：审查者只读、**未跑测试**，「改坏仍通过」为**静态推演**，共排除 40 余条并给理由 |
| v1.15.59 | **把 CLI 接线这条最高危盲区做成自动断言**；`verify` **53/53**。此前 6 个 selftest **100% 只调纯函数**，而本仓历史上真实踩过的两类缺陷都长在 **CLI 接线**上、一直被 footer 记成「要 spawn 子进程，未做」⇒ **先写探针**（`.docs/fix/2026-09-12/spawn-probe.ts`：本环境 `execFileSync` 管道可用、退出码可读）**再承诺**。**新增 `tools/cli-wiring.selftest.ts`（spawn 真 CLI，5 组）**：① `--ratchet` 写在 **root 位置** ⇒ 两个 CLI 都 **exit 2**（锁 v1.15.45「漏 root ⇒ ROOT 取到旗标 ⇒ 0 文件 ⇒ **静默全绿**」）；② 显式**空语料根** ⇒ 都 exit 2；③ `--update-ratchet` 坏语料 ⇒ exit 2 **且基线逐字节未变**（证明拒绝**先于** `writeFileSync`；测试内备份 + `finally` 还原）；④ 基线**必须分段** `wiring`/`drift` 且 `corpus.wiring`/`corpus.drift` **分键**（锁 v1.15.45 第二处「不同口径共用 `corpus` 键 ⇒ 判成骤降 76%」）；⑤ `run-tests.ts` 确实扫描 `tools/*.selftest.ts`（防死文件）。**判据收口/锁口**：**`retrieval-eval` 的第二份语料判据**（内联块、**零标定**，旧 footer 还误称它走 `classifyCorpus`）⇒ 抽成 **`corpusFloorVerdict`** + selftest **⑬** 锁边界，**两份判据刻意保留**（**绝对下限**拦工作区指错 vs **相对基线容许带**拦工具坏了，拦的是不同故障）；**`isProductionPath` 分叉**（wiring 把 `tools/**` 当生产面、drift 排除 ⇒ drift 看不见自家这对分叉）⇒ **不擅自统一**（会改某一侧基线口径），改为**锁住** —— 新增 ⑭ 断言「差异**恰好**是 `tools/`」，任何一侧改动即红，**逼出那个决定**。**实测修正**上一轮记录：`toolset-authority.ts --check` 未接进 `verify` 的真实原因是**耗时**（本机 **>120s**，winget 探测），不是「忘了接线」。棘轮如实变红 `b_keys 113→115`（+2 = 基线分段断言）后重录。**仍未修**：`isProductionPath` 口径统一（**等你拍板**）· `--update-ratchet` 成功路径未自动化 · 169 个测试类型错误 · §6.3 六条待定语义 · 整目录未读 |
| v1.15.58 | **披露面 4 处 + 标定测试自身的假绿（`tools/*.selftest.ts`）**；`verify` **52/52**。**披露面**：① **`shadow_query` 截断不披露**（检索路径早有 `truncationNote`，它只写 query-log、返回文本一字不提 ⇒ 两条读路径披露不一致）⇒ 附「命中 N · 只返回前 limit · **还有 k 个未显示**」。② **证据路径上限无披露**（`.slice(0,12)` 让「前 12 条都不缺失」被读成「全查过了」）⇒ 导出 `EVIDENCE_PATH_CAP` + `droppedByCap`，渲染与 `ev.unverifiedByCap` 均带出「另有 k 条**未核验**」。③ **快照坏件回退更旧后无声** ⇒ 回退时打印「跳过了哪些／实际用了哪份」+ 文档写明回退语义。④ **`observer/projection.ts` 两处失真** —— 读不出的记忆**既不进 relevant 也不进 excluded**（而 `reality.total` 按全量算 ⇒ 数字对不上却看不出为什么）；`候选相关` 报的是砍到 8 **之后**的长度（**那是上限不是命中数**）⇒ 新增 `unreadable` 与 `relTotal`。**标定测试自身的假绿**（此前从未被审、审查者点名「可能是最高危假绿源」）：**⑪ 同义反复（致命）** —— 它在测试内**重写了一遍产品侧的分桶 ternary** 再断言「四桶之和 = 总数」（同一段代码赋出的和**必然成立**）⇒ **把产品侧改成任何东西它照样全绿** ⇒ 分桶判据搬进 lib（`bucketOf`）两边共用，⑪ 重写为四桶正例 + **三条反例** + 语料非空，**变异验证**（改坏 A1 条件 ⇒ 立即红）；**`audit-drift` 判据 ③ 零标定** —— 原 NEG-2 条件里**没有 `.has(`**，在判据 ② 就被 `continue`，**根本走不到 ③**（`probeVars` 整段）⇒ 加**差分对** `POS-4`/`NEG-6`（只差「有没有探针局部名」，结果必须相反），**变异验证**（废掉 `probeVars` 填充 ⇒ 立即红）；**`isTestPath` 零覆盖**（在 CLI 里）⇒ 搬进 lib + 新增 ⑬（断言它与旧写法在产物/依赖上给出**不同**答案）；另有 `audit-layers.selftest` ③ 用空判据表、`retrieval-eval.selftest` 常量与自己比的恒真断言、`audit-drift.selftest` 真仓库断言无下限、`corpus-health.selftest` footer 与代码不符 —— 全部修掉。**新线索（未修）**：`retrieval-eval.ts:117-120` 的第二份「语料太小」判据（未标定）· `audit-wiring` ↔ `audit-drift` 的 `isProductionPath` 口径分叉（**需先拍板 `tools/` 算不算生产面**）· 全部 CLI 接线零自动断言 · `toolset-authority` 未接进 `verify`。棘轮如实变红 `b_keys 110→113`（+3 正是新增标定断言）后重录 |
| v1.15.57 | **证据面与报告面：「判不了」不得伪装成「已核实」**；`verify` **52/52**。修的是**会直接改变裁决**的两种谎。① **`fsExists` 把「判不了」返回 `true`** —— 无 `fs`／无工作区／`ref.path` 为空时 provider 报 **`verified` + `confidence 0.99` + `fresh`**：**缺件伪装成已核实**（**最高置信度那一档**），而它直接决定召回 `score×0.5` 与 stale 裁决，依据却是一次「没读到」⇒ `fsExists` 改**三态**（`exists`/`missing`/`undecidable`），`undecidable` ⇒ `status:"unavailable"`（既有状态，消费方已会打印 reason、不计入 missing）+ `confidence:0` + `stale`。② **「读不出来」被判成「不存在」** —— `readText`+`listDir` 都抛时一律 `false` ⇒ **EACCES / 后端异常**被当成「证据失效」⇒ **假漂移** ⇒ 只有**明确不存在**才算 `missing`，判据认**宿主契约自己的** `FS_NOT_FOUND`（`dsh-fs-local` 用这个，不是 `ENOENT`）与 ENOENT 文本。③ **zg 报错被改写成 `not_found`** ⇒ 一次工具失败被当成「该证据已失效」（`arbitrate` 计入 missing、召回 ×0.5 + stale）⇒ `unavailable:true, reason:"error"`，并把 `ERR_CHILD_PROCESS_STDIO_MAXBUFFER` 单列为 `output_too_large`（此前混在 `error` 里，看起来像「zg 坏了」）。④ **manifest 恒报「失败项 0」** —— `deriveShadowNodes` 的 `if (!gate.allowed) continue` **只丢不记**，而 `buildManifest("1", nodes)` **从不传第三参**（`failures`）⇒ 恒报 0 而实际有一批原子被挡在投影外（**报 0 失败比不报更坏**）⇒ 新增 `deriveShadowNodeFailures`（与 `deriveShadowNodes` **同源**）+ `rebuild(derive, failures)` + `loadOrBuildProjection` 的**惰性** failures thunk（命中投影缓存时**不多付**物化代价）+ `query/reads.ts` 接上。**闸**：不变量「**节点数 + 失败数 = 原子数**」；`renderManifest` 必须显示真实失败数且**不得**出现「失败项：0」；provider 三态（判不了 ⇒ `unavailable`/0/stale）。棘轮如实变红 `b_keys 107→110`（**+3 正是新增的三态判据**），逐条点名后重录。**仍未修**：`_index.md` 不进指纹 · 证据路径上限无披露 · `observer/projection.ts` 可见性不一致 · 快照坏件回退更旧 · `reads.ts` 截断只写 log · `episode.ts` 缺时刻被默认值掩盖 · §6.3 六条待定语义 · 169 个测试类型错误 · 整目录未读 |
| v1.15.56 | **继续 review fix all：修 5 处「报告面/闸面会说谎」**；`verify` **52/52**。**原则：报告的诚实性优先于功能**（报告「失败项 0」而实际有失败的系统会让人做错决定）。① **`audit-drift --json --update-ratchet` 把空表写进基线** —— `driftCounts` 只在**人读分支**赋值 ⇒ 走 `--json` 时保持 `{}`，**空 drift 表被写进棘轮基线**而工具照样打印「已写入棘轮基线」⇒ B 段派生**提到分支之前**两条路径共用，**端到端核对**现在写出 `drift_keys=11 / drift_sites=28`。② **`toolset-authority` 坏清单先落盘后标红** —— `writeFileSync` 在 `falseMeasured` 判定**之前** ⇒ **自己声明「必须为 0」的坏清单已被签进仓库** ⇒ 与同文件 `countInconsistency` 既有先例一致：**先判后写**、拒绝产出坏清单。③ **`registerMeta` 失败只 log** —— 记忆本体与索引**已写入**，于是它在索引里活跃而 `_meta.json` 没有它 ⇒ `hits` 永不计、生命周期恒 NEW、遗忘判据落默认值 ⇒ 返回 `boolean` + `core.lastMetaError`，读侧**独立一条**⚠（与「落盘失败」分开，是不同的事实）。④ **query-log 坏行无计数**（`catch { /* 单行坏跳过 */ }`）⇒ `badLines` + `badLinesNote` 披露（0 行时**不带**该字段，免得误以为有坏行）。⑤ **`candidateStats`/`factualOnly` 丢 `violations`** ⇒ `candidateStats.violations` 露出，`factualOnly` 写明边界与取用路径（「有记录被拒」≠「本来就没那些记录」）。**闸**：`test/review-fixes.test.ts` ⑥（两面违规数一致 —— 判据收一处）⑦（坏行计数 + 披露）。**仍未修**：`manifest.failures` 恒空 · `_index.md` 不进指纹 · zg 报错→not_found · `filesystem.ts` 读失败/不存在不分 · 缺 locator 当存在 · `adr/0083` §6.3 六条待定语义 · 169 个测试类型错误 · 整目录未读 |
