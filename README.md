...
**当前版本：`v1.17.1`**（材料入账：把 `jev-ultrafast` 补进 `MATERIALS.md`（它原先**一条都没有**），并在 `references.md` 追加 §20 记下**吸收结果** —— 吸收了什么、**刻意没吸收**什么（概率分布）、以及仍未核的边界；顺手实测出台账的**大缺口**：现枚举根 24 个目录里 **18 个**连名字都搜不到，已记为 `BACKLOG` 的 T20）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
