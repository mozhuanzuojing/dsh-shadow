...
**当前版本：`v1.15.92`**（**README 重设计**，两刀：① **第一屏重写 + 章节重排** —— 第一章不再是「兼容性（验证基线）」，改成人话第一屏（一句话是什么 · **投影三层**：灵魂 → 思维 → 笔记 → 回去长成人设新版本 · 日常只用哪 **3 个**入口 · 它明确不做什么 · 谁该用）；`### 灵魂投影系统` 升为 `##` 章并紧跟「为什么存在」；② **删掉版本历史表**（757 → 634 行）+ `audit:docs` ④ 的锚点从「表里那一行」挪到「**当前版本行**」—— 判据一字未改、负对照仍会响，详情 `adr/0094`）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
