...
**`D3`** 8 条契约**全是细粒度面、无一条是内部大对象** ⇒ 对内部大对象的重构是**允许的 breaking internal refactor**。
**仍未做**：没有为 `D2` 建「漂移类」的**可执行检测**（本轮只给分类的判据落点）。
**下一步（泳道 `adr/0083` §14.1）**：`T15` 与 `D1/D2/D3` 已判完 ⇒ 下一项是 **`A段`** ——
v1.15.75 **首次枚举**出它是哪 6 条（此前两处引用都不给内容）：`hasNoUpgradeApi` · `renderIntent` ·
`renderIdentityModel` · `progressiveDisclosure` + `refineTree` · `relationForProposal`，
判定落点见 `adr/0086` §8.10。其中 #3/#4/#5 **不是三个待核、是同一个阻塞**（v1.15.76 读码后收敛，见 §8.11）：
三条接线后都会改**某条读路径的返回内容** ⇒ 卡在**同一个登记册缺口**上（没有一条契约覆盖「工具的返回内容」）；
#6 **归 `T7`**。⚠ 补齐该契约**不能今天就入册**：那要把文档里 16 处手写的「8 条契约」一起改
（命令见 §8.11）⇒ **先把计数收一处，再加第 9 条**。
...
**当前版本：`v1.15.76`**（A 段「3 条待核」收敛为「1 个缺口」；查出契约计数散在 16 处）—— 最新几版摘要：
