...
**当前版本：`v1.18.2`**（新规则：材料（`vendor/_src`）的 git **访问前先查最新** —— 工具 `tools/materials-freshness.ts` 报「谁落后、落后到哪个 sha」，并给出**爆炸半径**（哪些文档里有该材料的 `文件:行号` 引用）；规则写进 `AGENTS.md`。要点：`--update` 只把 HEAD 移到最新（**工作树不动，读到的仍是旧内容**），`--hard` 才真更新但**会覆盖 vendored 拷贝**；**冻结快照默认不动**；它**要联网，所以不在 `verify` 里**。本次实测 **18 最新 / 6 落后**）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
