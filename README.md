...
**当前版本：`v1.19.0`**（**记录粒度改了一次**：一批里**只有工具/文件动作**时，它不再各占一个记忆文件，而是**一行一条 JSON 追加进 `.shadow/audit/<date>.jsonl`** —— 依据是实测：14,342 个记忆文件里 **93.1% 是「0 用户消息 且 0 决策」的纯动作回声**，中位仅 **439 B**，其中 **62.5% 的行是逐条重复的溯源样板**（`adr/0097`）；记忆改为「含用户/决策/思维落点的批」一条一个文件，实测 **14,342 → 5,423（2.64×）**，且审计流**一字不丢**、其材料会折叠进下一条记忆的「背景/材料」。配套新增 `capture.echo` 开关（默认 audit，可退回旧行为）与 `npm run audit:granularity` 粒度门）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
