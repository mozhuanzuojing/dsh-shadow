...
**当前版本：`v1.20.3`**（**Agent Teams 描述对齐 `0.1.7` 线 + 名额上限撤回**：`adr/0099` —— 先把 alpha.1→alpha.2 逐字节量清（唯一语义变化是 `spawn_teammate` 给 teammate 补了身份与回话方式），再把六处与实机不符的描述修正；**部署面那条 `maxMembers: 4` 的 override 整条删除**，名额随 profile 层 bundle 出厂值 8 —— 依据是 23 个会话里真 `spawn_teammate` 调用 0 次，4 从未生效；插件代码零改动）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
