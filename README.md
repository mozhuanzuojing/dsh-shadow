...
**当前版本：`v1.16.1`**（对 v1.16.0 的**自审修正**：退回并改正 **7 处**问题 —— 两处「拿注释当判据」的引用、一处被低估的门、四处判据条数不一致；把推迟的两件事**新开为 `BACKLOG` 的 T18 / T19**，并在 `adr/0096` §12 如实写下本设计**最弱的一环**）—— **完整变更历史见 [`CHANGELOG.md`](./CHANGELOG.md)**（历史只写一处：本文件不再保留版本历史表）。
