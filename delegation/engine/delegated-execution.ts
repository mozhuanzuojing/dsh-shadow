// dsh-shadow —— delegation/engine/delegated-execution.ts：委派执行边界三对象（context/check/event），并用 guard 锁死 181–189。
...
export const buildDelegationContext = (args: any): { ok: boolean; reason?: string; ctx?: DelegationContext } => {
  const authoritySource = String(args?.authoritySource || "");
  if (!AUTHORITY_SOURCE.test(authoritySource)) return { ok: false, reason: "authoritySource 须外部权威（human/system/user/delegated），禁 observer/self" };
  const objectiveRef = String(args?.objectiveRef || "");
  if (!objectiveRef || EXTERNAL_OBJ.test(objectiveRef)) return { ok: false, reason: "objectiveRef 禁自指（delegation 目标只能来自外部权威）" };
  const allowedScope = (args?.allowedScope as string[]) || [];
  const constraints = (args?.constraints as string[]) || [];
  if (!permissionNotOwnership({ scope: allowedScope.join(" "), constraint: constraints })) return { ok: false, reason: "allowedScope/constraints 禁所有权声称（Delegation ≠ Ownership：『被允许做什么』≠『拥有/能力』）" };
  const ctx: DelegationContext = { delegationId: String(args?.delegationId || `dlg-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`), authoritySource, objectiveRef, allowedScope, constraints, expiration: String(args?.expiration || ""), revocation: Boolean(args?.revocation), createdAt: today() };
  if (!contextHasNoExpansionField(ctx)) return { ok: false, reason: "DelegationContext 禁 trust/confidence/reputation/capabilityLevel（防授权历史→信任分→更多权限 绕过 184）" };
  return { ok: true, ctx };
};
...
export const recordDelegationEvent = async (fs: any, ws: string, args: any): Promise<{ ok: boolean; reason?: string; ev?: AutonomyBoundaryEvent }> => {
  ...
  if (!resultNoOwnership(result)) return { ok: false, reason: "执行结果禁所有权声称（permission to modify ≠ ownership of）" };
  if (!resultNoIdentityClaim(result)) return { ok: false, reason: "执行结果禁身份声称（保持『被委派做 X』，非『我是能做 X 的 agent』；identity 只来自 Reflection→Candidate→Evaluator）" };
  if (!resultNoPermissionUpgrade(result)) return { ok: false, reason: "执行结果禁权限升级/信任增长（Feedback ≠ Permission Upgrade：Success→Observation，非 Success→Authority）" };
  if (!resultNoLongRunAuthority(result)) return { ok: false, reason: "执行结果禁『运行更久→更受信任→权限扩大』（Long Running ≠ Self Authority）" };
  const satisfied = (args?.satisfiedConstraints as string[]) || [];
  const violated = ctx.constraints.filter((c) => !satisfied.includes(c));
  if (violated.length) return { ok: false, reason: `约束未满足：${violated.join("、")}` };
