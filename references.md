...
- **与 dsh-shadow 的关系**：浏览器自动化能力的参考实现（同清单里已有 `browser-harness`）。**不引入依赖**。
  ⚠ **别把这一行读成「本仓缺浏览器能力」** —— 宿主已有**原生面**（本文件 `## 宿主原生能力` 一节是唯一事实处；此处只给指针，不重复登记）。
...
## 吸收落地（2026-09-20 · `v1.17.0` —— 用户口径：「**不是接入，而是吸收**」）

> 用户 2026-09-20 指令「添加资料 https://github.com/browser-use/jev-ultrafast」。
> 这份材料的 §18（联网核实）与 §19（克隆 + 读码）**都已在档**，本轮补的是**最后一段：吸收结果**。
> **口径**（承本文一贯纪律，见本节上一段开头）：**不改 §18 / §19 正文**，只追加新日期段。

### 20.1 吸收了什么（判据与理由落 `adr/0096`）

| 吸收的 | 落点 | 说明 |
|---|---|---|
| **`choice` 原语** | `decision/choice.ts` 的 `choose` | Jev 唯一真有的决策题型（`model.py:81,91-106`）—— 本层的**唯一**原语 |
| **`validate_choice` 的校验口径** | `decision/guard.ts` | `choice ∈ ids`、键恰好覆盖候选集……**取的是它的口径**（`model.py:30-45`） |
| **「记录引擎声明 = 捕获，不是生成」** | `adr/0096` §5 / §12 | 支点是它把 `request` + `raw_answers` **逐字留存**（`model.py:143,147`）—— 那是**证据** |
| **Typed Decision 的形状** | `decision/types.ts` | 候选集 / `selected` / 原始依据 / 血缘 —— 类型里**恰好四个字段** |

### 20.2 **刻意没吸收**什么（本节重点）

- **它的概率分布。** 用户把口径纠正为「**吸收**」而不是「**接入**」之后，`v1.17.0` 把
  `reportedDistribution` 字段、随附的三条分布判据、**以及**唯一的排序视图（`orderByReported`）**一并删除**。
- **理由**：一个候选集上的概率分布**在语义上就是**「哪个更好的置信度」，而那正是 `adr/0037` 的
  「**❌ Confidence（决策置信度）**」。⇒ 删掉字段之后 `EngineDeclaration` 里**没有承载置信度的槽位**，
  0037 的禁令因此**结构性成立、与后端无关**（详见 `adr/0096` §12）。
- ⚠ **这是「设计改了」而不是「风险被证伪」** —— 0037 的顾虑本身**没有被推翻**，
  只是**不再有承受它的结构**。差别必须留着。
- 引擎原始输出里的数字**只留在 `rawOutput`**：逐字、归引擎，作 **provenance / 证据**。

### 20.3 仍未吸收 / 未核（诚实标注）

- **未安装、未运行、未调付费 API**（延续 §19.5；Jev 的决策本身就是 `model.py:119` 的硬编码远程服务
  `api.typesafe.ai`，且要两个付费 key）⇒「**照文档能不能真跑通**」**仍未验证**。
- §19.5 的其余未核项**一条都未变**：`snapshot.js` 未逐行、`tests/test_agent.py` 未读完、`scripts/` 6 个未读、
  `docs/design.md` 与 4 份 measurement JSON 未读、6 个 `codex/*` 分支内容未看、浅克隆 ⇒ 历史深度不可核。
